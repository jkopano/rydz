...
inline void block_input_if_console_open(
  ResMut<ecs::Input> input, Res<console::ConsoleState> console
) {
  if (console->is_open) {
    input->clear();
  }
...
inline void update_lua_camera_target_system(
  Query<Mut<IsometricCamera>> cam_query, Query<Transform, CameraTarget> target_query
) {
...
inline void setup_camera(Cmd cmd, NonSendMarker) {
  cmd.spawn(
    IsometricCameraBundle::setup(Vec3::ZERO, Vec3(10.0f, 10.0f, 10.0f), 20.0f, 12.0f)
  );
}
