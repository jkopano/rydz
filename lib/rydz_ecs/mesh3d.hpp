...
// IMPORTANT: GenMeshXxx() functions call UploadMesh internally, but due to
// a possible ABI mismatch between the C raylib struct and C++ compilation,
// the returned mesh struct may have zeroed GPU fields (vaoId=0, vboId[0]=0).
// To work around this, we clear vboId and re-upload if needed.

inline void ensure_uploaded(::Mesh &m) {
  if (m.vaoId == 0 && m.vboId != nullptr) {
    // GPU IDs were lost in return-by-value. Free the empty array and re-upload.
    RL_FREE(m.vboId);
    m.vboId = nullptr;
    UploadMesh(&m, false);
  }
...
inline ::Model cylinder(float radius = 0.5f, float height = 1,
                        int slices = 16) {
...
inline ::Model hemisphere(float radius = 0.5f, int rings = 16,
                          int slices = 16) {
