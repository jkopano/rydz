...
template <typename T> struct Handle {
  ...
  auto operator==(const Handle &o) const -> bool { return id == o.id; }
  auto operator!=(const Handle &o) const -> bool { return id != o.id; }
};
...
public:
  ...
  auto get(Handle<AssetT> handle) const -> const AssetT * {
    if (!handle.is_valid() || handle.id >= items_.size() ||
        !items_[handle.id].has_value()) {
      return nullptr;
    }
...
public:
  ...
  virtual auto load(const std::vector<u8> &data, const std::string &path)
      -> std::any = 0;
...
public:
  ...
  auto load(const std::vector<u8> &data, const std::string &path)
      -> std::any override {
...
public:
  ...
  auto register_loader(const std::shared_ptr<IAssetLoader> &loader) -> void {
  ...
  template <typename T> auto load(const std::string &path) const -> Handle<T> {
    {
  ...
  auto has_loader(const std::string &ext) const -> bool {
    return loaders_.contains(ext);
  }

private:
  static auto strip_fragment(const std::string &path) -> std::string {
    auto hash = path.find('#');
  ...
  static auto get_fragment(const std::string &path) -> std::string {
    auto hash = path.find('#');
    if (hash == std::string::npos)
      return "";
  ...
  static auto read_file(const std::string &path) -> std::vector<u8> {
