...
template <typename T> struct WorldQueryTraits<Mut<T>> {
  ...
  static auto is_valid(const Item &item) -> bool { return item.ptr != nullptr; }
...
struct WorldQueryTraits<Opt<Mut<T>>> : WorldQueryTraits<Mut<T>> {
  ...
  static auto is_valid(const typename WorldQueryTraits<Mut<T>>::Item &)
      -> bool {
    return true;
  }
...
template <> struct WorldQueryTraits<Entity> {
  ...
  struct Fetcher {
    ...
    static auto entities() -> std::span<const Entity> { return {}; }
  };
};
