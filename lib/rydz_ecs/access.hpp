...
struct SystemAccess {
  ...
  [[nodiscard]] auto is_archetype_disjoint(SystemAccess const& other) const
    -> bool {
    bool const we_are_excluded_by_other = std::ranges::any_of(
      archetype_required, [&](auto const& component_id) -> auto {
    ...
    return std::ranges::any_of(
      other.archetype_required, [&](auto const& other_component_id) -> auto {
        return archetype_excluded.contains(other_component_id);
      }
    );
  }

  auto merge(SystemAccess const& other) -> void {
    exclusive |= other.exclusive;
    main_thread_only |= other.main_thread_only;
    components_read.insert(
      other.components_read.begin(), other.components_read.end()
    );
    components_write.insert(
      other.components_write.begin(), other.components_write.end()
    );
    resources_read.insert(
      other.resources_read.begin(), other.resources_read.end()
    );
    resources_write.insert(
      other.resources_write.begin(), other.resources_write.end()
    );
    archetype_required.insert(
      other.archetype_required.begin(), other.archetype_required.end()
    );
    archetype_excluded.insert(
      other.archetype_excluded.begin(), other.archetype_excluded.end()
    );
  }

  [[nodiscard]] auto is_compatible(SystemAccess const& other) const -> bool {
    if (exclusive || other.exclusive) {
      return false;
    }

    auto are_disjoint = [](
                          std::set<std::type_index> const& set_a,
                          std::set<std::type_index> const& set_b
                        ) -> bool {
      return std::ranges::all_of(set_a, [&](auto const& type_id) -> auto {
