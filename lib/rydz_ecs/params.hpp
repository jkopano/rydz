...
public:
  ...
  auto send(const E &message) -> void { messages_->send(message); }
...
struct SystemParamTraits<NonSendMarker>
  ...
  static auto retrieve(World &, const SystemContext &) -> Item { return {}; }
  static auto available(const World &) -> bool { return true; }
};
...
template <typename T> struct SystemParamTraits<Local<T>> {
  ...
  static auto retrieve(World &, const SystemContext &, State &state) -> Item {
