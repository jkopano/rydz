...
struct Input {
  ...
  auto mouse_down(MouseButton button) const -> bool {
    return mouse_down_.contains(button);
  }
  auto mouse_pressed(MouseButton button) const -> bool {
    return mouse_pressed_.contains(button);
  }
  auto mouse_released(MouseButton button) const -> bool {
...
public:
  ...
  auto set_mouse_button_down(MouseButton button) -> void {
    if (!mouse_down_.contains(button)) {
      mouse_pressed_.insert(button);
    }
    mouse_down_.insert(button);
  }
  auto set_mouse_button_up(MouseButton button) -> void {
    if (mouse_down_.contains(button)) {
