...
struct Camera3DComponent {
  ...
  static constexpr auto perspective(float fov_y_deg = 45.0f,
                                    float near_plane = 0.1f,
                                    float far_plane = 1000.0f)
      -> Camera3DComponent {
    return {
  ...
  static constexpr auto orthographic(float height = 220.0f,
                                     float near_plane = 0.1f,
                                     float far_plane = 1000.0f)
      -> Camera3DComponent {
    return {
...
inline auto compute_camera_projection(const Camera3DComponent &comp,
                                      float aspect) -> Mat4 {
  if (comp.is_orthographic()) {
    float height = std::max(comp.orthographic_height, 0.0001f);
    float half_height = height * 0.5f;
    float half_width = half_height * aspect;
    return Mat4::orthographic_rh(-half_width, half_width, -half_height,
...
inline auto compute_camera_view(Vec3 position, Vec3 forward, Vec3 up,
                                const Camera3DComponent &comp, float aspect)
    -> CameraView {
...
inline auto compute_camera_view(const Transform &t,
                                const Camera3DComponent &comp, float aspect)
    -> CameraView {
  return compute_camera_view(t.translation, t.forward(), t.up(), comp, aspect);
}

inline auto compute_camera_view(const GlobalTransform &t,
                                const Camera3DComponent &comp, float aspect)
    -> CameraView {
