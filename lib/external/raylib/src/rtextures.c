 ...
 * Some of those formats are supported by default, to remove support, comment
 * unrequired #define in this module
 *
 ...
 *
 *   DEPENDENCIES:
 *       stb_image        - Multiple image formats loading (JPEG, PNG, BMP, TGA,
 * PSD, GIF, PIC) NOTE: stb_image has been slightly modified to support Android
 * platform stb_image_resize - Multiple image resize algorithms
 *
 *
 *   LICENSE: zlib/libpng
 *
...
#if (SUPPORT_FILEFORMAT_BMP || SUPPORT_FILEFORMAT_PNG ||                       \
     SUPPORT_FILEFORMAT_TGA || SUPPORT_FILEFORMAT_JPG ||                       \
     SUPPORT_FILEFORMAT_PSD || SUPPORT_FILEFORMAT_GIF ||                       \
     SUPPORT_FILEFORMAT_HDR || SUPPORT_FILEFORMAT_PIC ||                       \
...
#if (SUPPORT_FILEFORMAT_DDS || SUPPORT_FILEFORMAT_PKM ||                       \
     SUPPORT_FILEFORMAT_KTX || SUPPORT_FILEFORMAT_PVR ||                       \
...
#define PIXELFORMAT_UNCOMPRESSED_R5G5B5A1_ALPHA_THRESHOLD                      \
  50 // Threshold over 255 to set alpha as 0
#endif

#ifndef GAUSSIAN_BLUR_ITERATIONS
#define GAUSSIAN_BLUR_ITERATIONS                                               \
  4 // Number of box blur iterations to approximate gaussian blur
#endif
...
extern void LoadFontDefault(
  void
); // [Module: text] Loads default font, required by ImageDrawText()
...
static Vector4* LoadImageDataNormalized(
  Image image
...
Image rlLoadImage(char const* fileName) {
  Image image = {0};

#if SUPPORT_FILEFORMAT_PNG || SUPPORT_FILEFORMAT_BMP ||                        \
  SUPPORT_FILEFORMAT_TGA || SUPPORT_FILEFORMAT_JPG ||                          \
  SUPPORT_FILEFORMAT_GIF || SUPPORT_FILEFORMAT_PIC ||                          \
  SUPPORT_FILEFORMAT_HDR || SUPPORT_FILEFORMAT_PNM || SUPPORT_FILEFORMAT_PSD
...
Image LoadImageRaw(
  char const* fileName, int width, int height, int format, int headerSize
) {
  Image image = {0};
  ...
  if (fileData != NULL) {
    ...
    if (size <= dataSize) // Security check
    {
      ...
      if ((headerSize > 0) && ((headerSize + size) <= dataSize))
        dataPtr += headerSize;
...
Image LoadImageAnim(char const* fileName, int* frames) {
  Image image = {0};
  int frameCount = 0;
  ...
  if (IsFileExtension(fileName, ".gif")) {
    ...
    if (fileData != NULL) {
      int comp = 0;
      int* delays = NULL;
      image.data = stbi_load_gif_from_memory(
        fileData,
        dataSize,
        &delays,
        &image.width,
...
Image LoadImageAnimFromMemory(
  char const* fileType, unsigned char const* fileData, int dataSize, int* frames
) {
  Image image = {0};
  int frameCount = 0;
...
Image LoadImageFromMemory(
  char const* fileType, unsigned char const* fileData, int dataSize
) {
  Image image = {0};
  ...
  if (
    ...
    if (fileData != NULL) {
      ...
      image.data = stbi_load_from_memory(
        fileData, dataSize, &image.width, &image.height, &comp, 0
      );
    ...
    if (fileData != NULL) {
      ...
      image.format = (desc.channels == 4) ? PIXELFORMAT_UNCOMPRESSED_R8G8B8A8
    ...
    image.data = rl_load_dds_from_memory(
      fileData,
      dataSize,
      &image.width,
      &image.height,
    ...
    image.data = rl_load_pkm_from_memory(
      fileData,
      dataSize,
      &image.width,
      &image.height,
    ...
    image.data = rl_load_ktx_from_memory(
      fileData,
      dataSize,
      &image.width,
      &image.height,
    ...
    image.data = rl_load_pvr_from_memory(
      fileData,
      dataSize,
      &image.width,
      &image.height,
      ...
      &image.mipmaps
           ...
           (strcmp(fileType, ".ASTC") == 0)) {
    image.data = rl_load_astc_from_memory(
      fileData,
      dataSize,
      &image.width,
      &image.height,
  ...
  if (image.data != NULL)
    TRACELOG(
      LOG_INFO,
      "IMAGE: Data loaded successfully (%ix%i | %s | %i mipmaps)",
      image.width,
      image.height,
      rlGetPixelFormatName(image.format),
      image.mipmaps
    );
  else
...
Image LoadImageFromTexture(Texture2D texture) {
  ...
  if (texture.format < PIXELFORMAT_COMPRESSED_DXT1_RGB) {
    image.data = rlReadTexturePixels(
    ...
    if (image.data != NULL) {
      ...
      TRACELOG(
        LOG_INFO,
        "TEXTURE: [ID %i] Pixel data retrieved successfully",
        texture.id
      );
    } else
      TRACELOG(
        LOG_WARNING,
        "TEXTURE: [ID %i] Failed to retrieve pixel data",
        texture.id
      );
  } else
    TRACELOG(
      LOG_WARNING,
      "TEXTURE: [ID %i] Failed to retrieve compressed pixel data",
      texture.id
    );
...
bool ExportImage(Image image, char const* fileName) {
  int result = 0;
  ...
  if (false) {
  } // Required to attach following 'else' cases
#if SUPPORT_FILEFORMAT_PNG
  else if (IsFileExtension(fileName, ".png")) {
    int dataSize = 0;
    unsigned char* fileData = stbi_write_png_to_mem(
      (unsigned char const*) imgData,
    ...
    result =
      stbi_write_bmp(fileName, image.width, image.height, channels, imgData);
#endif
#if SUPPORT_FILEFORMAT_TGA
  else if (IsFileExtension(fileName, ".tga"))
    result =
      stbi_write_tga(fileName, image.width, image.height, channels, imgData);
#endif
#if SUPPORT_FILEFORMAT_JPG
  else if (IsFileExtension(fileName, ".jpg") ||
           IsFileExtension(fileName, ".jpeg"))
    result = stbi_write_jpg(
      fileName, image.width, image.height, channels, imgData, 90
    ); // JPG quality: between 1 and 100
#endif
#if SUPPORT_FILEFORMAT_QOI
    ...
    if (image.format == PIXELFORMAT_UNCOMPRESSED_R8G8B8)
      ...
      TRACELOG(
        LOG_WARNING, "IMAGE: Image pixel format must be R8G8B8 or R8G8B8A8"
      );
    ...
    TRACELOG(LOG_WARNING, "IMAGE: Export image format requested not supported");
...
unsigned char* ExportImageToMemory(
  Image image, char const* fileType, int* dataSize
) {
  ...
  *dataSize = 0;
  ...
  if ((strcmp(fileType, ".png") == 0) || (strcmp(fileType, ".PNG") == 0)) {
    fileData = stbi_write_png_to_mem(
      (unsigned char const*) image.data,
      image.width * channels,
      image.width,
      image.height,
      channels,
...
#else
  TRACELOG(
    LOG_WARNING, "IMAGE: To export image, enable flag SUPPORT_IMAGE_EXPORT"
  );
#endif
...
bool ExportImageAsCode(Image image, char const* fileName) {
  bool success = false;
...
#define TEXT_BYTES_PER_LINE 20
  ...
  byteCount += sprintf(
    txtData + byteCount,
    "// ImageAsCode exporter v1.0 - Image pixel data exported as an array of "
    "bytes         //\n"
  );
  byteCount += sprintf(
    txtData + byteCount,
  ...
  byteCount += sprintf(
    txtData + byteCount, "#define %s_WIDTH    %i\n", varFileName, image.width
  );
  byteCount += sprintf(
    txtData + byteCount, "#define %s_HEIGHT   %i\n", varFileName, image.height
  );
  byteCount += sprintf(
    txtData + byteCount,
  ...
  if (success != 0)
    TRACELOG(
      LOG_INFO, "FILEIO: [%s] Image as code exported successfully", fileName
    );
  else
    TRACELOG(
      LOG_WARNING, "FILEIO: [%s] Failed to export image as code", fileName
    );
...
// with 0 being vertical (from top to bottom), 90 being horizontal (from left to
// right) The gradient effectively rotates counter-clockwise by the specified
// amount
Image GenImageGradientLinear(
  int width, int height, int direction, rlColor start, rlColor end
) {
  ...
  // Calculate how far the top-left pixel is along the gradient direction from
  // the center of said gradient
  float startingPos = 0.5f - (cosDir * width / 2) - (sinDir * height / 2);

  // With directions that lie in the first or third quadrant (i.e. from top-left
  // to bottom-right or vice-versa), pixel (0, 0) is the farthest point on the
  // gradient (i.e. the pixel which should become one of the gradient's ends
  // color); while for directions that lie in the second or fourth quadrant,
  // that point is pixel (width, 0)
  float maxPosValue = ((signbit(sinDir) != 0) == (signbit(cosDir) != 0))
                        ? fabsf(startingPos)
                        : fabsf(startingPos + width * cosDir);
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      // Calculate the relative position of the pixel along the gradient
      // direction
      ...
      pixels[j * width + i].r =
        (int) ((float) end.r * factor + (float) start.r * (1.0f - factor));
      pixels[j * width + i].g =
        (int) ((float) end.g * factor + (float) start.g * (1.0f - factor));
      pixels[j * width + i].b =
        (int) ((float) end.b * factor + (float) start.b * (1.0f - factor));
      pixels[j * width + i].a =
        (int) ((float) end.a * factor + (float) start.a * (1.0f - factor));
    }
  }
...
Image GenImageGradientRadial(
  int width, int height, float density, rlColor inner, rlColor outer
) {
  rlColor* pixels = (rlColor*) RL_MALLOC(width * height * sizeof(rlColor));
  float radius =
    (width < height) ? (float) width / 2.0f : (float) height / 2.0f;
...
Image GenImageGradientSquare(
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      // Subtract the density from the manhattanDist, then divide by (1 -
      // density) This makes the gradient start from the center when density is
      // 0, and from the edge when density is 1
...
Image GenImageChecked(
  int width, int height, int checksX, int checksY, rlColor col1, rlColor col2
) {
  rlColor* pixels = (rlColor*) RL_MALLOC(width * height * sizeof(rlColor));
...
Image GenImagePerlinNoise(
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      //   weighting applied to each successive octave octaves    =  6     --
      //   number of "octaves" of noise3() to sum
    ...
    for (int x = 0; x < width; x++) {
      ...
      for (int i = -1; i < 2; i++) {
        ...
        for (int j = -1; j < 2; j++) {
          ...
          float dist =
            (float) hypot(x - (int) neighborSeed.x, y - (int) neighborSeed.y);
      ...
      pixels[y * width + x] =
        (rlColor) {intensityUC, intensityUC, intensityUC, 255};
    }
  }
...
Image GenImageText(int width, int height, char const* text) {
  Image image = {0};
...
Image ImageFromImage(Image image, rlRectangle rec) {
  ...
  result.data =
    RL_CALLOC((int) rec.width * (int) rec.height * bytesPerPixel, 1);
  result.format = image.format;
  result.mipmaps = 1;

  for (int y = 0; y < (int) rec.height; y++) {
    memcpy(
      ((unsigned char*) result.data) + y * (int) rec.width * bytesPerPixel,
      ((unsigned char*) image.data) +
        ((y + (int) rec.y) * image.width + (int) rec.x) * bytesPerPixel,
...
void ImageCrop(Image* image, rlRectangle crop) {
  ...
  if (image->mipmaps > 1)
    TRACELOG(
      LOG_WARNING, "Image manipulation only applied to base mipmap level"
    );
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    TRACELOG(
      LOG_WARNING, "Image manipulation not supported for compressed formats"
    );
  else {
    int bytesPerPixel = GetPixelDataSize(1, 1, image->format);

    unsigned char* croppedData = (unsigned char*) RL_MALLOC(
      (int) (crop.width * crop.height) * bytesPerPixel
    );

    // OPTION 1: Move cropped data line-by-line
    for (int y = (int) crop.y, offsetSize = 0; y < (int) (crop.y + crop.height);
         y++) {
      memcpy(
        croppedData + offsetSize,
        ((unsigned char*) image->data) +
          (y * image->width + (int) crop.x) * bytesPerPixel,
    ...
    for (int y = (int)crop.y; y < (int)(crop.y + crop.height); y++)
    {
        ...
        {
            //memcpy(croppedData + ((y - (int)crop.y)*(int)crop.width + (x -
    (int)crop.x))*bytesPerPixel, ((unsigned char *)image->data) +
    (y*image->width + x)*bytesPerPixel, bytesPerPixel); for (int i = 0; i <
    bytesPerPixel; i++) croppedData[((y - (int)crop.y)*(int)crop.width + (x -
    (int)crop.x))*bytesPerPixel + i] = ((unsigned char
...
void ImageFormat(Image* image, int newFormat) {
  ...
  if ((newFormat != 0) && (image->format != newFormat)) {
    if ((image->format < PIXELFORMAT_COMPRESSED_DXT1_RGB) &&
        (newFormat < PIXELFORMAT_COMPRESSED_DXT1_RGB)) {
      Vector4* pixels =
        LoadImageDataNormalized(*image); // Supports 8 to 32 bit per channel
      ...
      case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: {
        image->data = (unsigned char*) RL_MALLOC(
          image->width * image->height * sizeof(unsigned char)
        );

        for (int i = 0; i < image->width * image->height; i++) {
          ((unsigned char*) image->data)[i] =
            (unsigned char) ((pixels[i].x * 0.299f + pixels[i].y * 0.587f +
                              pixels[i].z * 0.114f) *
                             255.0f);
        }
      ...
      case PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA: {
        image->data = (unsigned char*) RL_MALLOC(
          image->width * image->height * 2 * sizeof(unsigned char)
        );

        for (int i = 0, k = 0; i < image->width * image->height * 2;
             i += 2, k++) {
          ((unsigned char*) image->data)[i] =
            (unsigned char) ((pixels[k].x * 0.299f +
                              (float) pixels[k].y * 0.587f +
                              (float) pixels[k].z * 0.114f) *
                             255.0f);
          ((unsigned char*) image->data)[i + 1] =
            (unsigned char) (pixels[k].w * 255.0f);
        }

      } break;
      case PIXELFORMAT_UNCOMPRESSED_R5G6B5: {
        image->data = (unsigned short*) RL_MALLOC(
        ...
        for (int i = 0; i < image->width * image->height; i++) {
          ...
          ((unsigned short*) image->data)[i] = (unsigned short) r << 11 |
                                               (unsigned short) g << 5 |
                                               (unsigned short) b;
        }
      ...
      case PIXELFORMAT_UNCOMPRESSED_R8G8B8: {
        ...
        for (int i = 0, k = 0; i < image->width * image->height * 3;
             i += 3, k++) {
          ((unsigned char*) image->data)[i] =
            (unsigned char) (pixels[k].x * 255.0f);
          ((unsigned char*) image->data)[i + 1] =
            (unsigned char) (pixels[k].y * 255.0f);
          ((unsigned char*) image->data)[i + 2] =
      ...
      case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1: {
        ...
        for (int i = 0; i < image->width * image->height; i++) {
          ...
          b = (unsigned char) (round(pixels[i].z * 31.0f));
          a = (pixels[i].w >
               ((float) PIXELFORMAT_UNCOMPRESSED_R5G5B5A1_ALPHA_THRESHOLD /
                255.0f))
                ? 1
                : 0;
      ...
      case PIXELFORMAT_UNCOMPRESSED_R8G8B8A8: {
        image->data = (unsigned char*) RL_MALLOC(
          image->width * image->height * 4 * sizeof(unsigned char)
        );

        for (int i = 0, k = 0; i < image->width * image->height * 4;
             i += 4, k++) {
          ((unsigned char*) image->data)[i] =
            (unsigned char) (pixels[k].x * 255.0f);
          ((unsigned char*) image->data)[i + 1] =
      ...
      case PIXELFORMAT_UNCOMPRESSED_R32: {
        ...
        image->data =
          (float*) RL_MALLOC(image->width * image->height * sizeof(float));

        for (int i = 0; i < image->width * image->height; i++) {
          ((float*) image->data)[i] =
            (float) (pixels[i].x * 0.299f + pixels[i].y * 0.587f +
                     pixels[i].z * 0.114f);
        }
      } break;
      case PIXELFORMAT_UNCOMPRESSED_R32G32B32: {
        image->data =
          (float*) RL_MALLOC(image->width * image->height * 3 * sizeof(float));
      ...
      case PIXELFORMAT_UNCOMPRESSED_R32G32B32A32: {
        image->data =
          (float*) RL_MALLOC(image->width * image->height * 4 * sizeof(float));
      ...
      case PIXELFORMAT_UNCOMPRESSED_R16: {
        ...
        for (int i = 0; i < image->width * image->height; i++) {
          ((unsigned short*) image->data)[i] =
            FloatToHalf((float) (pixels[i].x * 0.299f + pixels[i].y * 0.587f +
                                 pixels[i].z * 0.114f));
        }
      } break;
      case PIXELFORMAT_UNCOMPRESSED_R16G16B16: {
        image->data = (unsigned short*) RL_MALLOC(
          image->width * image->height * 3 * sizeof(unsigned short)
        );
      ...
      case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: {
        image->data = (unsigned short*) RL_MALLOC(
          image->width * image->height * 4 * sizeof(unsigned short)
        );
      ...
      // In case original image had mipmaps, generate mipmaps for formatted
      // image NOTE: Original mipmaps are replaced by new ones, if custom
      // mipmaps were used, they are lost
      if (image->mipmaps > 1) {
        image->mipmaps = 1;
        if (image->data != NULL)
          ImageMipmaps(image);
      ...
      TRACELOG(
        LOG_WARNING, "IMAGE: Data format is compressed, can not be converted"
      );
  }
}

// Create an image from text (default font)
Image ImageText(char const* text, int fontSize, rlColor color) {
  Image imText = {0};
  ...
  imText = ImageTextEx(
    GetFontDefault(), text, (float) fontSize, (float) spacing, color
...
// WARNING: Module required: rtext
Image ImageTextEx(
  Font font, char const* text, float fontSize, float spacing, rlColor tint
) {
  Image imText = {0};
#if SUPPORT_MODULE_RTEXT
  if (text == NULL)
  ...
  // NOTE: Text image is generated at font base size, later scaled to desired
  // font size
  Vector2 imSize = MeasureTextEx(
    font, text, (float) font.baseSize, spacing
  ); // WARNING: Module required: rtext
  ...
  for (int i = 0; i < textLength;) {
    ...
    int codepoint = GetCodepointNext(
      &text[i], &codepointByteCount
    ); // WARNING: Module required: rtext
    int index =
      GetGlyphIndex(font, codepoint); // WARNING: Module required: rtext
    ...
    if (codepoint == '\n') {
      ...
      if ((codepoint != ' ') && (codepoint != '\t')) {
        rlRectangle rec = {
          (float) (textOffsetX + font.glyphs[index].offsetX),
          (float) (textOffsetY + font.glyphs[index].offsetY),
          (float) font.recs[index].width,
        ...
        ImageDraw(
          ...
          (rlRectangle) {0,
                         0,
                         (float) font.glyphs[index].image.width,
                         (float) font.glyphs[index].image.height},
          rec,
          tint
        );
      }
  ...
  if (textSize.y != imSize.y) {
    ...
    if (font.texture.id == GetFontDefault().texture.id)
      ImageResizeNN(
        &imText, (int) (imSize.x * scaleFactor), (int) (imSize.y * scaleFactor)
      );
    else
      ImageResize(
        &imText, (int) (imSize.x * scaleFactor), (int) (imSize.y * scaleFactor)
...
Image ImageFromChannel(Image image, int selectedChannel) {
  ...
  if (image.format == PIXELFORMAT_UNCOMPRESSED_GRAYSCALE ||
    ...
    if (selectedChannel > 0) {
      TRACELOG(
        LOG_WARNING, "This image has only 1 channel. Setting channel to it."
      );
      selectedChannel = 0;
    }
  } else if (image.format == PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA) {
    if (selectedChannel > 1) {
      TRACELOG(
        LOG_WARNING, "This image has only 2 channels. Setting channel to alpha."
      );
      selectedChannel = 1;
    }
    ...
    if (selectedChannel > 2) {
      TRACELOG(
        LOG_WARNING, "This image has only 3 channels. Setting channel to red."
      );
      selectedChannel = 0;
    }
  }
  ...
  if (selectedChannel > 3) {
    TRACELOG(
      LOG_WARNING,
      "ImageFromChannel supports channels 0 to 3 (rgba). Setting channel to "
      "alpha."
    );
    selectedChannel = 3;
  }
  ...
  unsigned char* pixels = (unsigned char*) RL_CALLOC(
    image.width * image.height, sizeof(unsigned char)
    ...
    for (int i = 0, k = 0; i < image.width * image.height; i++) {
      ...
      case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1: {
        ...
        if (selectedChannel == 0)
          ...
          pixelValue =
            (float) ((pixel & 0b0000011111000000) >> 6) * (1.0f / 31);
        else if (selectedChannel == 2)
          pixelValue =
            (float) ((pixel & 0b0000000000111110) >> 1) * (1.0f / 31);
      ...
      case PIXELFORMAT_UNCOMPRESSED_R5G6B5: {
        ...
        if (selectedChannel == 0)
          ...
          pixelValue =
            (float) ((pixel & 0b0000011111100000) >> 5) * (1.0f / 63);
        else if (selectedChannel == 2)
      ...
      case PIXELFORMAT_UNCOMPRESSED_R4G4B4A4: {
        ...
        if (selectedChannel == 0)
          ...
          pixelValue =
            (float) ((pixel & 0b0000111100000000) >> 8) * (1.0f / 15);
        else if (selectedChannel == 2)
          pixelValue =
            (float) ((pixel & 0b0000000011110000) >> 4) * (1.0f / 15);
        else if (selectedChannel == 3)
      ...
      case PIXELFORMAT_UNCOMPRESSED_R16G16B16: {
        pixelValue =
          HalfToFloat(((unsigned short*) image.data)[k + selectedChannel]);
        k += 3;
...
void ImageResizeNN(Image* image, int newWidth, int newHeight) {
  ...
  rlColor* output =
    (rlColor*) RL_MALLOC(newWidth * newHeight * sizeof(rlColor));
...
// STBIR_DEFAULT_FILTER_DOWNSAMPLE  STBIR_FILTER_MITCHELL   (high-quality
// Catmull-Rom)
void ImageResize(Image* image, int newWidth, int newHeight) {
  ...
  if ((image->format == PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) ||
    ...
    unsigned char* output =
      (unsigned char*) RL_MALLOC(newWidth * newHeight * bytesPerPixel);
    ...
    // NOTE: Color data is cast to (unsigned char *), there shouldn't been any
    // problem...
    stbir_resize_uint8_linear(
...
void ImageResizeCanvas(
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    unsigned char* resizedData =
      (unsigned char*) RL_CALLOC(newWidth * newHeight * bytesPerPixel, 1);
    ...
    for (int y = 1; y < newHeight; y++) {
      memcpy(
        resizedData + y * newWidth * bytesPerPixel,
        resizedData,
    ...
    int dstOffsetSize =
      ((int) dstPos.y * newWidth + (int) dstPos.x) * bytesPerPixel;

    for (int y = 0; y < (int) srcRec.height; y++) {
      memcpy(
        resizedData + dstOffsetSize,
        ((unsigned char*) image->data) +
          ((y + (int) srcRec.y) * image->width + (int) srcRec.x) *
            bytesPerPixel,
...
void ImageToPOT(Image* image, rlColor fill) {
  ...
  // NOTE: Add the required amount of pixels at the right and bottom sides of
  // image...
...
void ImageAlphaClear(Image* image, rlColor color, float threshold) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1: {
      ...
      for (int i = 0; i < image->width * image->height; i++) {
        if ((((unsigned short*) image->data)[i] & 0b0000000000000001) <=
            thresholdValue) {
          ((unsigned short*) image->data)[i] =
            (unsigned short) r << 11 | (unsigned short) g << 6 |
            (unsigned short) b << 1 | (unsigned short) a;
    ...
    case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: {
      for (int i = 3; i < image->width * image->height * 4; i += 4) {
        if (HalfToFloat(((unsigned short*) image->data)[i]) <= threshold) {
          ((unsigned short*) image->data)[i - 3] =
            FloatToHalf((float) color.r / 255.0f);
          ((unsigned short*) image->data)[i - 2] =
            FloatToHalf((float) color.g / 255.0f);
          ((unsigned short*) image->data)[i - 1] =
            FloatToHalf((float) color.b / 255.0f);
          ((unsigned short*) image->data)[i] =
            FloatToHalf((float) color.a / 255.0f);
        }
      }
    } break;
    default:
...
void ImageAlphaMask(Image* image, Image alphaMask) {
  if ((image->width != alphaMask.width) ||
      (image->height != alphaMask.height)) {
    TRACELOG(LOG_WARNING, "IMAGE: Alpha mask must be same size as image");
  } else if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB) {
    TRACELOG(
      LOG_WARNING,
      "IMAGE: Alpha mask can not be applied to compressed data formats"
    );
  } else {
    ...
    if (image->format == PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) {
      unsigned char* data =
        (unsigned char*) RL_MALLOC(image->width * image->height * 2);

      // Apply alpha mask to alpha channel
      for (int i = 0, k = 0;
           (i < mask.width * mask.height) || (i < image->width * image->height);
           i++, k += 2) {
...
void ImageBlurGaussian(Image* image, int blurSize) {
  ...
  Vector4* pixelsCopy1 =
    (Vector4*) RL_MALLOC((image->height) * (image->width) * sizeof(Vector4));
  Vector4* pixelsCopy2 =
    (Vector4*) RL_MALLOC((image->height) * (image->width) * sizeof(Vector4));
  ...
  // Repeated convolution of rectangular window signal by itself converges to a
  ...
  for (int j = 0; j < GAUSSIAN_BLUR_ITERATIONS; j++) {
    ...
    for (int col = 0; col < image->width; col++) {
      ...
      for (int y = 0; y < image->height; y++) {
        ...
        pixelsCopy1[y * image->width + col].x =
          (unsigned char) (avgR / convolutionSize);
        pixelsCopy1[y * image->width + col].y =
          (unsigned char) (avgG / convolutionSize);
        pixelsCopy1[y * image->width + col].z =
          (unsigned char) (avgB / convolutionSize);
        pixelsCopy1[y * image->width + col].w =
          (unsigned char) (avgAlpha / convolutionSize);
      }
    }
  }
  ...
  for (int i = 0; i < (image->width) * (image->height); i++) {
    if (pixelsCopy1[i].w == 0.0f) {
      ...
      pixels[i].r =
        (unsigned char) fminf((float) pixelsCopy1[i].x / alpha, 255.0);
      pixels[i].g =
        (unsigned char) fminf((float) pixelsCopy1[i].y / alpha, 255.0);
      pixels[i].b =
        (unsigned char) fminf((float) pixelsCopy1[i].z / alpha, 255.0);
...
void ImageKernelConvolution(Image* image, float const* kernel, int kernelSize) {
  if ((image->data == NULL) || (image->width == 0) || (image->height == 0) ||
      kernel == NULL)
    return;
  ...
  if (kernelWidth * kernelWidth != kernelSize) {
    TRACELOG(
      LOG_WARNING, "IMAGE: Convolution kernel must be square to be applied"
    );
    return;
  }
      ...
      for (int xk = startRange; xk < endRange; xk++) {
        for (int yk = startRange; yk < endRange; yk++) {
          ...
          if (imgindex >= (unsigned int) (image->width * image->height)) {
            ...
            temp[kernelWidth * xkabs + ykabs].x =
              ((float) pixels[imgindex].r) / 255.0f *
              kernel[kernelWidth * xkabs + ykabs];
            temp[kernelWidth * xkabs + ykabs].y =
              ((float) pixels[imgindex].g) / 255.0f *
              kernel[kernelWidth * xkabs + ykabs];
            temp[kernelWidth * xkabs + ykabs].z =
              ((float) pixels[imgindex].b) / 255.0f *
              kernel[kernelWidth * xkabs + ykabs];
            temp[kernelWidth * xkabs + ykabs].w =
              ((float) pixels[imgindex].a) / 255.0f *
              kernel[kernelWidth * xkabs + ykabs];
          }
        }
      }
...
void ImageMipmaps(Image* image) {
  ...
  int mipSize = GetPixelDataSize(
    mipWidth, mipHeight, image->format
  ...
  while ((mipWidth != 1) || (mipHeight != 1)) {
    ...
    TRACELOG(
      LOG_DEBUG,
      "IMAGE: Next mipmap level: %i x %i - current size %i",
      mipWidth,
      mipHeight,
      mipSize
    );
  ...
  if (image->mipmaps < mipCount) {
    ...
    for (int i = 1; i < mipCount; i++) {
      ...
      TRACELOG(
        LOG_DEBUG,
        "IMAGE: Generating mipmap level: %i (%i x %i) - size: %i - offset: "
        "0x%x",
        i,
        mipWidth,
        mipHeight,
      ...
      ImageResize(
        &imCopy, mipWidth, mipHeight
      ); // Uses internally Mitchell cubic downscale filter
...
void ImageDither(Image* image, int rBpp, int gBpp, int bBpp, int aBpp) {
  ...
  if ((rBpp + gBpp + bBpp + aBpp) > 16) {
    TRACELOG(
      LOG_WARNING,
      "IMAGE: Unsupported dithering bpps (%ibpp), only 16bpp or lower modes "
      "supported",
      (rBpp + gBpp + bBpp + aBpp)
    );
  } else {
    ...
    if ((image->format != PIXELFORMAT_UNCOMPRESSED_R8G8B8) &&
      ...
      TRACELOG(
        LOG_WARNING,
        "IMAGE: Format is already 16bpp or lower, dithering could have no "
        "effect"
      );
    }
    ...
    if ((rBpp == 5) && (gBpp == 6) && (bBpp == 5) && (aBpp == 0))
      ...
      TRACELOG(
        LOG_WARNING,
        "IMAGE: Unsupported dithered OpenGL internal format: %ibpp "
        "(R%iG%iB%iA%i)",
        (rBpp + gBpp + bBpp + aBpp),
        rBpp,
        gBpp,
...
#define MIN(a, b) (((a) < (b)) ? (a) : (b))
    ...
    for (int y = 0; y < image->height; y++) {
      for (int x = 0; x < image->width; x++) {
        ...
        // NOTE: New pixel obtained by bits truncate, it would be better to
        // round values (check ImageFormat())
        newPixel.r = oldPixel.r >> (8 - rBpp); // R bits
        newPixel.g = oldPixel.g >> (8 - gBpp); // G bits
        newPixel.b = oldPixel.b >> (8 - bBpp); // B bits
        newPixel.a = oldPixel.a >> (8 - aBpp); // A bits (not used on dithering)

        // NOTE: Error must be computed between new and old pixel but using same
        // number of bits, to know how much color precision has been lost
        ...
        if (x < (image->width - 1)) {
          pixels[y * image->width + x + 1].r = MIN(
            (int) pixels[y * image->width + x + 1].r +
              (int) ((float) rError * 7.0f / 16),
            0xff
          );
          pixels[y * image->width + x + 1].g = MIN(
            (int) pixels[y * image->width + x + 1].g +
              (int) ((float) gError * 7.0f / 16),
            0xff
          );
          pixels[y * image->width + x + 1].b = MIN(
            (int) pixels[y * image->width + x + 1].b +
              (int) ((float) bError * 7.0f / 16),
            0xff
          );
        }

        if ((x > 0) && (y < (image->height - 1))) {
          pixels[(y + 1) * image->width + x - 1].r = MIN(
            (int) pixels[(y + 1) * image->width + x - 1].r +
              (int) ((float) rError * 3.0f / 16),
            0xff
          );
          pixels[(y + 1) * image->width + x - 1].g = MIN(
            (int) pixels[(y + 1) * image->width + x - 1].g +
              (int) ((float) gError * 3.0f / 16),
            0xff
          );
          pixels[(y + 1) * image->width + x - 1].b = MIN(
            (int) pixels[(y + 1) * image->width + x - 1].b +
              (int) ((float) bError * 3.0f / 16),
            0xff
          );
        }

        if (y < (image->height - 1)) {
          pixels[(y + 1) * image->width + x].r = MIN(
            (int) pixels[(y + 1) * image->width + x].r +
              (int) ((float) rError * 5.0f / 16),
            0xff
          );
          pixels[(y + 1) * image->width + x].g = MIN(
            (int) pixels[(y + 1) * image->width + x].g +
              (int) ((float) gError * 5.0f / 16),
            0xff
          );
          pixels[(y + 1) * image->width + x].b = MIN(
            (int) pixels[(y + 1) * image->width + x].b +
              (int) ((float) bError * 5.0f / 16),
            0xff
          );
        }
        ...
        if ((x < (image->width - 1)) && (y < (image->height - 1))) {
          pixels[(y + 1) * image->width + x + 1].r = MIN(
            (int) pixels[(y + 1) * image->width + x + 1].r +
              (int) ((float) rError * 1.0f / 16),
            0xff
          );
          pixels[(y + 1) * image->width + x + 1].g = MIN(
            (int) pixels[(y + 1) * image->width + x + 1].g +
              (int) ((float) gError * 1.0f / 16),
            0xff
          );
          pixels[(y + 1) * image->width + x + 1].b = MIN(
            (int) pixels[(y + 1) * image->width + x + 1].b +
              (int) ((float) bError * 1.0f / 16),
            0xff
          );
        }
        ...
        ((unsigned short*) image->data)[y * image->width + x] =
          (rPixel << (gBpp + bBpp + aBpp)) | (gPixel << (bBpp + aBpp)) |
          (bPixel << aBpp) | aPixel;
      }
    }
...
void ImageFlipVertical(Image* image) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    unsigned char* flippedData =
      (unsigned char*) RL_MALLOC(image->width * image->height * bytesPerPixel);

    for (int i = (image->height - 1), offsetSize = 0; i >= 0; i--) {
      memcpy(
        flippedData + offsetSize,
        ((unsigned char*) image->data) + i * image->width * bytesPerPixel,
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    for (int y = 0; y < image->height; y++) {
      for (int x = 0; x < image->width; x++) {
        ...
        // memcpy(flippedData + (y*image->width + x)*bytesPerPixel, ((unsigned
        // char *)image->data) + (y*image->width + (image->width - 1 -
        ...
        for (int i = 0; i < bytesPerPixel; i++)
          flippedData[(y * image->width + x) * bytesPerPixel + i] =
            ((unsigned char*) image->data)
              [(y * image->width + (image->width - 1 - x)) * bytesPerPixel + i];
      }
    }
    ...
    for (int y = 0; y < image->height; y++)
    {
        for (int x = 0; x < image->width/2; x++)
        {
            ...
            ptr[y*image->width + x] = ptr[y*image->width + (image->width - 1 -
    x)]; ptr[y*image->width + (image->width - 1 - x)] = backup;
        }
    }
    */
  }
...
void ImageRotate(Image* image, int degrees) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    int width = (int) (fabsf(image->width * cosRadius) +
                       fabsf(image->height * sinRadius));
    int height = (int) (fabsf(image->height * cosRadius) +
                        fabsf(image->width * sinRadius));

    int bytesPerPixel = GetPixelDataSize(1, 1, image->format);
    unsigned char* rotatedData =
      (unsigned char*) RL_CALLOC(width * height, bytesPerPixel);

    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        float oldX =
          ((x - width / 2.0f) * cosRadius + (y - height / 2.0f) * sinRadius) +
          image->width / 2.0f;
        float oldY =
          ((y - height / 2.0f) * cosRadius - (x - width / 2.0f) * sinRadius) +
          image->height / 2.0f;

        if ((oldX >= 0) && (oldX < image->width) && (oldY >= 0) &&
            (oldY < image->height)) {
          ...
          for (int i = 0; i < bytesPerPixel; i++) {
            float f1 = ((
                          unsigned char*
            ) image->data)[(y1 * image->width + x1) * bytesPerPixel + i];
            float f2 = ((
                          unsigned char*
            ) image->data)[(y1 * image->width + x2) * bytesPerPixel + i];
            float f3 = ((
                          unsigned char*
            ) image->data)[(y2 * image->width + x1) * bytesPerPixel + i];
            float f4 = ((
                          unsigned char*
            ) image->data)[(y2 * image->width + x2) * bytesPerPixel + i];

            float val = f1 * (1 - px) * (1 - py) + f2 * px * (1 - py) +
                        f3 * (1 - px) * py + f4 * px * py;

            rotatedData[(y * width + x) * bytesPerPixel + i] =
              (unsigned char) val;
          }
        }
      }
...
void ImageRotateCW(Image* image) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    for (int y = 0; y < image->height; y++) {
      for (int x = 0; x < image->width; x++) {
        // memcpy(rotatedData + (x*image->height + (image->height - y -
        // 1))*bytesPerPixel, ((unsigned char *)image->data) + (y*image->width +
        // x)*bytesPerPixel, bytesPerPixel);
        for (int i = 0; i < bytesPerPixel; i++)
          rotatedData
            [(x * image->height + (image->height - y - 1)) * bytesPerPixel +
             i] = ((unsigned char*)
                     image->data)[(y * image->width + x) * bytesPerPixel + i];
      }
    }
...
void ImageRotateCCW(Image* image) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    for (int y = 0; y < image->height; y++) {
      for (int x = 0; x < image->width; x++) {
        // memcpy(rotatedData + (x*image->height + y))*bytesPerPixel, ((unsigned
        // char *)image->data) + (y*image->width + (image->width - x -
        // 1))*bytesPerPixel, bytesPerPixel);
        for (int i = 0; i < bytesPerPixel; i++)
          rotatedData[(x * image->height + y) * bytesPerPixel + i] =
            ((unsigned char*) image->data)
              [(y * image->width + (image->width - x - 1)) * bytesPerPixel + i];
      }
    }
...
void ImageColorTint(Image* image, rlColor color) {
  ...
  for (int i = 0; i < image->width * image->height; i++) {
    unsigned char r =
      (unsigned char) (((int) pixels[i].r * (int) color.r) / 255);
    unsigned char g =
      (unsigned char) (((int) pixels[i].g * (int) color.g) / 255);
    unsigned char b =
      (unsigned char) (((int) pixels[i].b * (int) color.b) / 255);
    unsigned char a =
      (unsigned char) (((int) pixels[i].a * (int) color.a) / 255);
...
rlColor* LoadImageColors(Image image) {
  ...
  rlColor* pixels =
    (rlColor*) RL_MALLOC(image.width * image.height * sizeof(rlColor));
  ...
  if (image.format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    if ((image.format == PIXELFORMAT_UNCOMPRESSED_R32) ||
      ...
      TRACELOG(
        LOG_WARNING,
        "IMAGE: Pixel format converted from 32bit to 8bit per channel"
      );
    ...
    if ((image.format == PIXELFORMAT_UNCOMPRESSED_R16) ||
      ...
      TRACELOG(
        LOG_WARNING,
        "IMAGE: Pixel format converted from 16bit to 8bit per channel"
      );
    ...
    for (int i = 0, k = 0; i < image.width * image.height; i++) {
      ...
      case PIXELFORMAT_UNCOMPRESSED_R16: {
        pixels[i].r =
          (unsigned char) (HalfToFloat(((unsigned short*) image.data)[k]) *
                           255.0f);
        pixels[i].g = 0;
        pixels[i].b = 0;
        pixels[i].a = 255;
      ...
      case PIXELFORMAT_UNCOMPRESSED_R16G16B16: {
        pixels[i].r =
          (unsigned char) (HalfToFloat(((unsigned short*) image.data)[k]) *
                           255.0f);
        pixels[i].g =
          (unsigned char) (HalfToFloat(((unsigned short*) image.data)[k + 1]) *
                           255.0f);
        pixels[i].b =
          (unsigned char) (HalfToFloat(((unsigned short*) image.data)[k + 2]) *
                           255.0f);
        pixels[i].a = 255;
      ...
      case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: {
        ...
        pixels[i].b =
          (unsigned char) (HalfToFloat(((unsigned short*) image.data)[k + 2]) *
                           255.0f);
        pixels[i].a =
          (unsigned char) (HalfToFloat(((unsigned short*) image.data)[k + 3]) *
                           255.0f);
      ...
      if (pixels[i].a > 0) {
        ...
        if (!colorInPalette) {
          ...
          if (palCount >= maxPaletteSize) {
            ...
            TRACELOG(
              LOG_WARNING,
              "IMAGE: Palette is greater than %i colors",
              maxPaletteSize
            );
          }
        }
...
void UnloadImageColors(rlColor* colors) { RL_FREE(colors); }

// Unload colors palette loaded with LoadImagePalette()
void UnloadImagePalette(rlColor* colors) { RL_FREE(colors); }
...
rlColor GetImageColor(Image image, int x, int y) {
  ...
  if ((x >= 0) && (x < image.width) && (y >= 0) && (y < image.height)) {
    ...
    case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1: {
      ...
      color.r = (unsigned char) ((float) ((pixel & 0b1111100000000000) >> 11) *
                                 (255 / 31));
      color.g = (unsigned char) ((float) ((pixel & 0b0000011111000000) >> 6) *
                                 (255 / 31));
      color.b = (unsigned char) ((float) ((pixel & 0b0000000000111110) >> 1) *
                                 (255 / 31));
    ...
    case PIXELFORMAT_UNCOMPRESSED_R5G6B5: {
      ...
      color.r = (unsigned char) ((float) ((pixel & 0b1111100000000000) >> 11) *
                                 (255 / 31));
      color.g = (unsigned char) ((float) ((pixel & 0b0000011111100000) >> 5) *
                                 (255 / 63));
      color.b =
        (unsigned char) ((float) (pixel & 0b0000000000011111) * (255 / 31));
      color.a = 255;
    ...
    case PIXELFORMAT_UNCOMPRESSED_R4G4B4A4: {
      ...
      color.r = (unsigned char) ((float) ((pixel & 0b1111000000000000) >> 12) *
                                 (255 / 15));
      color.g = (unsigned char) ((float) ((pixel & 0b0000111100000000) >> 8) *
                                 (255 / 15));
      color.b = (unsigned char) ((float) ((pixel & 0b0000000011110000) >> 4) *
                                 (255 / 15));
      color.a =
        (unsigned char) ((float) (pixel & 0b0000000000001111) * (255 / 15));
    ...
    case PIXELFORMAT_UNCOMPRESSED_R8G8B8: {
      color.r = (unsigned char) ((unsigned char*)
                                   image.data)[(y * image.width + x) * 3];
      color.g = (unsigned char) ((unsigned char*)
                                   image.data)[(y * image.width + x) * 3 + 1];
      color.b = (unsigned char) ((unsigned char*)
                                   image.data)[(y * image.width + x) * 3 + 2];
      color.a = 255;

    } break;
    case PIXELFORMAT_UNCOMPRESSED_R32: {
      color.r =
        (unsigned char) (((float*) image.data)[y * image.width + x] * 255.0f);
      color.g = 0;
      color.b = 0;
      color.a = 255;
    ...
    case PIXELFORMAT_UNCOMPRESSED_R32G32B32: {
      color.r =
        (unsigned char) (((float*) image.data)[(y * image.width + x) * 3] *
                         255.0f);
      color.g =
        (unsigned char) (((float*) image.data)[(y * image.width + x) * 3 + 1] *
                         255.0f);
      color.b =
        (unsigned char) (((float*) image.data)[(y * image.width + x) * 3 + 2] *
                         255.0f);
      color.a = 255;
    ...
    case PIXELFORMAT_UNCOMPRESSED_R32G32B32A32: {
      color.r =
        (unsigned char) (((float*) image.data)[(y * image.width + x) * 4] *
                         255.0f);
      color.g =
        (unsigned char) (((float*) image.data)[(y * image.width + x) * 4 + 1] *
                         255.0f);
      color.b =
        (unsigned char) (((float*) image.data)[(y * image.width + x) * 4 + 2] *
                         255.0f);
      color.a =
        (unsigned char) (((float*) image.data)[(y * image.width + x) * 4 + 3] *
                         255.0f);

    } break;
    case PIXELFORMAT_UNCOMPRESSED_R16: {
      color.r =
        (unsigned char) (HalfToFloat(
                           ((unsigned short*) image.data)[y * image.width + x]
                         ) *
                         255.0f);
      color.g = 0;
    ...
    case PIXELFORMAT_UNCOMPRESSED_R16G16B16: {
      color.r = (unsigned char) (HalfToFloat((
                                   (unsigned short*) image.data
                                 )[(y * image.width + x) * 3]) *
                                 255.0f);
      color.g = (unsigned char) (HalfToFloat((
                                   (unsigned short*) image.data
                                 )[(y * image.width + x) * 3 + 1]) *
                                 255.0f);
      color.b = (unsigned char) (HalfToFloat((
                                   (unsigned short*) image.data
                                 )[(y * image.width + x) * 3 + 2]) *
                                 255.0f);
      color.a = 255;
    ...
    case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: {
      ...
      color.b = (unsigned char) (HalfToFloat((
                                 ...
                                 255.0f);
      color.a = (unsigned char) (HalfToFloat((
                                   (unsigned short*) image.data
                                 )[(y * image.width + x) * 4 + 3]) *
                                 255.0f);
    ...
    default:
      TRACELOG(
        LOG_WARNING, "Compressed image format does not support color reading"
      );
      break;
    }
  } else
...
void ImageClearBackground(Image* dst, rlColor color) {
  ...
  for (int i = 1; i < totalPixels; i *= 2) {
    ...
    memcpy(
      pSrcPixel + i * bytesPerPixel, pSrcPixel, pixelsToCopy * bytesPerPixel
    );
  }
}
...
void ImageDrawPixel(Image* dst, int x, int y, rlColor color) {
  // Security check to avoid program crash
  if ((dst->data == NULL) || (x < 0) || (x >= dst->width) || (y < 0) ||
      (y >= dst->height))
    return;
  ...
  case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: {
    ...
    unsigned char gray =
      (unsigned char) ((coln.x * 0.299f + coln.y * 0.587f + coln.z * 0.114f) *
                       255.0f);
  ...
  case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1: {
    ...
    unsigned char a =
      (coln.w >
       ((float) PIXELFORMAT_UNCOMPRESSED_R5G5B5A1_ALPHA_THRESHOLD / 255.0f))
        ? 1
        : 0;
  ...
  case PIXELFORMAT_UNCOMPRESSED_R32: {
    ...
    ((float*) dst->data)[y * dst->width + x] =
      coln.x * 0.299f + coln.y * 0.587f + coln.z * 0.114f;
  ...
  case PIXELFORMAT_UNCOMPRESSED_R16: {
    ...
    ((unsigned short*) dst->data)[y * dst->width + x] =
      FloatToHalf(coln.x * 0.299f + coln.y * 0.587f + coln.z * 0.114f);
  ...
  case PIXELFORMAT_UNCOMPRESSED_R16G16B16: {
    ...
    ((unsigned short*) dst->data)[(y * dst->width + x) * 3] =
      FloatToHalf(coln.x);
    ((unsigned short*) dst->data)[(y * dst->width + x) * 3 + 1] =
      FloatToHalf(coln.y);
    ((unsigned short*) dst->data)[(y * dst->width + x) * 3 + 2] =
      FloatToHalf(coln.z);
  } break;
  case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: {
    // NOTE: Calculate R32G32B32A32 equivalent color (normalized to 32bit)
    ...
    ((unsigned short*) dst->data)[(y * dst->width + x) * 4] =
      FloatToHalf(coln.x);
    ((unsigned short*) dst->data)[(y * dst->width + x) * 4 + 1] =
      FloatToHalf(coln.y);
    ((unsigned short*) dst->data)[(y * dst->width + x) * 4 + 2] =
      FloatToHalf(coln.z);
    ((unsigned short*) dst->data)[(y * dst->width + x) * 4 + 3] =
      FloatToHalf(coln.w);
...
void ImageDrawLineEx(
  Image* dst, Vector2 start, Vector2 end, int thick, rlColor color
) {
  // Round start and end positions to nearest integer coordinates
  int x1 = (int) (start.x + 0.5f);
  int y1 = (int) (start.y + 0.5f);
...
void ImageDrawCircle(
  Image* dst, int centerX, int centerY, int radius, rlColor color
) {
  int x = 0;
  int y = radius;
...
void ImageDrawCircleLinesV(
  Image* dst, Vector2 center, int radius, rlColor color
) {
...
void ImageDrawRectangle(
  Image* dst, int posX, int posY, int width, int height, rlColor color
) {
  ImageDrawRectangleRec(
    dst,
    (rlRectangle) {(float) posX, (float) posY, (float) width, (float) height},
    color
  );
}
...
void ImageDrawRectangleV(
  Image* dst, Vector2 position, Vector2 size, rlColor color
) {
  ImageDrawRectangle(
...
// Draw rectangle within an image
void ImageDrawRectangleRec(Image* dst, rlRectangle rec, rlColor color) {
  // Security check to avoid program crash
  if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0))
    return;
  ...
  for (int x = 1; x < (int) rec.width; x *= 2) {
    ...
    memcpy(
      pSrcPixel + x * bytesPerPixel, pSrcPixel, pixelsToCopy * bytesPerPixel
    );
  }
  ...
  for (int y = 1; y < (int) rec.height; y++) {
    memcpy(
      pSrcPixel + (y * dst->width) * bytesPerPixel, pSrcPixel, bytesPerRow
    );
  }
}
...
void ImageDrawRectangleLines(
  Image* dst, rlRectangle rec, int thick, rlColor color
) {
  ImageDrawRectangle(
    dst, (int) rec.x, (int) rec.y, (int) rec.width, thick, color
  );
  ImageDrawRectangle(
    dst,
    (int) rec.x,
    (int) (rec.y + thick),
    thick,
    (int) (rec.height - thick * 2),
    color
  );
  ImageDrawRectangle(
    dst,
    (int) (rec.x + rec.width - thick),
    (int) (rec.y + thick),
    thick,
    (int) (rec.height - thick * 2),
    color
...
void ImageDrawTriangle(
  Image* dst, Vector2 v1, Vector2 v2, Vector2 v3, rlColor color
) {
  // Calculate the 2D bounding box of the triangle
  // Determine the minimum and maximum x and y coordinates of the triangle
  // vertices
  int xMin = (int) ((v1.x < v2.x) ? ((v1.x < v3.x) ? v1.x : v3.x)
                                  : ((v2.x < v3.x) ? v2.x : v3.x));
  int yMin = (int) ((v1.y < v2.y) ? ((v1.y < v3.y) ? v1.y : v3.y)
                                  : ((v2.y < v3.y) ? v2.y : v3.y));
  int xMax = (int) ((v1.x > v2.x) ? ((v1.x > v3.x) ? v1.x : v3.x)
                                  : ((v2.x > v3.x) ? v2.x : v3.x));
  int yMax = (int) ((v1.y > v2.y) ? ((v1.y > v3.y) ? v1.y : v3.y)
                                  : ((v2.y > v3.y) ? v2.y : v3.y));
  ...
  // Calculate the initial barycentric coordinates for the top-left point of the
  // bounding box
...
void ImageDrawTriangleEx(
  ...
  // Calculate the inverse of the sum of the barycentric coordinates for
  // normalization
...
void ImageDrawTriangleFan(
  Image* dst, Vector2 const* points, int pointCount, rlColor color
) {
  if (pointCount >= 3) {
...
void ImageDrawTriangleStrip(
  ...
  if (pointCount >= 3) {
    for (int i = 2; i < pointCount; i++) {
      if ((i % 2) == 0)
        ImageDrawTriangle(dst, points[i], points[i - 2], points[i - 1], color);
      else
        ImageDrawTriangle(dst, points[i], points[i - 1], points[i - 2], color);
    }
  }
}
...
void ImageDraw(
  Image* dst, Image src, rlRectangle srcRec, rlRectangle dstRec, rlColor tint
) {
  // Security check to avoid program crash
  if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0) ||
  ...
  if (dst->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    if (((int) srcRec.width != (int) dstRec.width) ||
      ...
      ImageResize(
        &srcMod, (int) dstRec.width, (int) dstRec.height
      ); // Resize to destination rectangle
      srcRec =
        (rlRectangle) {0, 0, (float) srcMod.width, (float) srcMod.height};
    ...
    // for every pixel -> [get_src_format/get_dst_format -> blend ->
    // format_to_dst] Some optimization ideas:
    //    [x] Avoid creating source copy if not required (no resize required)
    //    [x] Optimize ImageResize() for pixel format (alternative:
    //    ImageResizeNN()) [x] Optimize ColorAlphaBlend() to avoid processing
    //    (alpha = 0) and (alpha = 1) [x] Optimize ColorAlphaBlend() for faster
    //    operations (maybe avoiding divs?) [x] Consider fast path: no alpha
    //    blending required cases (src has no alpha) [x] Consider fast path:
    //    same src/dst format with no alpha -> direct line copy
    //    [-] GetPixelColor(): Get Vector4 instead of Color, easier for
    //    ColorAlphaBlend() [ ] TODO: Support 16bit and 32bit (float) channels
    //    drawing
    ...
    unsigned char* pSrcBase =
      ...
      ((int) srcRec.y * srcPtr->width + (int) srcRec.x) * bytesPerPixelSrc;
    unsigned char* pDstBase =
      (unsigned char*) dst->data +
      ((int) dstRec.y * dst->width + (int) dstRec.x) * bytesPerPixelDst;

    for (int y = 0; y < (int) srcRec.height; y++) {
      unsigned char* pSrc = pSrcBase;
      unsigned char* pDst = pDstBase;

      // Fast path: Avoid moving pixel by pixel if no blend required and same
      // format
    ...
    if ((dst->mipmaps > 1) && (src.mipmaps > 1)) {
      ...
      mipmapDst.data =
        (char*) mipmapDst.data +
        GetPixelDataSize(mipmapDst.width, mipmapDst.height, mipmapDst.format);
      mipmapDst.width /= 2;
      mipmapDst.height /= 2;
      mipmapDst.mipmaps--;
      ...
      mipmapSrc.data =
        (char*) mipmapSrc.data +
        GetPixelDataSize(mipmapSrc.width, mipmapSrc.height, mipmapSrc.format);
      mipmapSrc.width /= 2;
      mipmapSrc.height /= 2;
      mipmapSrc.mipmaps--;
...
void ImageDrawText(
  Image* dst, char const* text, int posX, int posY, int fontSize, rlColor color
) {
#if SUPPORT_MODULE_RTEXT
  ...
  ImageDrawTextEx(
    dst, GetFontDefault(), text, position, (float) fontSize, 1.0f, color
  ); // WARNING: Module required: rtext
#else
...
void ImageDrawTextEx(
  ...
  rlRectangle srcRec = {
    0.0f, 0.0f, (float) imText.width, (float) imText.height
  };
  rlRectangle dstRec = {
...
Texture2D LoadTexture(char const* fileName) {
  Texture2D texture = {0};
...
Texture2D LoadTextureFromImage(Image image) {
  ...
  if ((image.width != 0) && (image.height != 0)) {
    texture.id = rlLoadTexture(
      image.data, image.width, image.height, image.format, image.mipmaps
    );
  } else
...
TextureCubemap LoadTextureCubemap(Image image, int layout) {
  ...
  if (layout ==
      CUBEMAP_LAYOUT_AUTO_DETECT) // Try to automatically guess layout type
  {
    ...
    if (layout == CUBEMAP_LAYOUT_LINE_HORIZONTAL)
      cubemap.width = image.width / 6;
    if (layout == CUBEMAP_LAYOUT_CROSS_THREE_BY_FOUR)
      cubemap.width = image.width / 3;
    if (layout == CUBEMAP_LAYOUT_CROSS_FOUR_BY_THREE)
      cubemap.width = image.width / 4;
  }
  ...
  if (layout != CUBEMAP_LAYOUT_AUTO_DETECT) {
    ...
    for (int i = 0; i < 6; i++)
      faceRecs[i] = (rlRectangle) {0, 0, (float) size, (float) size};

    if (layout == CUBEMAP_LAYOUT_LINE_VERTICAL) {
      faces =
        ImageCopy(image); // Image data already follows expected convention
    }
    ...
    // NOTE: Cubemap data is expected to be provided as 6 images in a single
    // data array, one after the other (that's a vertical image), following
    // convention: +X, -X, +Y, -Y, +Z, -Z
    cubemap.id =
      rlLoadTextureCubemap(faces.data, size, faces.format, faces.mipmaps);
...
// NOTE: Render texture is loaded by default with RGBA color attachment and
// depth RenderBuffer
...
bool IsTextureValid(Texture2D texture) {
  ...
  if ((texture.id > 0) &&     // Validate OpenGL id (texture uplaoded to GPU)
      ...
      (texture.mipmaps > 0))
    result =
      true; // Validate texture mipmaps (at least 1 for basic mipmap level)
...
void UnloadTexture(Texture2D texture) {
  if (texture.id > 0) {
    ...
    TRACELOG(
      LOG_INFO,
      "TEXTURE: [ID %i] Unloaded texture data from VRAM (GPU)",
      texture.id
    );
  }
}
...
bool IsRenderTextureValid(RenderTexture2D target) {
  ...
  if ((target.id > 0) && // Validate OpenGL id (loaded on GPU)
      IsTextureValid(
        target.depth
      ) && // Validate FBO depth texture/renderbuffer attachment
...
void UpdateTexture(Texture2D texture, void const* pixels) {
  rlUpdateTexture(
    texture.id, 0, 0, texture.width, texture.height, texture.format, pixels
  );
}
...
void UpdateTextureRec(Texture2D texture, rlRectangle rec, void const* pixels) {
  rlUpdateTexture(
    texture.id,
    (int) rec.x,
    (int) rec.y,
...
void SetTextureFilter(Texture2D texture, int filter) {
  ...
  case TEXTURE_FILTER_POINT: {
    if (texture.mipmaps > 1) {
      // RL_TEXTURE_FILTER_MIP_NEAREST - tex filter: POINT, mipmaps filter:
      // POINT (sharp switching between mipmaps)
      rlTextureParameters(
        texture.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_MIP_NEAREST
      );

      // RL_TEXTURE_FILTER_NEAREST - tex filter: POINT (no filter), no mipmaps
      rlTextureParameters(
        texture.id, RL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_NEAREST
      );
    } else {
      // RL_TEXTURE_FILTER_NEAREST - tex filter: POINT (no filter), no mipmaps
      rlTextureParameters(
        texture.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_NEAREST
      );
      rlTextureParameters(
        texture.id, RL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_NEAREST
      );
  ...
  case TEXTURE_FILTER_BILINEAR: {
    if (texture.mipmaps > 1) {
      // RL_TEXTURE_FILTER_LINEAR_MIP_NEAREST - tex filter: BILINEAR, mipmaps
      // filter: POINT (sharp switching between mipmaps) Alternative:
      // RL_TEXTURE_FILTER_NEAREST_MIP_LINEAR - tex filter: POINT, mipmaps
      // filter: BILINEAR (smooth transition between mipmaps)
      rlTextureParameters(
        texture.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_LINEAR_MIP_NEAREST
      );

      // RL_TEXTURE_FILTER_LINEAR - tex filter: BILINEAR, no mipmaps
      rlTextureParameters(
        texture.id, RL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_LINEAR
      );
    } else {
      // RL_TEXTURE_FILTER_LINEAR - tex filter: BILINEAR, no mipmaps
      rlTextureParameters(
        texture.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_LINEAR
      );
      rlTextureParameters(
        texture.id, RL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_LINEAR
      );
  ...
  case TEXTURE_FILTER_TRILINEAR: {
    if (texture.mipmaps > 1) {
      // RL_TEXTURE_FILTER_MIP_LINEAR - tex filter: BILINEAR, mipmaps filter:
      // BILINEAR (smooth transition between mipmaps)
      rlTextureParameters(
        texture.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_MIP_LINEAR
      );
      ...
      TRACELOG(
        LOG_WARNING,
        "TEXTURE: [ID %i] No mipmaps available for TRILINEAR texture filtering",
        texture.id
      );
...
void SetTextureWrap(Texture2D texture, int wrap) {
  ...
  case TEXTURE_WRAP_REPEAT: {
    // NOTE: It only works if NPOT textures are supported, i.e. OpenGL ES 2.0
  ...
  case TEXTURE_WRAP_MIRROR_REPEAT: {
    rlTextureParameters(
      texture.id, RL_TEXTURE_WRAP_S, RL_TEXTURE_WRAP_MIRROR_REPEAT
    );
    rlTextureParameters(
      texture.id, RL_TEXTURE_WRAP_T, RL_TEXTURE_WRAP_MIRROR_REPEAT
    );
  } break;
  case TEXTURE_WRAP_MIRROR_CLAMP: {
    rlTextureParameters(
      texture.id, RL_TEXTURE_WRAP_S, RL_TEXTURE_WRAP_MIRROR_CLAMP
    );
    rlTextureParameters(
      texture.id, RL_TEXTURE_WRAP_T, RL_TEXTURE_WRAP_MIRROR_CLAMP
    );
  } break;
  default:
    break;
...
void DrawTexture(Texture2D texture, int posX, int posY, rlColor tint) {
  DrawTextureEx(
    texture, (Vector2) {(float) posX, (float) posY}, 0.0f, 1.0f, tint
  );
}
...
void DrawTextureEx(
  Texture2D texture, Vector2 position, float rotation, float scale, rlColor tint
) {
  rlRectangle source = {
...
void DrawTextureRec(
  Texture2D texture, rlRectangle source, Vector2 position, rlColor tint
) {
  rlRectangle dest = {
    position.x, position.y, fabsf(source.width), fabsf(source.height)
  };
  Vector2 origin = {0.0f, 0.0f};
...
void DrawTexturePro(
  ...
  if (texture.id > 0) {
    ...
    if (rotation == 0.0f) {
      ...
      bottomRight.x =
        x + (dx + dest.width) * cosRotation - (dy + dest.height) * sinRotation;
      bottomRight.y =
        y + (dx + dest.width) * sinRotation + (dy + dest.height) * cosRotation;
    }
    ...
    vector pointing towards viewer

            // Bottom-left corner for texture and quad
            if (flipX) rlTexCoord2f((source.x + source.width)/width,
    source.y/height); else rlTexCoord2f(source.x/width, source.y/height);
            rlVertex2f(0.0f, 0.0f);

            // Bottom-right corner for texture and quad
            if (flipX) rlTexCoord2f((source.x + source.width)/width, (source.y +
    source.height)/height); else rlTexCoord2f(source.x/width, (source.y +
    source.height)/height); rlVertex2f(0.0f, dest.height);

            // Top-right corner for texture and quad
            if (flipX) rlTexCoord2f(source.x/width, (source.y +
    source.height)/height); else rlTexCoord2f((source.x + source.width)/width,
    (source.y + source.height)/height); rlVertex2f(dest.width, dest.height);
...
// Draws a texture (or part of it) that stretches or shrinks nicely using
// n-patch info
void DrawTextureNPatch(
  ...
  if (texture.id > 0) {
    ...
    // Adjust the lateral (left and right) border widths in case patchWidth <
    // texture.width
    if (patchWidth <= (leftBorder + rightBorder) &&
        nPatchInfo.layout != NPATCH_THREE_PATCH_VERTICAL) {
      drawCenter = false;
      leftBorder = (leftBorder / (leftBorder + rightBorder)) * patchWidth;
      rightBorder = patchWidth - leftBorder;
    }

    // Adjust the lateral (top and bottom) border heights in case patchHeight <
    // texture.height
    if (patchHeight <= (topBorder + bottomBorder) &&
        nPatchInfo.layout != NPATCH_THREE_PATCH_HORIZONTAL) {
      drawMiddle = false;
    ...
    coordC.x =
      (nPatchInfo.source.x + nPatchInfo.source.width - rightBorder) / width;
    coordC.y =
      (nPatchInfo.source.y + nPatchInfo.source.height - bottomBorder) / height;
...
int ColorToInt(rlColor color) {
  ...
  result =
    (int) (((unsigned int) color.r << 24) | ((unsigned int) color.g << 16) |
...
// Color->HSV->Color conversion will not yield exactly the same color due to
// rounding errors Hue is provided in degrees: [0..360] Saturation/Value are
// provided normalized: [0.0f..1.0f]
...
rlColor ColorAlphaBlend(rlColor dst, rlColor src, rlColor tint) {
  ...
  src.r =
    (unsigned char) (((unsigned int) src.r * ((unsigned int) tint.r + 1)) >> 8);
  src.g =
    (unsigned char) (((unsigned int) src.g * ((unsigned int) tint.g + 1)) >> 8);
  src.b =
    (unsigned char) (((unsigned int) src.b * ((unsigned int) tint.b + 1)) >> 8);
  src.a =
    (unsigned char) (((unsigned int) src.a * ((unsigned int) tint.a + 1)) >> 8);
...
#define COLORALPHABLEND_INTEGERS
  ...
  if (src.a == 0)
    ...
    unsigned int alpha =
      (unsigned int) src.a + 1; // Shifting by 8 (dividing by 256), so need to
                                // take that excess into account
    out.a = (unsigned char) (((unsigned int) alpha * 256 +
                              (unsigned int) dst.a * (256 - alpha)) >>
                             8);

    if (out.a > 0) {
      out.r = (unsigned char) ((((unsigned int) src.r * alpha * 256 +
                                 (unsigned int) dst.r * (unsigned int) dst.a *
                                   (256 - alpha)) /
                                out.a) >>
                               8);
      out.g = (unsigned char) ((((unsigned int) src.g * alpha * 256 +
                                 (unsigned int) dst.g * (unsigned int) dst.a *
                                   (256 - alpha)) /
                                out.a) >>
                               8);
      out.b = (unsigned char) ((((unsigned int) src.b * alpha * 256 +
                                 (unsigned int) dst.b * (unsigned int) dst.a *
                                   (256 - alpha)) /
                                out.a) >>
                               8);
    }
  ...
  if (src.a == 0)
    ...
    out = (Color) {(unsigned char) (fout.x * 255.0f),
...
rlColor GetPixelColor(void* srcPtr, int format) {
  ...
  switch (format) {
  case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
    color = (rlColor) {((unsigned char*) srcPtr)[0],
  ...
  case PIXELFORMAT_UNCOMPRESSED_R5G6B5: {
    color.r =
      (unsigned char) ((((unsigned short*) srcPtr)[0] >> 11) * 255 / 31);
    color.g = (unsigned char) (((((unsigned short*) srcPtr)[0] >> 5) &
                                0b0000000000111111) *
                               255 / 63);
    color.b =
      (unsigned char) ((((unsigned short*) srcPtr)[0] & 0b0000000000011111) *
                       255 / 31);
    color.a = 255;
  ...
  case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1: {
    color.r =
      (unsigned char) ((((unsigned short*) srcPtr)[0] >> 11) * 255 / 31);
    color.g = (unsigned char) (((((unsigned short*) srcPtr)[0] >> 6) &
                                0b0000000000011111) *
                               255 / 31);
    color.b =
      (unsigned char) ((((unsigned short*) srcPtr)[0] & 0b0000000000011111) *
  ...
  case PIXELFORMAT_UNCOMPRESSED_R4G4B4A4: {
    color.r =
      (unsigned char) ((((unsigned short*) srcPtr)[0] >> 12) * 255 / 15);
    color.g = (unsigned char) (((((unsigned short*) srcPtr)[0] >> 8) &
                                0b0000000000001111) *
                               255 / 15);
    color.b = (unsigned char) (((((unsigned short*) srcPtr)[0] >> 4) &
                                0b0000000000001111) *
                               255 / 15);
    color.a =
      (unsigned char) ((((unsigned short*) srcPtr)[0] & 0b0000000000001111) *
                       255 / 15);
  ...
  case PIXELFORMAT_UNCOMPRESSED_R16: {
    // NOTE: Pixel normalized float value is converted to [0..255]
    color.r =
      (unsigned char) (HalfToFloat(((unsigned short*) srcPtr)[0]) * 255.0f);
    color.g =
      (unsigned char) (HalfToFloat(((unsigned short*) srcPtr)[0]) * 255.0f);
    color.b =
      (unsigned char) (HalfToFloat(((unsigned short*) srcPtr)[0]) * 255.0f);
  ...
  case PIXELFORMAT_UNCOMPRESSED_R16G16B16: {
    ...
    color.r =
      (unsigned char) (HalfToFloat(((unsigned short*) srcPtr)[0]) * 255.0f);
    color.g =
      (unsigned char) (HalfToFloat(((unsigned short*) srcPtr)[1]) * 255.0f);
    color.b =
      (unsigned char) (HalfToFloat(((unsigned short*) srcPtr)[2]) * 255.0f);
    color.a = 255;
  ...
  case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: {
    ...
    color.b =
      (unsigned char) (HalfToFloat(((unsigned short*) srcPtr)[2]) * 255.0f);
    color.a =
      (unsigned char) (HalfToFloat(((unsigned short*) srcPtr)[3]) * 255.0f);
...
int GetPixelDataSize(int width, int height, int format) {
  ...
  if ((width < 4) && (height < 4)) {
    if ((format >= PIXELFORMAT_COMPRESSED_DXT1_RGB) &&
      ...
      dataSize = 8;
    else if ((format >= PIXELFORMAT_COMPRESSED_DXT3_RGBA) &&
...
static float HalfToFloat(unsigned short x) {
  ...
  unsigned int const e = (x & 0x7c00) >> 10; // Exponent
  unsigned int const m = (x & 0x03ff) << 13; // Mantissa
  uni.fm = (float) m;
...
static unsigned short FloatToHalf(float x) {
  ...
  unsigned int const e = (b & 0x7f800000) >> 23; // Exponent
...
static Vector4* LoadImageDataNormalized(Image image) {
  Vector4* pixels =
    (Vector4*) RL_MALLOC(image.width * image.height * sizeof(Vector4));
