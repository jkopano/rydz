...
// Define error rate to calculate how many segments are needed to draw a smooth
...
static rlRectangle texShapesRec = {
  0.0f, 0.0f, 1.0f, 1.0f
...
rlRectangle GetShapesTextureRectangle(void) { return texShapesRec; }
...
void DrawLine(
  int startPosX, int startPosY, int endPosX, int endPosY, rlColor color
) {
  rlBegin(RL_LINES);
  rlColor4ub(color.r, color.g, color.b, color.a);
...
void DrawLineStrip(Vector2 const* points, int pointCount, rlColor color) {
  if (pointCount < 2)
...
void DrawLineBezier(
  Vector2 startPos, Vector2 endPos, float thick, rlColor color
) {
...
void DrawLineDashed(
  ...
  while (distanceTraveled < lineLength) {
    ...
    Vector2 dashEndPos = {
      startPos.x + dashEndDist * dirX, startPos.y + dashEndDist * dirY
    };
...
void DrawCircleSector(
  ...
  for (int i = 0; i < segments / 2; i++) {
    ...
    rlVertex2f(
      center.x + cosf(DEG2RAD * (angle + stepLength * 2.0f)) * radius,
...
void DrawCircleGradient(
  int centerX, int centerY, float radius, rlColor inner, rlColor outer
) {
  rlBegin(RL_TRIANGLES);
  for (int i = 0; i < 360; i += 10) {
    ...
    rlVertex2f(
      (float) centerX + cosf(DEG2RAD * (i + 10)) * radius,
      (float) centerY + sinf(DEG2RAD * (i + 10)) * radius
    );
    rlColor4ub(outer.r, outer.g, outer.b, outer.a);
    rlVertex2f(
      (float) centerX + cosf(DEG2RAD * i) * radius,
...
void DrawCircleLinesV(Vector2 center, float radius, rlColor color) {
  ...
  for (int i = 0; i < 360; i += 10) {
    rlVertex2f(
      center.x + cosf(DEG2RAD * i) * radius,
      center.y + sinf(DEG2RAD * i) * radius
    );
    rlVertex2f(
      center.x + cosf(DEG2RAD * (i + 10)) * radius,
...
void DrawEllipse(
  ...
  DrawEllipseV(
    (Vector2) {(float) centerX, (float) centerY}, radiusH, radiusV, color
  );
}

// Draw ellipse (Vector version)
void DrawEllipseV(Vector2 center, float radiusH, float radiusV, rlColor color) {
  rlBegin(RL_TRIANGLES);
  for (int i = 0; i < 360; i += 10) {
    rlColor4ub(color.r, color.g, color.b, color.a);
    rlVertex2f(center.x, center.y);
    rlVertex2f(
      center.x + cosf(DEG2RAD * (i + 10)) * radiusH,
      center.y + sinf(DEG2RAD * (i + 10)) * radiusV
    );
    rlVertex2f(
      center.x + cosf(DEG2RAD * i) * radiusH,
...
void DrawEllipseLinesV(
  Vector2 center, float radiusH, float radiusV, rlColor color
) {
  rlBegin(RL_LINES);
  for (int i = 0; i < 360; i += 10) {
    rlColor4ub(color.r, color.g, color.b, color.a);
...
void DrawRing(
  ...
  if (innerRadius <= 0.0f) {
    DrawCircleSector(
      center, outerRadius, startAngle, endAngle, segments, color
    );
    return;
  }
  ...
  for (int i = 0; i < segments; i++) {
    ...
    rlVertex2f(
      center.x + cosf(DEG2RAD * angle) * outerRadius,
      center.y + sinf(DEG2RAD * angle) * outerRadius
    );

    rlTexCoord2f(shapeRect.x / texShapes.width, shapeRect.y / texShapes.height);
    rlVertex2f(
      center.x + cosf(DEG2RAD * angle) * innerRadius,
...
// Draw a color-filled rectangle
void DrawRectangleRec(rlRectangle rec, rlColor color) {
  DrawRectanglePro(rec, (Vector2) {0.0f, 0.0f}, 0.0f, color);
}

// Draw a color-filled rectangle with pro parameters
void DrawRectanglePro(
  rlRectangle rec, Vector2 origin, float rotation, rlColor color
) {
  Vector2 topLeft = {0};
  Vector2 topRight = {0};
...
void DrawRectangleGradientV(
  int posX, int posY, int width, int height, rlColor top, rlColor bottom
) {
...
void DrawRectangleGradientH(
  int posX, int posY, int width, int height, rlColor left, rlColor right
) {
  DrawRectangleGradientEx(
    (rlRectangle) {(float) posX, (float) posY, (float) width, (float) height},
    left,
...
void DrawRectangleLines(
  int posX, int posY, int width, int height, rlColor color
) {
  ...
  rlVertex2f((float) posX + (float) width - xOffset, (float) posY + yOffset);
  rlVertex2f(
    (float) posX + (float) width - xOffset,
  ...
  matrix... #if SUPPORT_QUADS_DRAW_MODE DrawRectangle(posX, posY, width, 1,
  color); DrawRectangle(posX + width - 1, posY + 1, 1, height - 2, color);
...
void DrawRectangleLinesEx(rlRectangle rec, float lineThick, rlColor color) {
  ...
  rlRectangle bottom = {
    rec.x, rec.y - lineThick + rec.height, rec.width, lineThick
  };
  rlRectangle left = {
    rec.x, rec.y + lineThick, lineThick, rec.height - lineThick * 2.0f
  };
  rlRectangle right = {
    rec.x - lineThick + rec.width,
    rec.y + lineThick,
    lineThick,
...
void DrawRectangleRounded(
  rlRectangle rec, float roundness, int segments, rlColor color
) {
  ...
  */
       ...
       ++k) // Hope the compiler is smart enough to unroll this loop
  {
    ...
    for (int i = 0; i < segments / 2; i++) {
      rlColor4ub(color.r, color.g, color.b, color.a);
      rlTexCoord2f(
        shapeRect.x / texShapes.width, shapeRect.y / texShapes.height
      );
      rlVertex2f(center.x, center.y);
    ...
    // NOTE: In case number of segments is odd, adding one last piece to the
    // cake
    if (segments % 2) {
      rlColor4ub(color.r, color.g, color.b, color.a);
      rlTexCoord2f(
...
void DrawRectangleRoundedLines(
  ...
  // NOTE: For line thicknes <=1.0f using RL_LINES, otherwise using
  // RL_QUADS/RL_TRIANGLES
  DrawRectangleRoundedLinesEx(rec, roundness, segments, 1.0f, color);
}

// Draw rectangle with rounded edges outline
void DrawRectangleRoundedLinesEx(
  rlRectangle rec, float roundness, int segments, float lineThick, rlColor color
) {
  if (lineThick < 0)
    lineThick = 0;
  ...
  float stepLength = 90.0f / (float) segments;
  float const outerRadius = radius + lineThick, innerRadius = radius;
...
void DrawTriangleFan(Vector2 const* points, int pointCount, rlColor color) {
  if (pointCount >= 3) {
    rlSetTexture(GetShapesTexture().id);
    rlRectangle shapeRect = GetShapesTextureRectangle();
...
void DrawTriangleStrip(Vector2 const* points, int pointCount, rlColor color) {
  if (pointCount >= 3) {
    rlBegin(RL_TRIANGLES);
    rlColor4ub(color.r, color.g, color.b, color.a);
...
void DrawPoly(
  Vector2 center, int sides, float radius, float rotation, rlColor color
) {
  if (sides < 3)
    sides = 3;
  ...
  for (int i = 0; i < sides; i++) {
    ...
    rlVertex2f(
      center.x + cosf(centralAngle) * radius,
    ...
    rlVertex2f(
      center.x + cosf(nextAngle) * radius, center.y + sinf(nextAngle) * radius
    );
  ...
  for (int i = 0; i < sides; i++) {
    ...
    rlVertex2f(center.x, center.y);
    rlVertex2f(
      center.x + cosf(centralAngle + angleStep) * radius,
...
void DrawPolyLinesEx(
  ...
  for (int i = 0; i < sides; i++) {
    ...
    rlTexCoord2f(shapeRect.x / texShapes.width, shapeRect.y / texShapes.height);
    rlVertex2f(
      center.x + cosf(centralAngle) * innerRadius,
    ...
    rlVertex2f(
      center.x + cosf(nextAngle) * innerRadius,
...
void DrawSplineLinear(
  Vector2 const* points, int pointCount, float thick, rlColor color
) {
  if (pointCount < 2)
    return;
  ...
  Vector2 prevRadius = {
    0.5f * thick * prevNormal.x, 0.5f * thick * prevNormal.y
  };
...
void DrawSplineBasis(
  ...
  for (int i = 0; i < (pointCount - 3); i++) {
    ...
    if (i == 0)
      DrawCircleV(
        currentPoint, thick / 2.0f, color
      ); // Draw init line circle-cap
  ...
  if (pointCount >= 3) {
    for (int i = 0; i < pointCount - 2; i += 2)
      DrawSplineSegmentBezierQuadratic(
        points[i], points[i + 1], points[i + 2], thick, color
      );
  ...
  if (pointCount >= 4) {
    for (int i = 0; i < pointCount - 3; i += 3)
      DrawSplineSegmentBezierCubic(
        points[i], points[i + 1], points[i + 2], points[i + 3], thick, color
      );
...
void DrawSplineSegmentLinear(
  Vector2 p1, Vector2 p2, float thick, rlColor color
) {
...
void DrawSplineSegmentBasis(
  Vector2 p1, Vector2 p2, Vector2 p3, Vector2 p4, float thick, rlColor color
) {
  float const step = 1.0f / SPLINE_SEGMENT_DIVISIONS;
...
void DrawSplineSegmentBezierQuadratic(
  Vector2 p1, Vector2 c2, Vector2 p3, float thick, rlColor color
) {
  float const step = 1.0f / SPLINE_SEGMENT_DIVISIONS;
...
void DrawSplineSegmentBezierCubic(
  Vector2 p1, Vector2 c2, Vector2 c3, Vector2 p4, float thick, rlColor color
) {
  float const step = 1.0f / SPLINE_SEGMENT_DIVISIONS;
...
Vector2 GetSplinePointBezierQuad(
  Vector2 startPos, Vector2 controlPos, Vector2 endPos, float t
) {
  Vector2 point = {0};
...
bool CheckCollisionPointPoly(
  Vector2 point, Vector2 const* points, int pointCount
) {
  bool collision = false;
...
// collision point by reference REF:
...
bool CheckCollisionCircleLine(
  Vector2 center, float radius, Vector2 p1, Vector2 p2
) {
  float dx = p1.x - p2.x;
  float dy = p1.y - p2.y;
