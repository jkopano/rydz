 ...
 *           Support procedural mesh generation functions, uses external
 * par_shapes.h library NOTE: Some generated meshes DO NOT include generated
 * texture coordinates
 *
 *
 *   LICENSE: zlib/libpng
...
// NOTE: Two additional vertex buffers required to store bone indices and bone
// weights WARNING: Some GPUs could not support more than 8 VBOs
...
static Model LoadOBJ(char const* fileName); // Load OBJ mesh data
#endif
#if SUPPORT_FILEFORMAT_IQM
static Model LoadIQM(const char* fileName); // Load IQM mesh data
static ModelAnimation* LoadModelAnimationsIQM(
  char const* fileName, int* animCount
...
static ModelAnimation* LoadModelAnimationsM3D(
  char const* fileName, int* animCount
); // Load M3D animation data
#endif
#if SUPPORT_FILEFORMAT_OBJ || SUPPORT_FILEFORMAT_MTL
static void ProcessMaterialsOBJ(
  rlMaterial* rayMaterials, tinyobj_material_t* materials, int materialCount
...
void DrawLine3D(Vector3 startPos, Vector3 endPos, rlColor color) {
  rlBegin(RL_LINES);
...
void DrawPoint3D(Vector3 position, rlColor color) {
  rlPushMatrix();
...
void DrawCircle3D(
  ...
  for (int i = 0; i < 360; i += 10) {
    ...
    rlVertex3f(
      sinf(DEG2RAD * (i + 10)) * radius, cosf(DEG2RAD * (i + 10)) * radius, 0.0f
    );
  }
  rlEnd();
  rlPopMatrix();
...
void DrawTriangle3D(Vector3 v1, Vector3 v2, Vector3 v3, rlColor color) {
...
void DrawTriangleStrip3D(Vector3 const* points, int pointCount, rlColor color) {
  if (pointCount < 3)
...
void DrawCube(
  Vector3 position, float width, float height, float length, rlColor color
) {
  float x = 0.0f;
  float y = 0.0f;
  float z = 0.0f;
...
void DrawCubeV(Vector3 position, Vector3 size, rlColor color) {
...
void DrawCubeWiresV(Vector3 position, Vector3 size, rlColor color) {
  DrawCubeWires(position, size.x, size.y, size.z, color);
}

// Draw sphere
void DrawSphere(Vector3 centerPos, float radius, rlColor color) {
  DrawSphereEx(centerPos, radius, 16, 16, color);
}

// Draw sphere with extended parameters
void DrawSphereEx(
  Vector3 centerPos, float radius, int rings, int slices, rlColor color
) {
#if 0
  ...
  float ringangle =
    DEG2RAD * (180.0f / (rings + 1)); // Angle between latitudinal parallels
  float sliceangle =
    DEG2RAD * (360.0f / slices); // Angle between longitudinal meridians
  ...
  for (int i = 0; i < rings + 1; i++) {
    for (int j = 0; j < slices; j++) {
      vertices[0] =
        vertices[2]; // Rotate around y axis to set up vertices for next face
      vertices[1] = vertices[3];
      vertices[2] =
        (Vector3) {cosslice * vertices[2].x - sinslice * vertices[2].z,
                   vertices[2].y,
                   sinslice * vertices[2].x +
                     cosslice * vertices[2].z}; // Rotation matrix around y axis
      vertices[3] =
        (Vector3) {cosslice * vertices[3].x - sinslice * vertices[3].z,
                   vertices[3].y,
    ...
    vertices[2] = vertices[3]; // Rotate around z axis to set up  starting
                               // vertices for next ring
    vertices[3] = (Vector3) {cosring * vertices[3].x + sinring * vertices[3].y,
    ...
    for (int j = 0; j < slices; j++) {
      rlVertex3f(
        cosf(DEG2RAD * (270 + (180.0f / (rings + 1)) * i)) *
          sinf(DEG2RAD * (360.0f * j / slices)),
        sinf(DEG2RAD * (270 + (180.0f / (rings + 1)) * i)),
        cosf(DEG2RAD * (270 + (180.0f / (rings + 1)) * i)) *
          cosf(DEG2RAD * (360.0f * j / slices))
      );
      rlVertex3f(
...
void DrawCylinder(
  ...
  float const angleStep = 360.0f / sides;
  ...
  if (radiusTop > 0) {
    ...
    for (int i = 0; i < sides; i++) {
      rlVertex3f(
        ...
        0,
        cosf(DEG2RAD * i * angleStep) * radiusBottom
      ); // Bottom Left
      rlVertex3f(
      ...
      rlVertex3f(
        ...
        height,
        cosf(DEG2RAD * i * angleStep) * radiusTop
      ); // Top Left
      rlVertex3f(
        sinf(DEG2RAD * i * angleStep) * radiusBottom,
        0,
...
void DrawCapsule(
  ...
  for (int c = 0; c < 2; c++) {
    for (int i = 0; i < rings; i++) {
      for (int j = 0; j < slices; j++) {
        // Building up the rings from capCenter in the direction of the
        // 'direction' vector computed earlier

        // Compute the four vertices
        float ringSin1 =
          sinf(baseSliceAngle * (j + 0)) * cosf(baseRingAngle * (i + 0));
        float ringCos1 =
          cosf(baseSliceAngle * (j + 0)) * cosf(baseRingAngle * (i + 0));
        Vector3 w1 =
          (Vector3) {capCenter.x + (sinf(baseRingAngle * (i + 0)) * b0.x +
                                    ringSin1 * b1.x + ringCos1 * b2.x) *
                                     radius,
                     capCenter.y + (sinf(baseRingAngle * (i + 0)) * b0.y +
                                    ringSin1 * b1.y + ringCos1 * b2.y) *
                                     radius,
                     capCenter.z + (sinf(baseRingAngle * (i + 0)) * b0.z +
                                    ringSin1 * b1.z + ringCos1 * b2.z) *
                                     radius};
        float ringSin2 =
          sinf(baseSliceAngle * (j + 1)) * cosf(baseRingAngle * (i + 0));
        float ringCos2 =
          cosf(baseSliceAngle * (j + 1)) * cosf(baseRingAngle * (i + 0));
        Vector3 w2 =
          (Vector3) {capCenter.x + (sinf(baseRingAngle * (i + 0)) * b0.x +
                                    ringSin2 * b1.x + ringCos2 * b2.x) *
                                     radius,
                     capCenter.y + (sinf(baseRingAngle * (i + 0)) * b0.y +
                                    ringSin2 * b1.y + ringCos2 * b2.y) *
                                     radius,
                     capCenter.z + (sinf(baseRingAngle * (i + 0)) * b0.z +
                                    ringSin2 * b1.z + ringCos2 * b2.z) *
                                     radius};

        float ringSin3 =
          sinf(baseSliceAngle * (j + 0)) * cosf(baseRingAngle * (i + 1));
        float ringCos3 =
          cosf(baseSliceAngle * (j + 0)) * cosf(baseRingAngle * (i + 1));
        Vector3 w3 =
          (Vector3) {capCenter.x + (sinf(baseRingAngle * (i + 1)) * b0.x +
                                    ringSin3 * b1.x + ringCos3 * b2.x) *
                                     radius,
                     capCenter.y + (sinf(baseRingAngle * (i + 1)) * b0.y +
                                    ringSin3 * b1.y + ringCos3 * b2.y) *
                                     radius,
                     capCenter.z + (sinf(baseRingAngle * (i + 1)) * b0.z +
                                    ringSin3 * b1.z + ringCos3 * b2.z) *
                                     radius};
        float ringSin4 =
          sinf(baseSliceAngle * (j + 1)) * cosf(baseRingAngle * (i + 1));
        float ringCos4 =
          cosf(baseSliceAngle * (j + 1)) * cosf(baseRingAngle * (i + 1));
        Vector3 w4 =
          (Vector3) {capCenter.x + (sinf(baseRingAngle * (i + 1)) * b0.x +
                                    ringSin4 * b1.x + ringCos4 * b2.x) *
                                     radius,
                     capCenter.y + (sinf(baseRingAngle * (i + 1)) * b0.y +
                                    ringSin4 * b1.y + ringCos4 * b2.y) *
                                     radius,
                     capCenter.z + (sinf(baseRingAngle * (i + 1)) * b0.z +
                                    ringSin4 * b1.z + ringCos4 * b2.z) *
                                     radius};
...
// Draw a plane
void DrawPlane(Vector3 centerPos, Vector2 size, rlColor color) {
...
// Draw a ray line
void DrawRay(Ray ray, rlColor color) {
  float scale = 10000;
...
Model LoadModel(char const* fileName) {
  ...
  if ((model.meshCount != 0) && (model.meshes != NULL)) {
    ...
    TRACELOG(
      LOG_WARNING, "MESH: [%s] Failed to load model mesh(es) data", fileName
    );

  if (model.materialCount == 0) {
    TRACELOG(
      LOG_WARNING,
      "MATERIAL: [%s] Failed to load model material data, default to white "
      "material",
      fileName
    );
    ...
    model.materials =
      (rlMaterial*) RL_CALLOC(model.materialCount, sizeof(rlMaterial));
...
bool IsModelValid(Model model) {
  ...
  if ((model.meshes != NULL) && // Validate model contains some mesh
      (model.materials !=
       NULL) && // Validate model contains some material (at least default one)
  ...
  // NOTE: Many elements could be validated from a model, including every model
  // mesh VAO/VBOs but some VBOs could not be used, it depends on Mesh vertex
  // data
  for (int i = 0; i < model.meshCount; i++) {
    ...
    if ((model.meshes[i].texcoords2 != NULL) &&
        (model.meshes[i].vboId[5] == 0)) {
      result = false;
      break;
    ...
    if ((model.meshes[i].boneIndices != NULL) &&
        (model.meshes[i].vboId[7] == 0)) {
      result = false;
      break;
    } // Vertex boneIndices buffer not uploaded to GPU
    if ((model.meshes[i].boneWeights != NULL) &&
        (model.meshes[i].vboId[8] == 0)) {
      result = false;
      break;
...
BoundingBox GetModelBoundingBox(Model model) {
  ...
  if (model.meshCount > 0) {
    ...
    for (int i = 1; i < model.meshCount; i++) {
      ...
      temp.x =
        (bounds.min.x < tempBounds.min.x) ? bounds.min.x : tempBounds.min.x;
      temp.y =
        (bounds.min.y < tempBounds.min.y) ? bounds.min.y : tempBounds.min.y;
      temp.z =
        (bounds.min.z < tempBounds.min.z) ? bounds.min.z : tempBounds.min.z;
      bounds.min = temp;

      temp.x =
        (bounds.max.x > tempBounds.max.x) ? bounds.max.x : tempBounds.max.x;
      temp.y =
        (bounds.max.y > tempBounds.max.y) ? bounds.max.y : tempBounds.max.y;
      temp.z =
        (bounds.max.z > tempBounds.max.z) ? bounds.max.z : tempBounds.max.z;
      bounds.max = temp;
    }
  }

  // Apply model.transform to bounding box
  // WARNING: Current BoundingBox structure design does not support rotation
  // transformations, in those cases is up to the user to calculate the proper
  // box bounds (8 vertices transformed)
...
void UploadMesh(rlMesh* mesh, bool dynamic) {
  ...
  void* vertices =
    (mesh->animVertices != NULL) ? mesh->animVertices : mesh->vertices;
  mesh->vboId[RL_DEFAULT_SHADER_ATTRIB_LOCATION_POSITION] = rlLoadVertexBuffer(
    vertices, mesh->vertexCount * 3 * sizeof(float), dynamic
  );
  rlSetVertexAttribute(
    RL_DEFAULT_SHADER_ATTRIB_LOCATION_POSITION, 3, RL_FLOAT, 0, 0, 0
  );
  ...
  if (mesh->texcoords != NULL) {
    ...
    rlSetVertexAttribute(
      RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD, 2, RL_FLOAT, 0, 0, 0
    );
    rlEnableVertexAttribute(RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD);
  } else {
    float value[2] = {0.0f, 0.0f};
    rlSetVertexAttributeDefault(
      RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD, value, SHADER_ATTRIB_VEC2, 2
    );
    rlDisableVertexAttribute(RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD);
  }
  // WARNING: When setting default vertex attribute values, the values for each
  // generic vertex attribute is part of current state, and it is maintained
  ...
  if (mesh->normals != NULL) {
    ...
    mesh->vboId[RL_DEFAULT_SHADER_ATTRIB_LOCATION_NORMAL] = rlLoadVertexBuffer(
      normals, mesh->vertexCount * 3 * sizeof(float), dynamic
    );
    rlSetVertexAttribute(
      RL_DEFAULT_SHADER_ATTRIB_LOCATION_NORMAL, 3, RL_FLOAT, 0, 0, 0
    );
  ...
  if (mesh->colors != NULL) {
    ...
    mesh->vboId[RL_DEFAULT_SHADER_ATTRIB_LOCATION_COLOR] = rlLoadVertexBuffer(
      mesh->colors, mesh->vertexCount * 4 * sizeof(unsigned char), dynamic
    );
    rlSetVertexAttribute(
      RL_DEFAULT_SHADER_ATTRIB_LOCATION_COLOR, 4, RL_UNSIGNED_BYTE, 1, 0, 0
    );
    rlEnableVertexAttribute(RL_DEFAULT_SHADER_ATTRIB_LOCATION_COLOR);
  } else {
    // Default vertex attribute: color
    // WARNING: Default value provided to shader if location available
    float value[4] = {1.0f, 1.0f, 1.0f, 1.0f}; // RLWHITE
    rlSetVertexAttributeDefault(
      RL_DEFAULT_SHADER_ATTRIB_LOCATION_COLOR, value, SHADER_ATTRIB_VEC4, 4
    );
  ...
  if (mesh->tangents != NULL) {
    ...
    mesh->vboId[RL_DEFAULT_SHADER_ATTRIB_LOCATION_TANGENT] = rlLoadVertexBuffer(
      mesh->tangents, mesh->vertexCount * 4 * sizeof(float), dynamic
    );
    rlSetVertexAttribute(
  ...
  if (mesh->texcoords2 != NULL) {
    ...
    mesh->vboId[RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD2] =
      rlLoadVertexBuffer(
        mesh->texcoords2, mesh->vertexCount * 2 * sizeof(float), dynamic
      );
    rlSetVertexAttribute(
      RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD2, 2, RL_FLOAT, 0, 0, 0
    );
  ...
  if (mesh->boneWeights != NULL) {
    ...
    mesh->vboId[RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEWEIGHTS] =
      rlLoadVertexBuffer(
        mesh->boneWeights, mesh->vertexCount * 4 * sizeof(float), dynamic
      );
    rlSetVertexAttribute(
      RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEWEIGHTS, 4, RL_FLOAT, 0, 0, 0
    );
  ...
  if (mesh->vaoId > 0)
    TRACELOG(
      LOG_INFO,
      "VAO: [ID %i] Mesh uploaded successfully to VRAM (GPU)",
      mesh->vaoId
    );
  else
...
void UpdateMeshBuffer(
  rlMesh mesh, int index, void const* data, int dataSize, int offset
) {
...
#define GL_TEXTURE_COORD_ARRAY 0x8078
  ...
  if ((mesh.texcoords != NULL) &&
      (material.maps[MATERIAL_MAP_DIFFUSE].texture.id > 0))
  ...
  if (material.shader.locs[SHADER_LOC_MATRIX_PROJECTION] != -1)
    rlSetUniformMatrix(
      material.shader.locs[SHADER_LOC_MATRIX_PROJECTION], matProjection
    );
  ...
  for (int i = 0; i < MAX_MATERIAL_MAPS; i++) {
    if (material.maps[i].texture.id > 0) {
      ...
      if ((i == MATERIAL_MAP_IRRADIANCE) || (i == MATERIAL_MAP_PREFILTER) ||
      ...
      rlSetUniform(
        material.shader.locs[SHADER_LOC_MAP_DIFFUSE + i],
        &i,
        SHADER_UNIFORM_INT,
        1
      );
  ...
  // approach because different meshes with different shaders can enable/disable
  // some attributes
  if (!rlEnableVertexArray(mesh.vaoId)) {
    ...
    rlSetVertexAttribute(
      material.shader.locs[SHADER_LOC_VERTEX_POSITION], 3, RL_FLOAT, 0, 0, 0
    );
    ...
    rlSetVertexAttribute(
      material.shader.locs[SHADER_LOC_VERTEX_TEXCOORD01], 2, RL_FLOAT, 0, 0, 0
    );
    ...
    if (material.shader.locs[SHADER_LOC_VERTEX_NORMAL] != -1) {
      ...
      rlSetVertexAttribute(
        material.shader.locs[SHADER_LOC_VERTEX_NORMAL], 3, RL_FLOAT, 0, 0, 0
      );
    ...
    if (material.shader.locs[SHADER_LOC_VERTEX_COLOR] != -1) {
      if (mesh.vboId[RL_DEFAULT_SHADER_ATTRIB_LOCATION_COLOR] != 0) {
        ...
        // Set default value for defined vertex attribute in shader but not
        // provided by mesh WARNING: It could result in GPU undefined behaviour
    ...
    if (material.shader.locs[SHADER_LOC_VERTEX_TANGENT] != -1) {
      ...
      rlSetVertexAttribute(
        material.shader.locs[SHADER_LOC_VERTEX_TANGENT], 4, RL_FLOAT, 0, 0, 0
      );
    ...
    if (material.shader.locs[SHADER_LOC_VERTEX_TEXCOORD02] != -1) {
      ...
      rlSetVertexAttribute(
        material.shader.locs[SHADER_LOC_VERTEX_TEXCOORD02], 2, RL_FLOAT, 0, 0, 0
      );
    ...
    if (material.shader.locs[SHADER_LOC_VERTEX_BONEIDS] != -1) {
      ...
      rlSetVertexAttribute(
        material.shader.locs[SHADER_LOC_VERTEX_BONEIDS],
        4,
        RL_UNSIGNED_BYTE,
        0,
        0,
    ...
    // Bind mesh VBO data: vertex bone weights (shader-location = 7, if
    // available)
    if (material.shader.locs[SHADER_LOC_VERTEX_BONEWEIGHTS] != -1) {
      ...
      rlSetVertexAttribute(
        material.shader.locs[SHADER_LOC_VERTEX_BONEWEIGHTS],
        4,
        RL_FLOAT,
        0,
        0,
  ...
  for (int eye = 0; eye < eyeCount; eye++) {
    ...
    if (eyeCount == 1)
      ...
      rlViewport(
        eye * rlGetFramebufferWidth() / 2,
        0,
        rlGetFramebufferWidth() / 2,
...
void DrawMeshInstanced(
  rlMesh mesh, rlMaterial material, Matrix const* transforms, int instances
) {
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
  ...
  for (int i = 0; i < instances; i++)
    instanceTransform[i] = MatrixToFloatV(transforms[i]);
  ...
  // cases and on all platforms, anecdotally glMapBuffer() seems quite slow
  // (syncs) while glBufferSubData() seems no faster, since all the transform
  // matrices are transferred anyway
  instancesVboId =
    rlLoadVertexBuffer(instanceTransform, instances * sizeof(float16), false);

  // Instances transformation matrices are sent to shader attribute location:
  // SHADER_LOC_VERTEX_INSTANCETRANSFORM
  if (material.shader.locs[SHADER_LOC_VERTEX_INSTANCETRANSFORM] != -1) {
    for (unsigned int i = 0; i < 4; i++) {
      rlEnableVertexAttribute(
        material.shader.locs[SHADER_LOC_VERTEX_INSTANCETRANSFORM] + i
      );
      rlSetVertexAttribute(
        material.shader.locs[SHADER_LOC_VERTEX_INSTANCETRANSFORM] + i,
        4,
      ...
      rlSetVertexAttributeDivisor(
        material.shader.locs[SHADER_LOC_VERTEX_INSTANCETRANSFORM] + i, 1
      );
    }
  }
  ...
  for (int eye = 0; eye < eyeCount; eye++) {
    ...
    if (mesh.indices != NULL)
      rlDrawVertexArrayElementsInstanced(
        0, mesh.triangleCount * 3, 0, instances
      );
    else
...
bool ExportMesh(rlMesh mesh, char const* fileName) {
  bool success = false;
  ...
  if (IsFileExtension(fileName, ".obj")) {
    ...
    int dataSize =
      vc * (int) strlen("v -0000.000000f -0000.000000f -0000.000000f\n") +
      vc * (int) strlen("vt -0.000000f -0.000000f\n") +
      vc * (int) strlen("vn -0.0000f -0.0000f -0.0000f\n") +
      mesh.triangleCount * snprintf(
                             NULL,
                             0,
                             "f %i/%i/%i %i/%i/%i %i/%i/%i\n",
                             vc,
    ...
    byteCount += sprintf(
      txtData + byteCount,
      "# // rMeshOBJ exporter v1.0 - Mesh exported as triangle faces and not "
      "optimized   //\n"
    );
    byteCount += sprintf(
      txtData + byteCount,
    ...
    byteCount += sprintf(
      txtData + byteCount, "# Vertex Count:     %i\n", mesh.vertexCount
    );
    byteCount += sprintf(
      txtData + byteCount, "# Triangle Count:   %i\n\n", mesh.triangleCount
    );
...
bool ExportMeshAsCode(rlMesh mesh, char const* fileName) {
  bool success = false;
...
#define TEXT_BYTES_PER_LINE 20
  ...
  strncpy(
    varFileName, GetFileNameWithoutExt(fileName), 256 - 1
  ); // NOTE: Using function provided by [rcore] module
  ...
  byteCount += sprintf(
    txtData + byteCount,
    "#define %s_VERTEX_COUNT    %i\n",
    varFileName,
    mesh.vertexCount
  );
  byteCount += sprintf(
    txtData + byteCount,
    "#define %s_TRIANGLE_COUNT   %i\n\n",
    varFileName,
    mesh.triangleCount
  );
  ...
  //-----------------------------------------------------------------------------------------
  if (mesh.vertices !=
      NULL) // Vertex position (XYZ - 3 components per vertex - float)
  {
    byteCount += sprintf(
      txtData + byteCount,
      "static float %s_VERTEX_DATA[%i] = { ",
      varFileName,
      mesh.vertexCount * 3
    );
  ...
  if (mesh.texcoords !=
      NULL) // Vertex texture coordinates (UV - 2 components per vertex - float)
  {
    byteCount += sprintf(
      txtData + byteCount,
      "static float %s_TEXCOORD_DATA[%i] = { ",
      varFileName,
      mesh.vertexCount * 2
    );
      ...
      NULL) // Vertex texture coordinates (UV - 2 components per vertex - float)
  {
    byteCount += sprintf(
      txtData + byteCount,
      "static float %s_TEXCOORD2_DATA[%i] = { ",
      varFileName,
      mesh.vertexCount * 2
    );
    for (int i = 0; i < mesh.vertexCount * 2 - 1; i++)
  ...
  if (mesh.normals !=
      NULL) // Vertex normals (XYZ - 3 components per vertex - float)
  {
    byteCount += sprintf(
      txtData + byteCount,
      "static float %s_NORMAL_DATA[%i] = { ",
      varFileName,
      mesh.vertexCount * 3
    );
    for (int i = 0; i < mesh.vertexCount * 3 - 1; i++)
  ...
  if (mesh.tangents !=
      NULL) // Vertex tangents (XYZW - 4 components per vertex - float)
  {
    byteCount += sprintf(
      txtData + byteCount,
      "static float %s_TANGENT_DATA[%i] = { ",
      varFileName,
      mesh.vertexCount * 4
    );
  ...
  if (mesh.colors !=
      NULL) // Vertex colors (RGBA - 4 components per vertex - unsigned char)
  {
    byteCount += sprintf(
      txtData + byteCount,
      "static unsigned char %s_COLOR_DATA[%i] = { ",
      varFileName,
      mesh.vertexCount * 4
    );
    for (int i = 0; i < mesh.vertexCount * 4 - 1; i++)
      byteCount += sprintf(
        txtData + byteCount,
        ((i % TEXT_BYTES_PER_LINE == 0) ? "0x%x,\n" : "0x%x, "),
        mesh.colors[i]
      );
    byteCount += sprintf(
      txtData + byteCount, "0x%x };\n\n", mesh.colors[mesh.vertexCount * 4 - 1]
    );
  }

  if (mesh.indices !=
      NULL) // Vertex indices (3 index per triangle - unsigned short)
  {
    byteCount += sprintf(
      txtData + byteCount,
  ...
  // if (success != 0) TRACELOG(LOG_INFO, "FILEIO: [%s] Image as code exported
  // successfully", fileName); else TRACELOG(LOG_WARNING, "FILEIO: [%s] Failed
  // to export image as code", fileName);
...
// Process obj materials
static void ProcessMaterialsOBJ(
  rlMaterial* materials, tinyobj_material_t* mats, int materialCount
) {
  // Init model mats
  for (int m = 0; m < materialCount; m++) {
    ...
    materials[m].maps[MATERIAL_MAP_DIFFUSE].texture = (Texture2D) {
    ...
    if (mats[m].diffuse_texname != NULL)
      ...
      materials[m].maps[MATERIAL_MAP_DIFFUSE].color =
        (rlColor) {(unsigned char) (mats[m].diffuse[0] * 255.0f),
    ...
    materials[m].maps[MATERIAL_MAP_SPECULAR].color =
      (rlColor) {(unsigned char) (mats[m].specular[0] * 255.0f),
    ...
    materials[m].maps[MATERIAL_MAP_EMISSION].color =
      (rlColor) {(unsigned char) (mats[m].emission[0] * 255.0f),
    ...
    if (mats[m].displacement_texname != NULL)
      materials[m].maps[MATERIAL_MAP_HEIGHT].texture = LoadTexture(
...
rlMaterial* LoadMaterials(char const* fileName, int* materialCount) {
  ...
  if (IsFileExtension(fileName, ".mtl")) {
    ...
    if (result != TINYOBJ_SUCCESS)
      TRACELOG(
        LOG_WARNING, "MATERIAL: [%s] Failed to parse materials file", fileName
      );

    materials = (rlMaterial*) RL_CALLOC(count, sizeof(rlMaterial));
...
rlMaterial LoadMaterialDefault(void) {
  rlMaterial material = {0};
  material.maps =
    (rlMaterialMap*) RL_CALLOC(MAX_MATERIAL_MAPS, sizeof(rlMaterialMap));
  ...
  material.maps[MATERIAL_MAP_DIFFUSE].texture = (Texture2D) {
    rlGetTextureIdDefault(), 1, 1, 1, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8
  };
  // material.maps[MATERIAL_MAP_NORMAL].texture;         // NOTE: By default,
  // not set material.maps[MATERIAL_MAP_SPECULAR].texture;       // NOTE: By
  // default, not set
...
bool IsMaterialValid(rlMaterial material) {
  bool result = false;
  ...
  // NOTE: Checking if available maps contain loaded textures does not determine
  // if a material is valid, there can be maps without a texture assigned, only
  // properties
...
void UnloadMaterial(rlMaterial material) {
  // Unload material shader (avoid unloading default shader, managed by raylib)
  if (material.shader.id != rlGetShaderIdDefault())
    UnloadShader(material.shader);

  // Unload loaded texture maps (avoid unloading default texture, managed by
  // raylib)
...
// MATERIAL_MAP_SPECULAR...) NOTE: Previous texture should be manually unloaded
void SetMaterialTexture(rlMaterial* material, int mapType, Texture2D texture) {
...
ModelAnimation* LoadModelAnimations(char const* fileName, int* animCount) {
...
// Update model animation data (vertex buffers / bone matrices) for a specific
// pose NOTE 1: Request frame could be fractional, using a lerp interpolation
// between two frames NOTE 2: Updated vertex animation data is uploaded to GPU
// in case of CPU skinning, for GPU skinning, bone matrices are uploaded to
// shader on DrawModelEx()
void UpdateModelAnimation(Model model, ModelAnimation anim, float frame) {
  ...
  if ((anim.keyframeCount > 0) && (model.skeleton.bones != NULL) &&
    ...
    for (int boneIndex = 0; boneIndex < model.skeleton.boneCount; boneIndex++) {
      ...
      //-----------------------------------------------------------------------------------
      rlTransform* bindPoseTransform = &model.skeleton.bindPose[boneIndex];
      bindPoseMatrix = MatrixMultiply(
        MatrixMultiply(
          MatrixScale(
            bindPoseTransform->scale.x,
            bindPoseTransform->scale.y,
            bindPoseTransform->scale.z
          ),
          QuaternionToMatrix(bindPoseTransform->rotation)
        ),
        MatrixTranslate(
          bindPoseTransform->translation.x,
          bindPoseTransform->translation.y,
          bindPoseTransform->translation.z
        )
      );

      rlTransform* currentPoseTransform = &model.currentPose[boneIndex];
      currentPoseMatrix = MatrixMultiply(
        MatrixMultiply(
          MatrixScale(
            currentPoseTransform->scale.x,
            currentPoseTransform->scale.y,
            currentPoseTransform->scale.z
          ),
          QuaternionToMatrix(currentPoseTransform->rotation)
        ),
        MatrixTranslate(
          currentPoseTransform->translation.x,
          currentPoseTransform->translation.y,
          currentPoseTransform->translation.z
        )
      );
...
// Update model animation data (vertex buffers / bone matrices) for a specific
// pose, defined by two different animations at specific frames blended together
// NOTE 1: Request frames could be fractional, using a lerp interpolation
// between two frames NOTE 2: Updated vertex animation data is uploaded to GPU
// in case of CPU skinning, for GPU skinning, bone matrices are uploaded to
// shader on DrawModelEx()
void UpdateModelAnimationEx(
  ...
  if ((animA.keyframeCount > 0) && (animA.keyframePoses != NULL) &&
    ...
    if (currentFrameB >= animB.keyframeCount)
      currentFrameB = currentFrameB % animB.keyframeCount;
    if (nextFrameB >= animB.keyframeCount)
      nextFrameB = nextFrameB % animB.keyframeCount;
    ...
    for (int boneIndex = 0; boneIndex < model.skeleton.boneCount; boneIndex++) {
      ...
      model.currentPose[boneIndex].translation =
        Vector3Lerp(frameATranslation, frameBTranslation, blend);
      model.currentPose[boneIndex].rotation =
        QuaternionSlerp(frameARotation, frameBRotation, blend);
      model.currentPose[boneIndex].scale =
        Vector3Lerp(frameAScale, frameBScale, blend);
      ...
      animA.keyframePoses[currentFrameA][boneIndex].translation; Quaternion
      outARotation = animA.keyframePoses[currentFrameA][boneIndex].rotation;
      Vector3 outAScale = animA.keyframePoses[currentFrameA][boneIndex].scale;

      Vector3 outBTranslation =
      animB.keyframePoses[currentFrameB][boneIndex].translation; Quaternion
      outBRotation = animB.keyframePoses[currentFrameB][boneIndex].rotation;
      ...
      QuaternionInvert(model.skeleton.bindPose[boneIndex].rotation); Vector3
      invBindScale = Vector3Divide((Vector3){ 1.0f, 1.0f, 1.0f },
      model.skeleton.bindPose[boneIndex].scale);

      Vector3 boneTranslation = Vector3Add(Vector3RotateByQuaternion(
          Vector3Multiply(model.currentPose[boneIndex].scale,
      invBindTranslation), model.currentPose[boneIndex].rotation),
              model.currentPose[boneIndex].translation);
      Quaternion boneRotation =
      QuaternionMultiply(model.currentPose[boneIndex].rotation,
      invBindRotation); Vector3 boneScale =
      Vector3Multiply(model.currentPose[boneIndex].scale, invBindScale);

      model.boneMatrices[boneIndex] = MatrixMultiply(
          MatrixMultiply(QuaternionToMatrix(boneRotation),
              MatrixTranslate(boneTranslation.x, boneTranslation.y,
      boneTranslation.z)), MatrixScale(boneScale.x, boneScale.y, boneScale.z));
      */
    }
...
static void UpdateModelAnimationVertexBuffers(Model model) {
  for (int m = 0; m < model.meshCount; m++) {
    ...
    int const vertexValuesCount = mesh.vertexCount * 3;

    int boneIndex = 0;
    int boneCounter = 0;
    float boneWeight = 0.0f;
    bool bufferUpdateRequired =
      false; // Flag to check when anim vertex information is updated
    ...
    for (int vCounter = 0; vCounter < vertexValuesCount; vCounter += 3) {
      ...
      for (int j = 0; j < 4; j++, boneCounter++) {
        ...
        if (boneWeight == 0.0f)
          continue;
        animVertex = (Vector3) {mesh.vertices[vCounter],
                                mesh.vertices[vCounter + 1],
                                mesh.vertices[vCounter + 2]};
        animVertex =
          Vector3Transform(animVertex, model.boneMatrices[boneIndex]);
        ...
        // Normals processing
        // NOTE: Using meshes.baseNormals (default normal) to calculate
        // meshes.normals (animated normals)
        if ((mesh.normals != NULL) && (mesh.animNormals != NULL)) {
          animNormal = (Vector3) {mesh.normals[vCounter],
                                  mesh.normals[vCounter + 1],
                                  mesh.normals[vCounter + 2]};
          animNormal = Vector3Transform(
            animNormal,
...
rlMesh GenMeshPoly(int sides, float radius) {
  ...
  for (int v = 0; v < vertexCount - 2; v += 3) {
    ...
    vertices[v + 1] =
      (Vector3) {sinf(DEG2RAD * d) * radius, 0.0f, cosf(DEG2RAD * d) * radius};
    vertices[v + 2] = (Vector3) {sinf(DEG2RAD * (d + dStep)) * radius,
                                 0.0f,
                                 cosf(DEG2RAD * (d + dStep)) * radius};
    d += dStep;
  }
...
#define CUSTOM_MESH_GEN_PLANE
  ...
  par_shapes_mesh* plane =
    par_shapes_create_plane(resX, resZ); // No normals/texcoords generated!!!
  ...
  mesh.texcoords =
    (float*) RL_MALLOC(plane->ntriangles * 3 * 2 * sizeof(float));
...
#define CUSTOM_MESH_GEN_CUBE
  ...
  /*
  // Platonic solids:
  par_shapes_mesh *par_shapes_create_tetrahedron();       // 4 sides polyhedron
  (pyramid) par_shapes_mesh *par_shapes_create_cube();              // 6 sides
  polyhedron (cube) par_shapes_mesh *par_shapes_create_octahedron();        // 8
  sides polyhedron (diamond) par_shapes_mesh *par_shapes_create_dodecahedron();
  // 12 sides polyhedron par_shapes_mesh *par_shapes_create_icosahedron(); // 20
  sides polyhedron
  */
...
rlMesh GenMeshSphere(float radius, int rings, int slices) {
  ...
  if ((rings >= 3) && (slices >= 3)) {
    ...
    par_shapes_mesh* sphere =
      par_shapes_create_parametric_sphere(slices, rings);
    par_shapes_scale(sphere, radius, radius, radius);
    // NOTE: Soft normals are computed internally

    mesh.vertices =
      (float*) RL_MALLOC(sphere->ntriangles * 3 * 3 * sizeof(float));
    mesh.texcoords =
      (float*) RL_MALLOC(sphere->ntriangles * 3 * 2 * sizeof(float));
    mesh.normals =
      (float*) RL_MALLOC(sphere->ntriangles * 3 * 3 * sizeof(float));
...
rlMesh GenMeshCylinder(float radius, float height, int slices) {
  ...
  if (slices >= 3) {
    // Instance a cylinder that sits on the Z=0 plane using the given
    // tessellation levels across the UV domain.  Think of "slices" like a
    // number of pizza slices, and "stacks" like a number of stacked rings
    // Height and radius are both 1.0, but they can easily be changed with
    // par_shapes_scale
    ...
    par_shapes_mesh* capTop = par_shapes_create_disk(
      radius, slices, (float[]) {0, 0, 0}, (float[]) {0, 0, 1}
    );
    ...
    par_shapes_mesh* capBottom = par_shapes_create_disk(
      radius, slices, (float[]) {0, 0, 0}, (float[]) {0, 0, -1}
    );
    ...
    mesh.vertices =
      (float*) RL_MALLOC(cylinder->ntriangles * 3 * 3 * sizeof(float));
    mesh.texcoords =
      (float*) RL_MALLOC(cylinder->ntriangles * 3 * 2 * sizeof(float));
    mesh.normals =
      (float*) RL_MALLOC(cylinder->ntriangles * 3 * 3 * sizeof(float));
    ...
    for (int k = 0; k < mesh.vertexCount; k++) {
      ...
      mesh.vertices[k * 3 + 1] =
        cylinder->points[cylinder->triangles[k] * 3 + 1];
      mesh.vertices[k * 3 + 2] =
        cylinder->points[cylinder->triangles[k] * 3 + 2];

      mesh.normals[k * 3] = cylinder->normals[cylinder->triangles[k] * 3];
      mesh.normals[k * 3 + 1] =
        cylinder->normals[cylinder->triangles[k] * 3 + 1];
      mesh.normals[k * 3 + 2] =
        cylinder->normals[cylinder->triangles[k] * 3 + 2];

      mesh.texcoords[k * 2] = cylinder->tcoords[cylinder->triangles[k] * 2];
      mesh.texcoords[k * 2 + 1] =
        cylinder->tcoords[cylinder->triangles[k] * 2 + 1];
    }
...
rlMesh GenMeshCone(float radius, float height, int slices) {
  ...
  if (slices >= 3) {
    ...
    mesh.vertices =
      (float*) RL_MALLOC(cone->ntriangles * 3 * 3 * sizeof(float));
    mesh.texcoords =
      (float*) RL_MALLOC(cone->ntriangles * 3 * 2 * sizeof(float));
...
rlMesh GenMeshTorus(float radius, float size, int radSeg, int sides) {
  ...
  if ((sides >= 3) && (radSeg >= 3)) {
    ...
    mesh.vertices =
      (float*) RL_MALLOC(torus->ntriangles * 3 * 3 * sizeof(float));
    mesh.texcoords =
      (float*) RL_MALLOC(torus->ntriangles * 3 * 2 * sizeof(float));
    mesh.normals =
      (float*) RL_MALLOC(torus->ntriangles * 3 * 3 * sizeof(float));
...
rlMesh GenMeshKnot(float radius, float size, int radSeg, int sides) {
  ...
  if ((sides >= 3) && (radSeg >= 3)) {
    ...
    par_shapes_mesh* knot =
      par_shapes_create_trefoil_knot(radSeg, sides, radius);
    par_shapes_scale(knot, size, size, size);

    mesh.vertices =
      (float*) RL_MALLOC(knot->ntriangles * 3 * 3 * sizeof(float));
    mesh.texcoords =
      (float*) RL_MALLOC(knot->ntriangles * 3 * 2 * sizeof(float));
...
#define GRAY_VALUE(c) ((float) (c.r + c.g + c.b) / 3.0f)
  ...
  rlColor* pixels = LoadImageColors(heightmap);
  ...
  Vector3 scaleFactor = {
    size.x / (mapX - 1), size.y / 255.0f, size.z / (mapZ - 1)
  };
  ...
  for (int z = 0; z < mapZ - 1; z++) {
    for (int x = 0; x < mapX - 1; x++) {
      ...
      mesh.vertices[vCounter + 1] =
        GRAY_VALUE(pixels[x + z * mapX]) * scaleFactor.y;
      mesh.vertices[vCounter + 2] = (float) z * scaleFactor.z;

      mesh.vertices[vCounter + 3] = (float) x * scaleFactor.x;
      mesh.vertices[vCounter + 4] =
        GRAY_VALUE(pixels[x + (z + 1) * mapX]) * scaleFactor.y;
      mesh.vertices[vCounter + 5] = (float) (z + 1) * scaleFactor.z;

      mesh.vertices[vCounter + 6] = (float) (x + 1) * scaleFactor.x;
      mesh.vertices[vCounter + 7] =
        GRAY_VALUE(pixels[(x + 1) + z * mapX]) * scaleFactor.y;
      ...
      mesh.vertices[vCounter + 16] =
        GRAY_VALUE(pixels[(x + 1) + (z + 1) * mapX]) * scaleFactor.y;
      ...
      for (int i = 0; i < 18; i += 9) {
        ...
        vN = Vector3Normalize(
          Vector3CrossProduct(Vector3Subtract(vB, vA), Vector3Subtract(vC, vA))
        );
...
#define COLOR_EQUAL(col1, col2)                                                \
  ...
  Vector3* mapVertices =
    (Vector3*) RL_MALLOC(maxTriangles * 3 * sizeof(Vector3));
  Vector2* mapTexcoords =
    (Vector2*) RL_MALLOC(maxTriangles * 3 * sizeof(Vector2));
  Vector3* mapNormals =
    (Vector3*) RL_MALLOC(maxTriangles * 3 * sizeof(Vector3));
  ...
  for (int z = 0; z < cubicmap.height; ++z) {
    for (int x = 0; x < cubicmap.width; x++) {
      ...
      // Check pixel color to be RLWHITE -> draw full cube
      if (COLOR_EQUAL(pixels[z * cubicmap.width + x], RLWHITE)) {
        // Define triangles and checking collateral cubes
        //------------------------------------------------

        // Define top triangles (2 tris, 6 vertex --> v1-v2-v3, v1-v3-v4)
        // WARNING: Not required for a RLWHITE cubes, created to allow seeing
        ...
        mapTexcoords[tcCounter + 1] =
          (Vector2) {topTexUV.x, topTexUV.y + topTexUV.height};
        mapTexcoords[tcCounter + 2] =
          (Vector2) {topTexUV.x + topTexUV.width, topTexUV.y + topTexUV.height};
        mapTexcoords[tcCounter + 3] = (Vector2) {topTexUV.x, topTexUV.y};
        mapTexcoords[tcCounter + 4] =
          (Vector2) {topTexUV.x + topTexUV.width, topTexUV.y + topTexUV.height};
        mapTexcoords[tcCounter + 5] =
          (Vector2) {topTexUV.x + topTexUV.width, topTexUV.y};
        tcCounter += 6;
        ...
        mapTexcoords[tcCounter] =
          (Vector2) {bottomTexUV.x + bottomTexUV.width, bottomTexUV.y};
        mapTexcoords[tcCounter + 1] =
          (Vector2) {bottomTexUV.x, bottomTexUV.y + bottomTexUV.height};
        mapTexcoords[tcCounter + 2] =
          (Vector2) {bottomTexUV.x + bottomTexUV.width,
                     bottomTexUV.y + bottomTexUV.height};
        mapTexcoords[tcCounter + 3] =
          (Vector2) {bottomTexUV.x + bottomTexUV.width, bottomTexUV.y};
        ...
        if (((z < cubicmap.height - 1) &&
             COLOR_EQUAL(pixels[(z + 1) * cubicmap.width + x], RLBLACK)) ||
            (z == cubicmap.height - 1)) {
          ...
          mapTexcoords[tcCounter + 1] =
            (Vector2) {frontTexUV.x, frontTexUV.y + frontTexUV.height};
          mapTexcoords[tcCounter + 2] =
            (Vector2) {frontTexUV.x + frontTexUV.width, frontTexUV.y};
          mapTexcoords[tcCounter + 3] =
            (Vector2) {frontTexUV.x + frontTexUV.width, frontTexUV.y};
          mapTexcoords[tcCounter + 4] =
            (Vector2) {frontTexUV.x, frontTexUV.y + frontTexUV.height};
          mapTexcoords[tcCounter + 5] =
            (Vector2) {frontTexUV.x + frontTexUV.width,
                       frontTexUV.y + frontTexUV.height};
          tcCounter += 6;
        }
        ...
        if (((z > 0) &&
          ...
          mapTexcoords[tcCounter] =
            (Vector2) {backTexUV.x + backTexUV.width, backTexUV.y};
          mapTexcoords[tcCounter + 1] =
            (Vector2) {backTexUV.x, backTexUV.y + backTexUV.height};
          mapTexcoords[tcCounter + 2] =
            (Vector2) {backTexUV.x + backTexUV.width,
                       backTexUV.y + backTexUV.height};
          mapTexcoords[tcCounter + 3] =
            (Vector2) {backTexUV.x + backTexUV.width, backTexUV.y};
        ...
        if (((x < cubicmap.width - 1) &&
             COLOR_EQUAL(pixels[z * cubicmap.width + (x + 1)], RLBLACK)) ||
            (x == cubicmap.width - 1)) {
          ...
          mapTexcoords[tcCounter + 4] =
            (Vector2) {rightTexUV.x, rightTexUV.y + rightTexUV.height};
          mapTexcoords[tcCounter + 5] =
            (Vector2) {rightTexUV.x + rightTexUV.width,
                       rightTexUV.y + rightTexUV.height};
          tcCounter += 6;
        }
        ...
        if (((x > 0) &&
          ...
          mapTexcoords[tcCounter + 1] =
            (Vector2) {leftTexUV.x + leftTexUV.width,
                       leftTexUV.y + leftTexUV.height};
          mapTexcoords[tcCounter + 2] =
            (Vector2) {leftTexUV.x + leftTexUV.width, leftTexUV.y};
          mapTexcoords[tcCounter + 3] = (Vector2) {leftTexUV.x, leftTexUV.y};
          mapTexcoords[tcCounter + 4] =
            (Vector2) {leftTexUV.x, leftTexUV.y + leftTexUV.height};
          mapTexcoords[tcCounter + 5] =
            (Vector2) {leftTexUV.x + leftTexUV.width,
                       leftTexUV.y + leftTexUV.height};
          tcCounter += 6;
      ...
      // Check pixel color to be RLBLACK, in that case only drawing floor and
      // roof
      else if (COLOR_EQUAL(pixels[z * cubicmap.width + x], RLBLACK)) {
        // Define top triangles (2 tris, 6 vertex --> v1-v2-v3, v1-v3-v4)
        mapVertices[vCounter] = v1;
...
void GenMeshTangents(rlMesh* mesh) {
  ...
  if ((mesh == NULL) || (mesh->vertices == NULL) || (mesh->texcoords == NULL) ||
      (mesh->normals == NULL)) {
    TRACELOG(
      LOG_WARNING,
      "MESH: Tangents generation requires vertices, texcoords and normals "
  ...
  if (tan1 == NULL || tan2 == NULL) {
    TRACELOG(
      LOG_WARNING,
      "MESH: Failed to allocate temporary memory for tangent calculation"
    );
    if (tan1)
      RL_FREE(tan1);
    if (tan2)
  ...
  for (int i = 0; i < mesh->vertexCount; i++) {
    ...
    Vector3 orthogonalized = Vector3Subtract(
      tangent, Vector3Scale(normal, Vector3DotProduct(normal, tangent))
    );
    ...
    if (Vector3Length(orthogonalized) < 0.0001f) {
      ...
      if (fabsf(normal.z) > 0.707f)
        ...
        orthogonalized =
          Vector3Normalize((Vector3) {-normal.y, normal.x, 0.0f});
    } else {
    ...
    mesh->tangents[i * 4 + 3] =
      (Vector3DotProduct(Vector3CrossProduct(normal, orthogonalized), tan2[i]) <
       0.0f)
        ? -1.0f
        : 1.0f;
  }
  ...
  TRACELOG(
    LOG_INFO, "MESH: Tangents data computed and uploaded for provided mesh"
  );
}

// Draw a model (with texture if set)
void DrawModel(Model model, Vector3 position, float scale, rlColor tint) {
...
void DrawModelEx(
  ...
  // Combine model transformation matrix (model.transform) with matrix generated
  // by function parameters (matTransform)
  model.transform = MatrixMultiply(model.transform, matTransform);

  for (int i = 0; i < model.meshCount; i++) {
    rlMaterial mat = model.materials[model.meshMaterial[i]];
    rlColor colDiffuse = mat.maps[MATERIAL_MAP_DIFFUSE].color;
    ...
    // Upload runtime bone transforms matrices, to compute skinning on the
    // shader (GPU-skinning) NOTE: Required location must be found and Mesh
    // bones indices and weights must be also uploaded to shader
    if ((mat.shader.locs != NULL) &&
        (mat.shader.locs[SHADER_LOC_MATRIX_BONETRANSFORMS] != -1) &&
        (model.boneMatrices != NULL)) {
      rlEnableShader(
        mat.shader.id
      ); // Enable shader to set bone transform matrices
      rlSetUniformMatrices(
        mat.shader.locs[SHADER_LOC_MATRIX_BONETRANSFORMS],
        model.boneMatrices,
...
void DrawModelWires(Model model, Vector3 position, float scale, rlColor tint) {
  rlEnableWireMode();
...
// Draw a model points
// WARNING: OpenGL ES 2.0 does not support point mode drawing
void DrawModelPoints(Model model, Vector3 position, float scale, rlColor tint) {
  rlEnablePointMode();
...
// Draw a billboard
void DrawBillboard(
  Camera camera, Texture2D texture, Vector3 position, float scale, rlColor tint
) {
  rlRectangle source = {
...
void DrawBillboardPro(
  ...
  // Flip the content of the billboard while maintaining the counterclockwise
  // edge rendering order
  if (size.x < 0.0f) {
    source.x -= size.x;
    source.width *= -1.0;
  ...
  // Draw the texture region described by source on the following rectangle in
  // 3D space:
  //
  //                size.x          <--.
  //  3 ^---------------------------+ 2 \ rotation
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (rotation != 0.0)
      points[i] =
        Vector3RotateByAxisAngle(points[i], forward, rotation * DEG2RAD);
    points[i] = Vector3Add(points[i], position);
  }

  Vector2 texcoords[4];
  texcoords[0] =
    (Vector2) {(float) source.x / texture.width,
               (float) (source.y + source.height) / texture.height};
  texcoords[1] =
    (Vector2) {(float) (source.x + source.width) / texture.width,
               (float) (source.y + source.height) / texture.height};
...
void DrawBoundingBox(BoundingBox box, rlColor color) {
  Vector3 size = {0};
...
bool CheckCollisionSpheres(
  Vector3 center1, float radius1, Vector3 center2, float radius2
) {
  bool collision = false;

  // Simple way to check for collision, checking distance between two points
  // Unfortunately, sqrtf() is a costly operation, so avoid it with following
  // solution
  /*
  ...
  */
  ...
  if (Vector3DotProduct(
        Vector3Subtract(center2, center1), Vector3Subtract(center2, center1)
...
RayCollision GetRayCollisionSphere(Ray ray, Vector3 center, float radius) {
  ...
  // Check if ray origin is inside the sphere to calculate the correct collision
  // point
  if (distance < radius) {
    ...
    collision.normal =
      Vector3Negate(Vector3Normalize(Vector3Subtract(collision.point, center)));
  } else {
    collision.distance = vector - sqrtf(d);
    ...
    collision.normal =
      Vector3Normalize(Vector3Subtract(collision.point, center));
  }
...
RayCollision GetRayCollisionTriangle(
  Ray ray, Vector3 p1, Vector3 p2, Vector3 p3
) {
...
RayCollision GetRayCollisionQuad(
  Ray ray, Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4
) {
  RayCollision collision = {0};
...
static void BuildPoseFromParentJoints(
  BoneInfo* bones, int boneCount, rlTransform* transforms
) {
  for (int i = 0; i < boneCount; i++) {
    if (bones[i].parent >= 0) {
      if (bones[i].parent > i) {
        TRACELOG(
          LOG_WARNING,
          "Skipping bone not topologically sorted: Bone %d has parent %d",
          i,
          bones[i].parent
        );
        continue;
      }
      transforms[i].rotation = QuaternionMultiply(
        transforms[bones[i].parent].rotation, transforms[i].rotation
      );
      transforms[i].scale =
        Vector3Multiply(transforms[i].scale, transforms[bones[i].parent].scale);
      transforms[i].translation = Vector3Multiply(
        transforms[i].translation, transforms[bones[i].parent].scale
      );
      transforms[i].translation = Vector3RotateByQuaternion(
        transforms[i].translation, transforms[bones[i].parent].rotation
      );
      transforms[i].translation = Vector3Add(
        transforms[i].translation, transforms[bones[i].parent].translation
      );
    }
  }
}
...
static Model LoadOBJ(char const* fileName) {
  ...
  strncpy(
    currentDir, GetWorkingDirectory(), MAX_FILEPATH_LENGTH - 1
  ); // Save current working directory
  char const* workingDir = GetDirectoryPath(
    fileName
  ); // Switch to OBJ directory for material path correctness
  if (CHDIR(workingDir) != 0)
    TRACELOG(
      LOG_WARNING, "MODEL: [%s] Failed to change working directory", workingDir
    );
  ...
  for (unsigned int faceId = 0; faceId < objAttributes.num_faces; faceId++) {
    if (faceId >= nextShapeEnd) {
      ...
      if (nextShape < objShapeCount)
        ...
        nextShapeEnd =
          objAttributes
            .num_face_num_verts; // This is actually the total number of face
                                 // verts in the file, not faces
      meshIndex++;
    } else if ((lastMaterial != -1) &&
               (objAttributes.material_ids[faceId] != lastMaterial)) {
  ...
  if (objMaterialCount > 0) {
    ...
    model.materials =
      (rlMaterial*) MemAlloc(sizeof(rlMaterial) * objMaterialCount);
  } else // Allocate at least one material
  {
    model.materialCount = 1;
    model.materials = (rlMaterial*) MemAlloc(sizeof(rlMaterial) * 1);
  }

  model.meshMaterial = (int*) MemAlloc(sizeof(int) * model.meshCount);

  // See how many verts are in each mesh
  unsigned int* localMeshVertexCounts =
    (unsigned int*) MemAlloc(sizeof(unsigned int) * model.meshCount);
  ...
  for (int i = 0; i < model.meshCount; i++) {
    ...
    model.meshes[i].vertices =
      (float*) MemAlloc(sizeof(float) * vertexCount * 3);
    model.meshes[i].normals =
      (float*) MemAlloc(sizeof(float) * vertexCount * 3);
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    model.meshes[i].texcoords =
      (float*) MemAlloc(sizeof(float) * vertexCount * 2);
    model.meshes[i].colors =
      (unsigned char*) MemAlloc(sizeof(unsigned char) * vertexCount * 4);
#else
  ...
  for (unsigned int faceId = 0; faceId < objAttributes.num_faces; faceId++) {
    ...
    for (int f = 0; f < objAttributes.face_num_verts[faceId]; f++) {
      ...
      if ((objAttributes.texcoords != NULL) &&
          (texcordIndex != TINYOBJ_INVALID_INDEX) && (texcordIndex >= 0) &&
          (model.meshes[meshIndex].texcoords)) {
        for (int i = 0; i < 2; i++)
      ...
      if ((objAttributes.normals != NULL) &&
          (normalIndex != TINYOBJ_INVALID_INDEX) && (normalIndex >= 0)) {
        for (int i = 0; i < 3; i++)
  ...
  if (objMaterialCount > 0)
    ...
    model.materials[0] =
      LoadMaterialDefault(); // Set default material for the mesh
  ...
  if (CHDIR(currentDir) != 0) {
    TRACELOG(
      LOG_WARNING, "MODEL: [%s] Failed to change working directory", currentDir
    );
  }
...
#if SUPPORT_FILEFORMAT_IQM
// Load IQM mesh data
static Model LoadIQM(char const* fileName) {
...
#define MATERIAL_NAME_LENGTH 32 // Material name string length
  ...
  */
  ...
  char const* basePath = GetDirectoryPath(fileName);
  ...
  if (memcmp(iqmHeader->magic, IQM_MAGIC, sizeof(IQM_MAGIC)) != 0) {
    TRACELOG(
      LOG_WARNING, "MODEL: [%s] IQM file is not a valid model", fileName
    );
  ...
  if (iqmHeader->version != IQM_VERSION) {
    TRACELOG(
      LOG_WARNING,
      "MODEL: [%s] IQM file version not supported (%i)",
      fileName,
      iqmHeader->version
    );
    UnloadFileData(fileData);
  ...
  memcpy(
    imesh,
    fileDataPtr + iqmHeader->ofs_meshes,
    iqmHeader->num_meshes * sizeof(IQMMesh)
  );
  ...
  for (int i = 0; i < model.meshCount; i++) {
    ...
    memcpy(
      name,
      fileDataPtr + iqmHeader->ofs_text + imesh[i].name,
      MESH_NAME_LENGTH * sizeof(char)
    );
    ...
    memcpy(
      material,
      fileDataPtr + iqmHeader->ofs_text + imesh[i].material,
      MATERIAL_NAME_LENGTH * sizeof(char)
    );
    ...
    TRACELOG(
      LOG_DEBUG,
      "MODEL: [%s] mesh name (%s), material (%s)",
      fileName,
      name,
      material
    );
    ...
    model.meshes[i].vertices = (float*) RL_CALLOC(
      model.meshes[i].vertexCount * 3, sizeof(float)
    ); // Default vertex positions
    model.meshes[i].normals = (float*) RL_CALLOC(
      model.meshes[i].vertexCount * 3, sizeof(float)
    ); // Default vertex normals
    model.meshes[i].texcoords = (float*) RL_CALLOC(
      model.meshes[i].vertexCount * 2, sizeof(float)
    ); // Default vertex texcoords

    model.meshes[i].boneIndices = (unsigned char*) RL_CALLOC(
      model.meshes[i].vertexCount * 4, sizeof(unsigned char)
    ); // Up-to 4 bones supported!
    model.meshes[i].boneWeights = (float*) RL_CALLOC(
      model.meshes[i].vertexCount * 4, sizeof(float)
    ); // Up-to 4 bones supported!

    model.meshes[i].triangleCount = imesh[i].num_triangles;
    model.meshes[i].indices = (unsigned short*) RL_CALLOC(
      model.meshes[i].triangleCount * 3, sizeof(unsigned short)
    );

#if !SUPPORT_GPU_SKINNING
    // Animated vertex data, processed for rendering
    // NOTE: Animated vertex should be re-uploaded to GPU (if not using GPU
    // skinning)
    model.meshes[i].animVertices =
      (float*) RL_CALLOC(model.meshes[i].vertexCount * 3, sizeof(float));
  ...
  tri =
    (IQMTriangle*) RL_MALLOC(iqmHeader->num_triangles * sizeof(IQMTriangle));
  // fseek(iqmFile, iqmHeader->ofs_triangles, SEEK_SET);
  // fread(tri, sizeof(IQMTriangle), iqmHeader->num_triangles, iqmFile);
  memcpy(
    tri,
    fileDataPtr + iqmHeader->ofs_triangles,
    iqmHeader->num_triangles * sizeof(IQMTriangle)
  );
  ...
  for (int m = 0; m < model.meshCount; m++) {
    ...
    for (unsigned int i = imesh[m].first_triangle;
         i < (imesh[m].first_triangle + imesh[m].num_triangles);
         i++) {
      // IQM triangles indexes are stored in counter-clockwise, but raylib
      // processes the index in linear order, expecting they point to the
      // counter-clockwise vertex triangle, so triangle indexes need to be
      // reversed NOTE: raylib renders vertex data in counter-clockwise order
      // (standard convention) by default
      model.meshes[m].indices[tcounter + 2] =
        tri[i].vertex[0] - imesh[m].first_vertex;
      model.meshes[m].indices[tcounter + 1] =
        tri[i].vertex[1] - imesh[m].first_vertex;
      model.meshes[m].indices[tcounter] =
        tri[i].vertex[2] - imesh[m].first_vertex;
      tcounter += 3;
    }
  }

  // Vertex arrays data processing
  va = (IQMVertexArray*) RL_MALLOC(
  ...
  memcpy(
    va,
    fileDataPtr + iqmHeader->ofs_vertexarrays,
    iqmHeader->num_vertexarrays * sizeof(IQMVertexArray)
  );
  ...
  for (unsigned int i = 0; i < iqmHeader->num_vertexarrays; i++) {
    ...
    case IQM_POSITION: {
      ...
      memcpy(
        vertex,
        fileDataPtr + va[i].offset,
        iqmHeader->num_vertexes * 3 * sizeof(float)
      );
    ...
    case IQM_TEXCOORD: {
      ...
      memcpy(
        text,
        fileDataPtr + va[i].offset,
        iqmHeader->num_vertexes * 2 * sizeof(float)
      );
    ...
    case IQM_BLENDINDEXES: {
      ...
      memcpy(
        blendi,
        fileDataPtr + va[i].offset,
        iqmHeader->num_vertexes * 4 * sizeof(char)
      );
    ...
    case IQM_BLENDWEIGHTS: {
      blendw = (unsigned char*) RL_MALLOC(
        iqmHeader->num_vertexes * 4 * sizeof(unsigned char)
      );
      // fseek(iqmFile, va[a].offset, SEEK_SET);
      // fread(blendw, iqmHeader->num_vertexes*4*sizeof(unsigned char), 1,
      // iqmFile);
      memcpy(
        blendw,
        fileDataPtr + va[i].offset,
    ...
    case IQM_COLOR: {
      ...
      for (unsigned int m = 0; m < iqmHeader->num_meshes; m++) {
        model.meshes[m].colors = (unsigned char*) RL_CALLOC(
          model.meshes[m].vertexCount * 4, sizeof(unsigned char)
        );
  ...
  memcpy(
    ijoint,
    fileDataPtr + iqmHeader->ofs_joints,
    iqmHeader->num_joints * sizeof(IQMJoint)
  );

  model.skeleton.boneCount = iqmHeader->num_joints;
  model.skeleton.bones =
    (BoneInfo*) RL_CALLOC(iqmHeader->num_joints, sizeof(BoneInfo));
  model.skeleton.bindPose =
    (rlTransform*) RL_CALLOC(iqmHeader->num_joints, sizeof(rlTransform));
  ...
  BuildPoseFromParentJoints(
    model.skeleton.bones, model.skeleton.boneCount, model.skeleton.bindPose
  );

  // Initialize runtime animation data: current pose and bone matrices
  model.currentPose =
    (rlTransform*) RL_CALLOC(model.skeleton.boneCount, sizeof(rlTransform));
  model.boneMatrices =
    (Matrix*) RL_CALLOC(model.skeleton.boneCount, sizeof(Matrix));
  for (int j = 0; j < model.skeleton.boneCount; j++)
...
#define IQM_VERSION 2               // only IQM version 2 supported
  ...
  memcpy(
    ...
    iqmHeader->num_poses * sizeof(IQMPose)
  );
  ...
  *animCount = iqmHeader->num_anims;
  ...
  memcpy(
    ...
    iqmHeader->num_anims * sizeof(IQMAnim)
  );

  ModelAnimation* animations =
    (ModelAnimation*) RL_CALLOC(iqmHeader->num_anims, sizeof(ModelAnimation));

  // frameposes
  unsigned short* framedata = (unsigned short*) RL_CALLOC(
    iqmHeader->num_frames * iqmHeader->num_framechannels, sizeof(unsigned short)
  );
  // fseek(iqmFile, iqmHeader->ofs_frames, SEEK_SET);
  // fread(framedata, sizeof(unsigned short),
  // iqmHeader->num_frames*iqmHeader->num_framechannels, iqmFile);
  memcpy(
    framedata,
    fileDataPtr + iqmHeader->ofs_frames,
    iqmHeader->num_frames * iqmHeader->num_framechannels *
      sizeof(unsigned short)
  );

  // joints
  IQMJoint* joints =
    (IQMJoint*) RL_CALLOC(iqmHeader->num_joints, sizeof(IQMJoint));
  memcpy(
    joints,
    fileDataPtr + iqmHeader->ofs_joints,
    iqmHeader->num_joints * sizeof(IQMJoint)
  ...
  for (unsigned int a = 0; a < iqmHeader->num_anims; a++) {
    ...
    BoneInfo* bones =
      (BoneInfo*) RL_CALLOC(iqmHeader->num_poses, sizeof(BoneInfo));

    animations[a].keyframeCount = anim[a].num_frames;
    animations[a].keyframePoses =
      (rlTransform**) RL_CALLOC(anim[a].num_frames, sizeof(rlTransform*));
    memcpy(
      animations[a].name, fileDataPtr + iqmHeader->ofs_text + anim[a].name, 32
    );
    ...
    TRACELOG(
      LOG_INFO,
      "MODEL: [%s] Loaded animation: %s | Frames: %d | Framerate: %f",
      fileName,
      animations[a].name,
      animations[a].keyframeCount,
      anim[a].framerate
    );
    ...
    for (unsigned int j = 0; j < iqmHeader->num_poses; j++) {
      ...
      // NOTE: No need to store bones names, bones only required to generate
      // keyframe poses
      // if (iqmHeader->num_joints > 0) memcpy(bones[j].name, fileDataPtr +
      // iqmHeader->ofs_text + joints[j].name, BONE_NAME_LENGTH*sizeof(char));
      // else memcpy(bones[j].name, "ANIMJOINTNAME", 13); // Default bone name
      // otherwise
    }

    for (unsigned int j = 0; j < anim[a].num_frames; j++)
      animations[a].keyframePoses[j] =
        (rlTransform*) RL_MALLOC(iqmHeader->num_poses * sizeof(rlTransform));

    int dcounter = anim[a].first_frame * iqmHeader->num_framechannels;

    for (unsigned int frame = 0; frame < anim[a].num_frames; frame++) {
      for (unsigned int i = 0; i < iqmHeader->num_poses; i++) {
        animations[a].keyframePoses[frame][i].translation.x =
          poses[i].channeloffset[0];
        ...
        animations[a].keyframePoses[frame][i].translation.y =
          poses[i].channeloffset[1];
        ...
        animations[a].keyframePoses[frame][i].translation.z =
          poses[i].channeloffset[2];
        ...
        animations[a].keyframePoses[frame][i].rotation.x =
          poses[i].channeloffset[3];
        ...
        animations[a].keyframePoses[frame][i].rotation.y =
          poses[i].channeloffset[4];
        ...
        animations[a].keyframePoses[frame][i].rotation.z =
          poses[i].channeloffset[5];
        ...
        animations[a].keyframePoses[frame][i].rotation.w =
          poses[i].channeloffset[6];
        ...
        animations[a].keyframePoses[frame][i].scale.x =
          poses[i].channeloffset[7];
        ...
        animations[a].keyframePoses[frame][i].scale.y =
          poses[i].channeloffset[8];
        ...
        animations[a].keyframePoses[frame][i].scale.z =
          poses[i].channeloffset[9];
        ...
        animations[a].keyframePoses[frame][i].rotation =
          QuaternionNormalize(animations[a].keyframePoses[frame][i].rotation);
      }
    }
    ...
    for (unsigned int frame = 0; frame < anim[a].num_frames; frame++) {
      for (int i = 0; i < animations[a].boneCount; i++) {
        if (bones[i].parent >= 0) {
          animations[a].keyframePoses[frame][i].rotation = QuaternionMultiply(
            animations[a].keyframePoses[frame][bones[i].parent].rotation,
            animations[a].keyframePoses[frame][i].rotation
          );
          animations[a].keyframePoses[frame][i].translation =
            Vector3RotateByQuaternion(
              animations[a].keyframePoses[frame][i].translation,
              animations[a].keyframePoses[frame][bones[i].parent].rotation
            );
          animations[a].keyframePoses[frame][i].translation = Vector3Add(
            animations[a].keyframePoses[frame][i].translation,
            animations[a].keyframePoses[frame][bones[i].parent].translation
          );
          animations[a].keyframePoses[frame][i].scale = Vector3Multiply(
            animations[a].keyframePoses[frame][i].scale,
            animations[a].keyframePoses[frame][bones[i].parent].scale
          );
        }
      }
    }
...
static Image LoadImageFromCgltfImage(
  cgltf_image* cgltfImage, char const* texPath
) {
  Image image = {0};
  ...
  if (cgltfImage->uri !=
      NULL) // Check if image data is provided as an uri (base64 or path)
  {
    if ((strlen(cgltfImage->uri) > 5) && (cgltfImage->uri[0] == 'd') &&
        (cgltfImage->uri[1] == 'a') && (cgltfImage->uri[2] == 't') &&
        (cgltfImage->uri[3] == 'a') &&
        (cgltfImage->uri[4] ==
         ':')) // Check if image is provided as base64 text data
    {
      ...
      if (cgltfImage->uri[i] == 0)
        ...
        int numberOfEncodedBits =
          base64Size * 6 -
          (base64Size * 6) % 8; // Encoded bits minus extra bits, so it becomes
        ...
        cgltf_result result = cgltf_load_buffer_base64(
          &options, outSize, cgltfImage->uri + i + 1, &data
        );
  ...
  } else if ((cgltfImage->buffer_view != NULL) &&
             (cgltfImage->buffer_view->buffer->data !=
              NULL)) // Check if image is provided as data buffer
  {
    unsigned char* data =
      (unsigned char*) RL_MALLOC(cgltfImage->buffer_view->size);
    int offset = (int) cgltfImage->buffer_view->offset;
    int stride = (int) cgltfImage->buffer_view->stride
    ...
    for (unsigned int i = 0; i < cgltfImage->buffer_view->size; i++) {
      data[i] =
        ((unsigned char*) cgltfImage->buffer_view->buffer->data)[offset];
      offset += stride;
    }
    ...
    if ((strcmp(cgltfImage->mime_type, "image\\/png") == 0) ||
      ...
      image =
        LoadImageFromMemory(".png", data, (int) cgltfImage->buffer_view->size);
    } else if ((strcmp(cgltfImage->mime_type, "image\\/jpeg") == 0) ||
               (strcmp(cgltfImage->mime_type, "image/jpeg") == 0)) {
      image =
        LoadImageFromMemory(".jpg", data, (int) cgltfImage->buffer_view->size);
    } else
      TRACELOG(
        LOG_WARNING,
...
static Model LoadGLTF(char const* fileName) {
/*********************************************************************************************

    Function implemented by Wilhem Barbier(@wbrbr), with modifications by Tyler
Bezera(@gamerfiend) Transform handling implemented by Paul Melis (@paulmelis)
    Reviewed by Ramon Santamaria (@raysan5)

    FEATURES:
      - Supports .gltf and .glb files
      - Supports embedded (base64) or external textures
      - Supports PBR metallic/roughness flow, loads material textures, values
and colors PBR specular/glossiness flow and extended texture flows not supported
      - Supports multiple meshes per model (every primitives is loaded as a
separate mesh)
      - Supports basic animations
      - Transforms, including parent-child relations, are applied on the mesh
data, but the hierarchy is not kept (as it can't be represented)
      - Mesh instances in the glTF file (a.e. same mesh linked from multiple
nodes) are turned into separate raylib Meshes
    ...
    RESTRICTIONS:
      ...
      - Scenes defined in the glTF file are ignored. All nodes in the file are
used
...
#define LOAD_ATTRIBUTE_CAST(accesor, numComp, srcType, dstPtr, dstType)        \
  ...
  if (result == cgltf_result_success) {
    if (data->file_type == cgltf_file_type_glb)
      TRACELOG(
        LOG_INFO,
        "MODEL: [%s] Model basic data (glb) loaded successfully",
        fileName
      );
    else if (data->file_type == cgltf_file_type_gltf)
      TRACELOG(
        LOG_INFO,
        "MODEL: [%s] Model basic data (glTF) loaded successfully",
        fileName
      );
    else
      TRACELOG(
        LOG_WARNING, "MODEL: [%s] Model format not recognized", fileName
      );

    TRACELOG(LOG_INFO, "    > Meshes count: %i", data->meshes_count);
    TRACELOG(
      LOG_INFO, "    > Materials count: %i (+1 default)", data->materials_count
    );
    ...
    // NOTE: If an uri is defined to base64 data or external path, it's
    // automatically loaded
    result = cgltf_load_buffers(&options, data, fileName);
    if (result != cgltf_result_success)
      TRACELOG(
        LOG_INFO, "MODEL: [%s] Failed to load mesh/material buffers", fileName
      );
    ...
    for (unsigned int i = 0; i < data->nodes_count; i++) {
      ...
      for (unsigned int p = 0; p < mesh->primitives_count; p++) {
        if (mesh->primitives[p].has_draco_mesh_compression) {
          ...
          TRACELOG(
            LOG_WARNING,
            "MODEL: [%s] Failed to load mesh data, Draco compression not "
            "supported",
            fileName
          );
          break;
        } else if (mesh->primitives[p].type == cgltf_primitive_type_triangles)
          primitivesCount++;
      }
    }
    ...
    TRACELOG(
      LOG_DEBUG,
      "    > Primitives (triangles only) count based on hierarchy : %i",
      primitivesCount
    );
    ...
    // NOTE: Keep an extra slot for default material, in case some mesh requires
    // it
    model.materialCount = (int) data->materials_count + 1;
    model.materials =
      (rlMaterial*) RL_CALLOC(model.materialCount, sizeof(rlMaterial));
    model.materials[0] =
      LoadMaterialDefault(); // Load default material (index: 0)

    // Load mesh-material indices, by default all meshes are mapped to material
    // index: 0
    model.meshMaterial = (int*) RL_CALLOC(model.meshCount, sizeof(int));
    ...
    for (unsigned int i = 0, j = 1; i < data->materials_count; i++, j++) {
      ...
      char const* texPath = GetDirectoryPath(fileName);
      ...
      if (data->materials[i].has_pbr_metallic_roughness) {
        ...
        if (data->materials[i]
          ...
          Image imAlbedo = LoadImageFromCgltfImage(
            data->materials[i]
              .pbr_metallic_roughness.base_color_texture.texture->image,
            texPath
          );
        ...
        model.materials[j].maps[MATERIAL_MAP_ALBEDO].color.r =
          (unsigned char) (data->materials[i]
        ...
        if (data->materials[i]
          ...
          Image imMetallicRoughness = LoadImageFromCgltfImage(
            data->materials[i]
              .pbr_metallic_roughness.metallic_roughness_texture.texture->image,
            texPath
          );
          if (imMetallicRoughness.data != NULL) {
            ...
            imMetallic.data =
              RL_MALLOC(imMetallicRoughness.width * imMetallicRoughness.height);
            imRoughness.data =
              RL_MALLOC(imMetallicRoughness.width * imMetallicRoughness.height);
            ...
            imMetallic.format = imRoughness.format =
              PIXELFORMAT_UNCOMPRESSED_GRAYSCALE;
            imMetallic.mipmaps = imRoughness.mipmaps = 1;

            for (int x = 0; x < imRoughness.width; x++) {
              for (int y = 0; y < imRoughness.height; y++) {
                rlColor color = GetImageColor(imMetallicRoughness, x, y);

                ((unsigned char*) imRoughness.data)[y * imRoughness.width + x] =
                  color.g; // Roughness color channel
                ((unsigned char*) imMetallic.data)[y * imMetallic.width + x] =
                  color.b; // Metallic color channel
              }
            }

            model.materials[j].maps[MATERIAL_MAP_ROUGHNESS].texture =
              LoadTextureFromImage(imRoughness);
            model.materials[j].maps[MATERIAL_MAP_METALNESS].texture =
              LoadTextureFromImage(imMetallic);
          ...
          float roughness =
            data->materials[i].pbr_metallic_roughness.roughness_factor;
          model.materials[j].maps[MATERIAL_MAP_ROUGHNESS].value = roughness;

          float metallic =
            data->materials[i].pbr_metallic_roughness.metallic_factor;
          model.materials[j].maps[MATERIAL_MAP_METALNESS].value = metallic;
        }

        // Load normal texture
        if (data->materials[i].normal_texture.texture) {
          Image imNormal = LoadImageFromCgltfImage(
            data->materials[i].normal_texture.texture->image, texPath
          );
          if (imNormal.data != NULL) {
            model.materials[j].maps[MATERIAL_MAP_NORMAL].texture =
              LoadTextureFromImage(imNormal);
        ...
        if (data->materials[i].occlusion_texture.texture) {
          Image imOcclusion = LoadImageFromCgltfImage(
        ...
        if (data->materials[i].emissive_texture.texture) {
          Image imEmissive = LoadImageFromCgltfImage(
            data->materials[i].emissive_texture.texture->image, texPath
          );
      ...
      // has_clearcoat, has_transmission, has_volume, has_ior, has specular,
      // has_sheen
    }
    //----------------------------------------------------------------------------------------------------
    ...
    // Load meshes data
    //
    // NOTE: Visit each node in the hierarchy and process any mesh linked from
    // it
    ...
    //  - Any glTF mesh linked from more than one Node (a.e. instancing) is
    //  turned into multiple Mesh's, as each Node will have its own transform
    //  applied
    //
    // WARNING: The code below disregards the scenes defined in the file, all
    // nodes are used
    //----------------------------------------------------------------------------------------------------
    int meshIndex = 0;
    for (unsigned int i = 0; i < data->nodes_count; i++) {
      ...
      for (unsigned int p = 0; p < mesh->primitives_count; p++) {
        ...
        // NOTE: Attributes data could be provided in several data formats (8,
        // 8u, 16u, 32...), Only some formats for each attribute type are
        // supported, read info at the top of this function!

        for (unsigned int j = 0; j < mesh->primitives[p].attributes_count;
             j++) {
              ...
              cgltf_attribute_type_position) // POSITION, vec3, float
          {
            ...
            // WARNING: SPECS: POSITION accessor MUST have its min and max
            // properties defined

            if (model.meshes[meshIndex].vertices != NULL)
              TRACELOG(
                LOG_WARNING,
                "MODEL: [%s] Vertices attribute data already loaded",
                fileName
              );
            else {
              if ((attribute->type == cgltf_type_vec3) &&
                ...
                LOAD_ATTRIBUTE(
                  attribute, 3, float, model.meshes[meshIndex].vertices
                )
                ...
                // Load data into a temp buffer to be converted to raylib data
                // type
                unsigned short* temp = (unsigned short*) RL_MALLOC(
                  attribute->count * 3 * sizeof(unsigned short)
                );
                LOAD_ATTRIBUTE(attribute, 3, unsigned short, temp);

                // Convert data to raylib vertex data type (float) the matrix
                // will scale it to the correct size as a float
                ...
                // type
                short* temp =
                  (short*) RL_MALLOC(attribute->count * 3 * sizeof(short));
                ...
                TRACELOG(
                  LOG_WARNING,
                  "MODEL: [%s] Vertices attribute data format not supported, "
                  "use vec3 float",
                  fileName
                );
            }
          } else if (mesh->primitives[p].attributes[j].type ==
                     cgltf_attribute_type_normal) // NORMAL, vec3, float
          {
            cgltf_accessor* attribute = mesh->primitives[p].attributes[j].data;
            ...
            if (model.meshes[meshIndex].normals != NULL)
              TRACELOG(
                LOG_WARNING,
                "MODEL: [%s] Normals attribute data already loaded",
                fileName
              );
            else {
              if ((attribute->type == cgltf_type_vec3) &&
                ...
                LOAD_ATTRIBUTE(
                  attribute, 3, float, model.meshes[meshIndex].normals
                )
                ...
                float* normals = model.meshes[meshIndex].normals;
                for (unsigned int k = 0; k < attribute->count; k++) {
                  Vector3 nt = Vector3Normalize(Vector3Transform(
                    (Vector3) {normals[3 * k],
                               normals[3 * k + 1],
                               normals[3 * k + 2]},
                    worldMatrixNormals
                ...
                // Load data into a temp buffer to be converted to raylib data
                // type
                unsigned char* temp = (unsigned char*) RL_MALLOC(
                  attribute->count * 3 * sizeof(unsigned char)
                );
                ...
                // type
                char* temp =
                  (char*) RL_MALLOC(attribute->count * 3 * sizeof(char));
                ...
                TRACELOG(
                  LOG_WARNING,
                  "MODEL: [%s] Normals attribute data format not supported, "
                  "use vec3 float",
                  fileName
                );
            }
          } else if (mesh->primitives[p].attributes[j].type ==
                     cgltf_attribute_type_tangent) // TANGENT, vec4, float, w is
                                                   // tangent basis sign
          {
            cgltf_accessor* attribute = mesh->primitives[p].attributes[j].data;
            ...
            if (model.meshes[meshIndex].tangents != NULL)
              TRACELOG(
                LOG_WARNING,
                "MODEL: [%s] Tangents attribute data already loaded",
                fileName
              );
            else {
              if ((attribute->type == cgltf_type_vec4) &&
                ...
                LOAD_ATTRIBUTE(
                  attribute, 4, float, model.meshes[meshIndex].tangents
                )
                ...
                TRACELOG(
                  LOG_WARNING,
                  "MODEL: [%s] Tangents attribute data format not supported, "
                  "use vec4 float",
                  fileName
                );
            }
          } else if (mesh->primitives[p].attributes[j].type ==
                     cgltf_attribute_type_texcoord) // TEXCOORD_n, vec2,
                                                    // float/u8n/u16n
          {
            ...
            if (attribute->type == cgltf_type_vec2) {
              if (attribute->component_type ==
                  cgltf_component_type_r_32f) // vec2, float
              {
                // Init raylib mesh texcoords to copy glTF attribute data
                texcoordPtr =
                  (float*) RL_MALLOC(attribute->count * 2 * sizeof(float));

                // Load 3 components of float data type into mesh.texcoords
                LOAD_ATTRIBUTE(attribute, 2, float, texcoordPtr)
              } else if (attribute->component_type ==
                         cgltf_component_type_r_8u) // vec2, u8n
              {
                // Init raylib mesh texcoords to copy glTF attribute data
                texcoordPtr =
                  (float*) RL_MALLOC(attribute->count * 2 * sizeof(float));
                ...
                // type
                unsigned char* temp = (unsigned char*) RL_MALLOC(
                  attribute->count * 2 * sizeof(unsigned char)
                );
                ...
                RL_FREE(temp);
                         ...
                         cgltf_component_type_r_16u) // vec2, u16n
              {
                // Init raylib mesh texcoords to copy glTF attribute data
                texcoordPtr =
                  (float*) RL_MALLOC(attribute->count * 2 * sizeof(float));
                ...
                // type
                unsigned short* temp = (unsigned short*) RL_MALLOC(
                  attribute->count * 2 * sizeof(unsigned short)
                );
            ...
            if (index == 0)
              ...
              TRACELOG(
                LOG_WARNING,
                "MODEL: [%s] No more than 2 texture coordinates attributes "
                "supported",
                fileName
              );
              if (texcoordPtr != NULL)
                RL_FREE(texcoordPtr);
                     ...
                     cgltf_attribute_type_color) // COLOR_n, vec3/vec4,
                                                 // float/u8n/u16n
          {
            cgltf_accessor* attribute = mesh->primitives[p].attributes[j].data;

            // WARNING: SPECS: All components of each COLOR_n accessor element
            // MUST be clamped to [0.0, 1.0] range

            if (model.meshes[meshIndex].colors != NULL)
              TRACELOG(
                LOG_WARNING,
                "MODEL: [%s] Colors attribute data already loaded",
                fileName
              );
            else {
              if (attribute->type == cgltf_type_vec3) // RGB
              {
                if (attribute->component_type == cgltf_component_type_r_8u) {
                  ...
                  model.meshes[meshIndex].colors = (unsigned char*) RL_MALLOC(
                    attribute->count * 4 * sizeof(unsigned char)
                  );
                  ...
                  for (unsigned int c = 0, k = 0;
                    ...
                    model.meshes[meshIndex].colors[c + 1] =
                      (unsigned char) (((float) temp[k + 1] / 65535.0f) *
                                       255.0f);
                    model.meshes[meshIndex].colors[c + 2] =
                      (unsigned char) (((float) temp[k + 2] / 65535.0f) *
                                       255.0f);
                    model.meshes[meshIndex].colors[c + 3] = 255;
                  }
              ...
              } else if (attribute->type == cgltf_type_vec4) // RGBA
              {
                if (attribute->component_type == cgltf_component_type_r_8u) {
                  ...
                  // Load 4 components of unsigned char data type into
                  // mesh.colors
                  LOAD_ATTRIBUTE(
                    attribute, 4, unsigned char, model.meshes[meshIndex].colors
                  )
                } else if (attribute->component_type ==
                           cgltf_component_type_r_16u) {
                  // Init raylib mesh color to copy glTF attribute data
                  ...
                  // type
                  unsigned short* temp = (unsigned short*) RL_MALLOC(
                    attribute->count * 4 * sizeof(unsigned short)
                  );
                  ...
                  // Convert data to raylib color data type (4 bytes), color
          ...
          // NOTE: Attributes related to animations data are processed after
          // mesh data loading
        }

        // Load primitive indices data (if provided)
        if ((mesh->primitives[p].indices != NULL) &&
            (mesh->primitives[p].indices->buffer_view != NULL)) {
          ...
          if (model.meshes[meshIndex].indices != NULL)
            TRACELOG(
              LOG_WARNING,
              "MODEL: [%s] Indices attribute data already loaded",
              fileName
            );
          else {
            if (attribute->component_type == cgltf_component_type_r_16u) {
              // Init raylib mesh indices to copy glTF attribute data
              model.meshes[meshIndex].indices = (unsigned short*) RL_MALLOC(
                attribute->count * sizeof(unsigned short)
              );

              // Load unsigned short data type into mesh.indices
              LOAD_ATTRIBUTE(
                attribute, 1, unsigned short, model.meshes[meshIndex].indices
              )
              ...
              TRACELOG(
                LOG_WARNING,
                "MODEL: [%s] Indices data converted from u32 to u16, possible "
                "loss of data",
                fileName
              );
            } else
              TRACELOG(
                LOG_WARNING,
                "MODEL: [%s] Indices data format not supported, use u16",
                fileName
              );
          }
        } else
        ...
        // NOTE: If no material defined, mesh uses the already assigned default
        // material (index: 0)
        for (unsigned int m = 0; m < data->materials_count; m++) {
          // The primitive actually keeps the pointer to the corresponding
          // material, raylib instead assigns to the mesh the by its index, as
          // loaded in model.materials array To get the index, check if material
          // pointers match, and assign the corresponding index, skipping index
          // 0, the default material
    ...
    //
    // LIMITATIONS:
    //  - Only supports 1 armature per file, and skips loading it if there are
    //  multiple armatures
    //  - Only supports linear interpolation (default method in Blender when
    //  checked "Always Sample Animations" when exporting a GLTF file)
    //  - Only supports translation/rotation/scale animation channel.path,
    //  weights not considered (a.e. morph targets)
    //----------------------------------------------------------------------------------------------------
    if (data->skins_count > 0) {
      ...
      if (data->skins_count > 1)
        TRACELOG(
          LOG_WARNING,
          "MODEL: [%s] can only load one skin (armature) per model, but gltf "
          "skins_count == %i",
          fileName,
          data->skins_count
        );
    ...
    for (unsigned int i = 0; i < data->nodes_count; i++) {
      ...
      for (unsigned int p = 0; p < mesh->primitives_count; p++) {
        ...
        for (unsigned int j = 0; j < mesh->primitives[p].attributes_count;
             j++) {
          // NOTE: JOINTS_1 + WEIGHT_1 will be used for +4 joints influencing a
          // vertex -> Not supported by raylib
          if (mesh->primitives[p].attributes[j].type ==
              cgltf_attribute_type_joints) // JOINTS_n (vec4: 4 bones max per
                                           // vertex / u8, u16)
          {
            hasJoints = true;
            cgltf_accessor* attribute = mesh->primitives[p].attributes[j].data;
            ...
            // WARNING: raylib only supports model.meshes[].boneIndices as u8
            // (unsigned char), if data is provided in any other format, it is
            // converted to supported format but it could imply data loss (a
            // warning message is issued in that case)

            if (attribute->type == cgltf_type_vec4) {
              if (attribute->component_type == cgltf_component_type_r_8u) {
                // Init raylib mesh boneIndices to copy glTF attribute data
                model.meshes[meshIndex].boneIndices =
                  (unsigned char*) RL_CALLOC(
                    model.meshes[meshIndex].vertexCount * 4,
                ...
                // Load data into a temp buffer to be converted to raylib data
                // type
                unsigned short* temp = (unsigned short*) RL_CALLOC(
                  model.meshes[meshIndex].vertexCount * 4,
                  sizeof(unsigned short)
                );
                LOAD_ATTRIBUTE(attribute, 4, unsigned short, temp);
                ...
                for (int b = 0; b < model.meshes[meshIndex].vertexCount * 4;
                     b++) {
                  ...
                  // Despite the possible overflow, convert data to unsigned
                  // char
                  model.meshes[meshIndex].boneIndices[b] =
              ...
              TRACELOG(
                ...
                fileName
                     ...
                     cgltf_attribute_type_weights) // WEIGHTS_n (vec4,
                                                   // u8n/u16n/f32)
          {
            cgltf_accessor* attribute = mesh->primitives[p].attributes[j].data;
            ...
            if (attribute->type == cgltf_type_vec4) {
              if (attribute->component_type == cgltf_component_type_r_8u) {
                ...
                model.meshes[meshIndex].boneWeights = (float*) RL_CALLOC(
                  model.meshes[meshIndex].vertexCount * 4, sizeof(float)
                );
                ...
                //   - 256 values, provided as cgltf_type_vec4 of float (4 byte
                //   per joint, stride 16)
                LOAD_ATTRIBUTE(
                  attribute, 4, float, model.meshes[meshIndex].boneWeights
                )
              } else
                TRACELOG(
                  LOG_WARNING,
                  "MODEL: [%s] Joint weight attribute data format not "
                  "supported, use vec4 float",
                  fileName
                );
            } else
              TRACELOG(
                LOG_WARNING,
                "MODEL: [%s] Joint weight attribute data format not supported, "
                "use vec4 float",
                fileName
              );
          }
        ...
        // Check if animated, and the mesh was not given any bone assignments,
        // but is the child of a bone node in this case, all the verts need to
        // be attached to the parent bone so it will animate with the bone
        if ((data->skins_count > 0) && !hasJoints && (node->parent != NULL) &&
            (node->parent->mesh == NULL)) {
          int parentBoneId = -1;
          ...
          if (parentBoneId >= 0) {
            model.meshes[meshIndex].boneIndices = (unsigned char*) RL_CALLOC(
            ...
            for (int vertexIndex = 0;
                 vertexIndex < model.meshes[meshIndex].vertexCount * 4;
                 vertexIndex += 4) {
              model.meshes[meshIndex].boneIndices[vertexIndex] =
                (unsigned char) parentBoneId;
        ...
        model.meshes[meshIndex].animVertices = (float*) RL_CALLOC(
        ...
        model.meshes[meshIndex].animNormals = (float*) RL_CALLOC(
          model.meshes[meshIndex].vertexCount * 3, sizeof(float)
        );
        if (model.meshes[meshIndex].normals != NULL)
          memcpy(
...
// Get interpolated pose for bone sampler at a specific time. Returns true on
// success
...
static ModelAnimation* LoadModelAnimationsGLTF(
  const char* fileName, int* animCount
) {
  // glTF file loading
  int dataSize = 0;
  unsigned char* fileData = LoadFileData(fileName, &dataSize);
  ...
  if (result != cgltf_result_success)
    TRACELOG(
      LOG_INFO, "MODEL: [%s] Failed to load animation buffers", fileName
    );
  ...
  if (result == cgltf_result_success) {
    if (data->skins_count > 0) {
      ...
      *animCount = (int) data->animations_count;
      animations = (ModelAnimation*) RL_CALLOC(
        data->animations_count, sizeof(ModelAnimation)
      );

      rlTransform worldTransform = {0};
      ...
      Matrix worldMatrix = {
        cgltf_worldTransform[0],
        cgltf_worldTransform[4],
        cgltf_worldTransform[8],
        cgltf_worldTransform[12],
        cgltf_worldTransform[1],
        cgltf_worldTransform[5],
        cgltf_worldTransform[9],
        cgltf_worldTransform[13],
        cgltf_worldTransform[2],
        cgltf_worldTransform[6],
        cgltf_worldTransform[10],
        cgltf_worldTransform[14],
        cgltf_worldTransform[3],
        cgltf_worldTransform[7],
        cgltf_worldTransform[11],
        cgltf_worldTransform[15]
      };
      MatrixDecompose(
        worldMatrix,
      ...
      for (unsigned int a = 0; a < data->animations_count; a++) {
        ...
        struct Channels* boneChannels = (struct Channels*) RL_CALLOC(
          animations[a].boneCount, sizeof(struct Channels)
        );
        ...
        for (unsigned int j = 0; j < animData.channels_count; j++) {
          ...
          if (boneIndex == -1)
            continue; // Animation channel for a node not in the skeleton

          boneChannels[boneIndex].interpolationType =
            animData.channels[j].sampler->interpolation;

          if (animData.channels[j].sampler->interpolation !=
            ...
            if (channel.target_path == cgltf_animation_path_type_translation) {
              ...
              TRACELOG(
                LOG_WARNING,
                "MODEL: [%s] Unsupported target_path on channel %d's sampler "
                "for animation %d. Skipping.",
                fileName,
                j,
                a
              );
            ...
            TRACELOG(
              LOG_WARNING,
              "MODEL: [%s] Invalid interpolation curve encountered for GLTF "
              "animation.",
              fileName
            );

          float time = 0.0f;
          cgltf_bool result = cgltf_accessor_read_float(
            channel.sampler->input, channel.sampler->input->count - 1, &time, 1
          );

          if (!result) {
            TRACELOG(
              LOG_WARNING, "MODEL: [%s] Failed to load input time", fileName
            );
            continue;
          }
        ...
        if (animData.name != NULL)
          strncpy(
            animations[a].name, animData.name, sizeof(animations[a].name) - 1
          );

        animations[a].keyframeCount = (int) (animDuration * GLTF_FRAMERATE) + 1;
        animations[a].keyframePoses = (rlTransform**) RL_CALLOC(
          animations[a].keyframeCount, sizeof(rlTransform*)
        );

        for (int j = 0; j < animations[a].keyframeCount; j++) {
          animations[a].keyframePoses[j] = (rlTransform*) RL_CALLOC(
            animations[a].boneCount, sizeof(rlTransform)
          );
          ...
          for (int k = 0; k < animations[a].boneCount; k++) {
            ...
            animations[a].keyframePoses[j][k] = (rlTransform) {
              .translation = translation, .rotation = rotation, .scale = scale
            };
          }

          rlTransform* root = &animations[a].keyframePoses[j][0];
          root->rotation =
            QuaternionMultiply(worldTransform.rotation, root->rotation);
          root->scale = Vector3Multiply(root->scale, worldTransform.scale);
          root->translation =
            Vector3Multiply(root->translation, worldTransform.scale);
          root->translation = Vector3RotateByQuaternion(
            root->translation, worldTransform.rotation
          );
          root->translation =
            Vector3Add(root->translation, worldTransform.translation);

          BuildPoseFromParentJoints(
            bones, animations[a].boneCount, animations[a].keyframePoses[j]
          );
        }

        TRACELOG(
          LOG_INFO,
          "MODEL: [%s] Loaded animation: %s | Frames: %d | Duration: %fs",
          fileName,
    ...
    if (data->skins_count > 1) {
      TRACELOG(
        LOG_WARNING,
        "MODEL: [%s] Expected one unique skin to load animation data from, but "
        "found %i",
        fileName,
        data->skins_count
      );
...
static Model LoadVOX(char const* fileName) {
  Model model = {0};
  ...
  int verticesMax =
    65532; // 5461 voxels x 12 vertices per voxel -> 65532 (must be inf 65536)
  ...
  unsigned short* pindices =
    voxarray.indices.array; // 5461*6*6 = 196596 indices max per mesh
...
unsigned char* m3d_loaderhook(char* fn, unsigned int* len) {
  return LoadFileData((char const*) fn, (int*) len);
}
void m3d_freehook(void* data) { UnloadFileData((unsigned char*) data); }

// Load M3D mesh data
static Model LoadM3D(char const* fileName) {
  Model model = {0};
  ...
  if (fileData != NULL) {
    ...
    if (m3d->nummaterial > 0) {
      ...
      TRACELOG(
        LOG_INFO, "MODEL: model has %i material meshes", model.materialCount
      );
    } else {
      model.meshCount = 1;
      model.materialCount = 0;
    ...
    // Faces must be in non-decreasing materialid order. Verify that quickly,
    // sorting them otherwise WARNING: Sorting is not needed, valid M3D model
    // files should already be sorted Keeping the sorting function for reference
    // (Check PR #3363 #3385)
    /*
    ...
    */
    ...
    model.meshMaterial = (int*) RL_CALLOC(model.meshCount, sizeof(int));
    model.materials =
      (rlMaterial*) RL_CALLOC(model.materialCount + 1, sizeof(rlMaterial));

    // Map no material to index 0 with default shader, everything else
    // materialid + 1
    model.materials[0] = LoadMaterialDefault();
    ...
    for (i = l = 0, k = -1; i < (int) m3d->numface; i++, l++) {
      ...
      if (mi != m3d->face[i].materialid) {
        ...
        if (k + 1 >= model.meshCount) {
          ...
          rlMesh* tempMeshes =
            (rlMesh*) RL_CALLOC(model.meshCount, sizeof(rlMesh));
          memcpy(
            tempMeshes, model.meshes, (model.meshCount - 1) * sizeof(rlMesh)
          );
          RL_FREE(model.meshes);
          model.meshes = tempMeshes;

          // Create a second buffer for material re-allocation
          int* tempMeshMaterial =
            (int*) RL_CALLOC(model.meshCount, sizeof(int));
          memcpy(
            tempMeshMaterial,
            model.meshMaterial,
            (model.meshCount - 1) * sizeof(int)
          );
          RL_FREE(model.meshMaterial);
        ...
        // Only allocate colors VertexBuffer if there's a color vertex in the
        // model for this material batch if all colors are fully transparent
        // black for all verteces of this materal, then assuming no vertex
        // colors
        for (j = i, l = vcolor = 0;
             (j < (int) m3d->numface) && (mi == m3d->face[j].materialid);
             j++, l++) {
        ...
        model.meshes[k].vertices =
          (float*) RL_CALLOC(model.meshes[k].vertexCount * 3, sizeof(float));
        model.meshes[k].texcoords =
          (float*) RL_CALLOC(model.meshes[k].vertexCount * 2, sizeof(float));
        model.meshes[k].normals =
          (float*) RL_CALLOC(model.meshes[k].vertexCount * 3, sizeof(float));

        // If no map is provided, or colors are defined, allocate storage for
        // vertex colors M3D specs only consider vertex colors if no material is
        // provided, however raylib uses both and mixes the colors
        if ((mi == M3D_UNDEF) || vcolor)
          model.meshes[k].colors = (unsigned char*) RL_CALLOC(
            model.meshes[k].vertexCount * 4, sizeof(unsigned char)
          );

        // If no map is provided and vertex colors are allocated, set them to
        // white
        ...
        if (m3d->numbone && m3d->numskin) {
          model.meshes[k].boneIndices = (unsigned char*) RL_CALLOC(
            model.meshes[k].vertexCount * 4, sizeof(unsigned char)
          );
      ...
      model.meshes[k].vertices[l * 9 + 0] =
        m3d->vertex[m3d->face[i].vertex[0]].x * m3d->scale;
      model.meshes[k].vertices[l * 9 + 1] =
        m3d->vertex[m3d->face[i].vertex[0]].y * m3d->scale;
      model.meshes[k].vertices[l * 9 + 2] =
        m3d->vertex[m3d->face[i].vertex[0]].z * m3d->scale;
      model.meshes[k].vertices[l * 9 + 3] =
        m3d->vertex[m3d->face[i].vertex[1]].x * m3d->scale;
      model.meshes[k].vertices[l * 9 + 4] =
        m3d->vertex[m3d->face[i].vertex[1]].y * m3d->scale;
      model.meshes[k].vertices[l * 9 + 5] =
        m3d->vertex[m3d->face[i].vertex[1]].z * m3d->scale;
      model.meshes[k].vertices[l * 9 + 6] =
        m3d->vertex[m3d->face[i].vertex[2]].x * m3d->scale;
      model.meshes[k].vertices[l * 9 + 7] =
        m3d->vertex[m3d->face[i].vertex[2]].y * m3d->scale;
      model.meshes[k].vertices[l * 9 + 8] =
        m3d->vertex[m3d->face[i].vertex[2]].z * m3d->scale;
      ...
      if (model.meshes[k].colors != NULL) {
        if (m3d->vertex[m3d->face[i].vertex[0]].color & 0xff000000)
          memcpy(
            &model.meshes[k].colors[l * 12 + 0],
            &m3d->vertex[m3d->face[i].vertex[0]].color,
            4
          );
        if (m3d->vertex[m3d->face[i].vertex[1]].color & 0xff000000)
          memcpy(
            &model.meshes[k].colors[l * 12 + 4],
            &m3d->vertex[m3d->face[i].vertex[1]].color,
            4
          );
        if (m3d->vertex[m3d->face[i].vertex[2]].color & 0xff000000)
          memcpy(
            &model.meshes[k].colors[l * 12 + 8],
            &m3d->vertex[m3d->face[i].vertex[2]].color,
            4
          );
      }

      if (m3d->face[i].texcoord[0] != M3D_UNDEF) {
        model.meshes[k].texcoords[l * 6 + 0] =
          m3d->tmap[m3d->face[i].texcoord[0]].u;
        model.meshes[k].texcoords[l * 6 + 1] =
          1.0f - m3d->tmap[m3d->face[i].texcoord[0]].v;
        model.meshes[k].texcoords[l * 6 + 2] =
          m3d->tmap[m3d->face[i].texcoord[1]].u;
        model.meshes[k].texcoords[l * 6 + 3] =
          1.0f - m3d->tmap[m3d->face[i].texcoord[1]].v;
        model.meshes[k].texcoords[l * 6 + 4] =
          m3d->tmap[m3d->face[i].texcoord[2]].u;
        model.meshes[k].texcoords[l * 6 + 5] =
          1.0f - m3d->tmap[m3d->face[i].texcoord[2]].v;
      }

      if (m3d->face[i].normal[0] != M3D_UNDEF) {
        model.meshes[k].normals[l * 9 + 0] =
          m3d->vertex[m3d->face[i].normal[0]].x;
        model.meshes[k].normals[l * 9 + 1] =
          m3d->vertex[m3d->face[i].normal[0]].y;
        model.meshes[k].normals[l * 9 + 2] =
          m3d->vertex[m3d->face[i].normal[0]].z;
        model.meshes[k].normals[l * 9 + 3] =
          m3d->vertex[m3d->face[i].normal[1]].x;
        model.meshes[k].normals[l * 9 + 4] =
          m3d->vertex[m3d->face[i].normal[1]].y;
        model.meshes[k].normals[l * 9 + 5] =
          m3d->vertex[m3d->face[i].normal[1]].z;
        model.meshes[k].normals[l * 9 + 6] =
          m3d->vertex[m3d->face[i].normal[2]].x;
        model.meshes[k].normals[l * 9 + 7] =
          m3d->vertex[m3d->face[i].normal[2]].y;
        model.meshes[k].normals[l * 9 + 8] =
          m3d->vertex[m3d->face[i].normal[2]].z;
      }
      ...
      if (m3d->numbone && m3d->numskin) {
        for (n = 0; n < 3; n++) {
          ...
          if ((skinid != M3D_UNDEF) && (skinid < (int) m3d->numskin)) {
            for (j = 0; j < 4; j++) {
              model.meshes[k].boneIndices[l * 12 + n * 4 + j] =
                m3d->skin[skinid].boneid[j];
              model.meshes[k].boneWeights[l * 12 + n * 4 + j] =
                m3d->skin[skinid].weight[j];
            }
          } else {
    ...
    for (i = 0; i < (int) m3d->nummaterial; i++) {
      ...
      for (j = 0; j < m3d->material[i].numprop; j++) {
        ...
        case m3dp_Kd: {
          memcpy(
            &model.materials[i + 1].maps[MATERIAL_MAP_DIFFUSE].color,
            &prop->value.color,
            4
          );
        ...
        case m3dp_Ks: {
          memcpy(
            &model.materials[i + 1].maps[MATERIAL_MAP_SPECULAR].color,
            &prop->value.color,
            4
          );
        } break;
        case m3dp_Ns: {
          model.materials[i + 1].maps[MATERIAL_MAP_SPECULAR].value =
            prop->value.fnum;
        } break;
        case m3dp_Ke: {
          memcpy(
            &model.materials[i + 1].maps[MATERIAL_MAP_EMISSION].color,
            &prop->value.color,
            4
          );
        ...
        case m3dp_Pm: {
          model.materials[i + 1].maps[MATERIAL_MAP_METALNESS].value =
            prop->value.fnum;
        } break;
        case m3dp_Pr: {
          model.materials[i + 1].maps[MATERIAL_MAP_ROUGHNESS].value =
            prop->value.fnum;
        } break;
        case m3dp_Ps: {
          model.materials[i + 1].maps[MATERIAL_MAP_NORMAL].color = RLWHITE;
          model.materials[i + 1].maps[MATERIAL_MAP_NORMAL].value =
            prop->value.fnum;
        } break;
        default: {
          if (prop->type >= 128) {
            ...
            image.mipmaps = 1;
            image.format =
              (m3d->texture[prop->value.textureid].f == 4)
            ...
            case m3dp_map_Ke:
              ...
              break;
            case m3dp_map_Km:
              model.materials[i + 1].maps[MATERIAL_MAP_NORMAL].texture =
                LoadTextureFromImage(image);
              break;
            case m3dp_map_Ka:
    ...
    if (m3d->numbone) {
      ...
      model.skeleton.bones =
        (BoneInfo*) RL_CALLOC(model.skeleton.boneCount, sizeof(BoneInfo));
      model.skeleton.bindPose =
        (rlTransform*) RL_CALLOC(model.skeleton.boneCount, sizeof(rlTransform));
      ...
      for (i = 0; i < (int) m3d->numbone; i++) {
        ...
        strncpy(
          model.skeleton.bones[i].name,
          m3d->bone[i].name,
          sizeof(model.skeleton.bones[i].name) - 1
        );
        model.skeleton.bindPose[i].translation.x =
          m3d->vertex[m3d->bone[i].pos].x * m3d->scale;
        model.skeleton.bindPose[i].translation.y =
          m3d->vertex[m3d->bone[i].pos].y * m3d->scale;
        model.skeleton.bindPose[i].translation.z =
          m3d->vertex[m3d->bone[i].pos].z * m3d->scale;
        ...
        // NOTE: If the orientation quaternion is not normalized, then that's
        // encoding scaling
        model.skeleton.bindPose[i].rotation =
          QuaternionNormalize(model.skeleton.bindPose[i].rotation);
        model.skeleton.bindPose[i].scale.x =
          model.skeleton.bindPose[i].scale.y =
            model.skeleton.bindPose[i].scale.z = 1.0f;

        // Child bones are stored in parent bone relative space, convert that
        // into model space
        if (model.skeleton.bones[i].parent >= 0) {
          model.skeleton.bindPose[i].rotation = QuaternionMultiply(
            model.skeleton.bindPose[model.skeleton.bones[i].parent].rotation,
            model.skeleton.bindPose[i].rotation
          );
          model.skeleton.bindPose[i].translation = Vector3RotateByQuaternion(
            model.skeleton.bindPose[i].translation,
            model.skeleton.bindPose[model.skeleton.bones[i].parent].rotation
          );
          model.skeleton.bindPose[i].translation = Vector3Add(
            model.skeleton.bindPose[i].translation,
            model.skeleton.bindPose[model.skeleton.bones[i].parent].translation
          );
          model.skeleton.bindPose[i].scale = Vector3Multiply(
            model.skeleton.bindPose[i].scale,
            model.skeleton.bindPose[model.skeleton.bones[i].parent].scale
          );
        }
      }
      ...
      model.skeleton.bindPose[i].scale.x = model.skeleton.bindPose[i].scale.y =
        model.skeleton.bindPose[i].scale.z = 1.0f;
    }

    // Load bone-pose default mesh into animation vertices. These will be
    // updated when UpdateModelAnimation gets called, but not before, however
    // DrawMesh uses these if they exist (so not good if they are left empty)
    if (m3d->numbone && m3d->numskin) {
...
#define M3D_ANIMDELAY                                                          \
  17 // Animation frames delay, (~1000 ms/60 FPS = 16.666666* ms)
  ...
  *animCount = 0;
  ...
  if (fileData != NULL) {
    ...
    if (!m3d || M3D_ERR_ISFATAL(m3d->errcode)) {
      ...
      TRACELOG(
        LOG_INFO,
        "MODEL: [%s] M3D data loaded successfully: %i animations, %i bones, %i "
        "skins",
        fileName,
        m3d->numaction,
        m3d->numbone,
    ...
    // No animation or bones, exit out. skins are not required because some
    // people use one animation for N models
    ...
    animations =
      (ModelAnimation*) RL_CALLOC(m3d->numaction, sizeof(ModelAnimation));
    *animCount = m3d->numaction;

    for (unsigned int a = 0; a < m3d->numaction; a++) {
      animations[a].boneCount = m3d->numbone + 1;
      BoneInfo* bones =
        (BoneInfo*) RL_CALLOC((m3d->numbone + 1), sizeof(BoneInfo));
      ...
      strncpy(
        animations[a].name, m3d->action[a].name, sizeof(animations[a].name) - 1
      );
      ...
      // M3D stores frames at arbitrary intervals with sparse skeletons; Full
      // skeletons is required at regular intervals, so let the M3D SDK do the
      // heavy lifting and calculate interpolated bones
      for (i = 0; i < animations[a].keyframeCount; i++) {
        animations[a].keyframePoses[i] =
          (rlTransform*) RL_CALLOC((m3d->numbone + 1), sizeof(rlTransform));

        m3db_t* pose = m3d_pose(m3d, a, i * M3D_ANIMDELAY);

        if (pose != NULL) {
          for (j = 0; j < (int) m3d->numbone; j++) {
            animations[a].keyframePoses[i][j].translation.x =
              m3d->vertex[pose[j].pos].x * m3d->scale;
            animations[a].keyframePoses[i][j].translation.y =
              m3d->vertex[pose[j].pos].y * m3d->scale;
            animations[a].keyframePoses[i][j].translation.z =
              m3d->vertex[pose[j].pos].z * m3d->scale;
            animations[a].keyframePoses[i][j].rotation.x =
              m3d->vertex[pose[j].ori].x;
            animations[a].keyframePoses[i][j].rotation.y =
              m3d->vertex[pose[j].ori].y;
            animations[a].keyframePoses[i][j].rotation.z =
              m3d->vertex[pose[j].ori].z;
            animations[a].keyframePoses[i][j].rotation.w =
              m3d->vertex[pose[j].ori].w;
            animations[a].keyframePoses[i][j].rotation =
              QuaternionNormalize(animations[a].keyframePoses[i][j].rotation);
            animations[a].keyframePoses[i][j].scale.x =
              animations[a].keyframePoses[i][j].scale.y =
                animations[a].keyframePoses[i][j].scale.z = 1.0f;

            // Child bones are stored in parent bone relative space, convert
            // that into model space
            if (bones[j].parent >= 0) {
              animations[a].keyframePoses[i][j].rotation = QuaternionMultiply(
                animations[a].keyframePoses[i][bones[j].parent].rotation,
                animations[a].keyframePoses[i][j].rotation
              );
              animations[a].keyframePoses[i][j].translation =
                Vector3RotateByQuaternion(
                  animations[a].keyframePoses[i][j].translation,
                  animations[a].keyframePoses[i][bones[j].parent].rotation
                );
              animations[a].keyframePoses[i][j].translation = Vector3Add(
                animations[a].keyframePoses[i][j].translation,
                animations[a].keyframePoses[i][bones[j].parent].translation
              );
              animations[a].keyframePoses[i][j].scale = Vector3Multiply(
                animations[a].keyframePoses[i][j].scale,
                animations[a].keyframePoses[i][bones[j].parent].scale
              );
            }
          }
...
// NOTE: This function preserves node names and parent-child relationships in
// mesh metadata
GltfScene LoadGltfScene(char const* fileName) {
  GltfScene scene = {0};

#if SUPPORT_FILEFORMAT_GLTF
// Macro to simplify attributes loading code
#define LOAD_ATTRIBUTE_SCENE(accesor, numComp, srcType, dstPtr)                \
  LOAD_ATTRIBUTE_CAST_SCENE(accesor, numComp, srcType, dstPtr, srcType)
...
#define LOAD_ATTRIBUTE_CAST_SCENE(accesor, numComp, srcType, dstPtr, dstType)  \
  ...
  if (result == cgltf_result_success) {
    if (data->file_type == cgltf_file_type_glb)
      TRACELOG(
        LOG_INFO, "MODEL: [%s] GltfScene (glb) loaded successfully", fileName
      );
    else if (data->file_type == cgltf_file_type_gltf)
      TRACELOG(
        LOG_INFO, "MODEL: [%s] GltfScene (glTF) loaded successfully", fileName
      );
    ...
    for (unsigned int i = 0; i < data->nodes_count; i++) {
      ...
      for (unsigned int p = 0; p < mesh->primitives_count; p++) {
        if (mesh->primitives[p].has_draco_mesh_compression) {
          TRACELOG(
            LOG_WARNING, "MODEL: [%s] Draco compression not supported", fileName
          );
          cgltf_free(data);
          UnloadFileData(fileData);
          return scene;
    ...
    scene.materials =
      (rlMaterial*) RL_CALLOC(scene.materialCount, sizeof(rlMaterial));
    ...
    for (unsigned int i = 0, j = 1; i < data->materials_count; i++, j++) {
      ...
      if (data->materials[i].has_pbr_metallic_roughness) {
        if (data->materials[i]
          ...
          if (imAlbedo.data != NULL) {
            scene.materials[j].maps[MATERIAL_MAP_ALBEDO].texture =
              LoadTextureFromImage(imAlbedo);
            UnloadImage(imAlbedo);
          }
        }

        scene.materials[j].maps[MATERIAL_MAP_ALBEDO].color.r =
          (unsigned char) (data->materials[i]
                             .pbr_metallic_roughness.base_color_factor[0] *
                           255);
        scene.materials[j].maps[MATERIAL_MAP_ALBEDO].color.g =
          (unsigned char) (data->materials[i]
                             .pbr_metallic_roughness.base_color_factor[1] *
                           255);
        scene.materials[j].maps[MATERIAL_MAP_ALBEDO].color.b =
          (unsigned char) (data->materials[i]
                             .pbr_metallic_roughness.base_color_factor[2] *
                           255);
        scene.materials[j].maps[MATERIAL_MAP_ALBEDO].color.a =
          (unsigned char) (data->materials[i]
                             .pbr_metallic_roughness.base_color_factor[3] *
                           255);
        ...
        if (data->materials[i]
          ...
          if (imMetallicRoughness.data != NULL) {
            ...
            scene.materials[j].maps[MATERIAL_MAP_ROUGHNESS].texture =
              LoadTextureFromImage(imRoughness);
            scene.materials[j].maps[MATERIAL_MAP_METALNESS].texture =
              LoadTextureFromImage(imMetallic);
          ...
          scene.materials[j].maps[MATERIAL_MAP_ROUGHNESS].value =
            data->materials[i].pbr_metallic_roughness.roughness_factor;
          scene.materials[j].maps[MATERIAL_MAP_METALNESS].value =
            data->materials[i].pbr_metallic_roughness.metallic_factor;
        }
        ...
        if (data->materials[i].normal_texture.texture) {
          ...
          if (imNormal.data != NULL) {
            scene.materials[j].maps[MATERIAL_MAP_NORMAL].texture =
              LoadTextureFromImage(imNormal);
            UnloadImage(imNormal);
          }
        }
        ...
        if (data->materials[i].occlusion_texture.texture) {
          ...
          if (imOcclusion.data != NULL) {
            scene.materials[j].maps[MATERIAL_MAP_OCCLUSION].texture =
              LoadTextureFromImage(imOcclusion);
            UnloadImage(imOcclusion);
          }
        }
        ...
        if (data->materials[i].emissive_texture.texture) {
          ...
          if (imEmissive.data != NULL) {
            scene.materials[j].maps[MATERIAL_MAP_EMISSION].texture =
              LoadTextureFromImage(imEmissive);
            UnloadImage(imEmissive);
          }

          scene.materials[j].maps[MATERIAL_MAP_EMISSION].color.r =
            (unsigned char) (data->materials[i].emissive_factor[0] * 255);
          scene.materials[j].maps[MATERIAL_MAP_EMISSION].color.g =
            (unsigned char) (data->materials[i].emissive_factor[1] * 255);
          scene.materials[j].maps[MATERIAL_MAP_EMISSION].color.b =
            (unsigned char) (data->materials[i].emissive_factor[2] * 255);
      ...
      for (unsigned int p = 0; p < mesh->primitives_count; p++) {
        ...
        while (parent != NULL) {
          ...
          if (parentNodeIndex >= 0 &&
              parentNodeIndex < (int) data->nodes_count &&
              nodeToMeshId[parentNodeIndex] >= 0) {
        ...
        if (node->name != NULL)
          strncpy(scene.meshes[meshIndex].name, node->name, 63);
        else if (mesh->name != NULL)
          strncpy(scene.meshes[meshIndex].name, mesh->name, 63);
        ...
        for (unsigned int j = 0; j < mesh->primitives[p].attributes_count;
          ...
          if (mesh->primitives[p].attributes[j].type ==
            ...
            if ((attribute->type == cgltf_type_vec3) &&
              ...
              scene.meshes[meshIndex].vertices =
                (float*) RL_MALLOC(attribute->count * 3 * sizeof(float));
              LOAD_ATTRIBUTE_SCENE(
                attribute, 3, float, scene.meshes[meshIndex].vertices
              )
              ...
              for (unsigned int t = 0; t < attribute->count * 3; t++)
                scene.meshes[meshIndex].vertices[t] = (float) temp[t];
              RL_FREE(temp);
            ...
            if ((attribute->type == cgltf_type_vec3) &&
                (attribute->component_type == cgltf_component_type_r_32f)) {
              scene.meshes[meshIndex].normals =
                (float*) RL_MALLOC(attribute->count * 3 * sizeof(float));
              LOAD_ATTRIBUTE_SCENE(
                attribute, 3, float, scene.meshes[meshIndex].normals
              )
            ...
            int index = mesh->primitives[p].attributes[j].index;
            if (index == 0)
              scene.meshes[meshIndex].texcoords = texcoordPtr;
            else if (index == 1)
              scene.meshes[meshIndex].texcoords2 = texcoordPtr;
            else if (texcoordPtr != NULL)
              RL_FREE(texcoordPtr);
          } else if (mesh->primitives[p].attributes[j].type ==
            ...
            if (attribute->type == cgltf_type_vec4) {
              if (attribute->component_type == cgltf_component_type_r_8u) {
                scene.meshes[meshIndex].colors = (unsigned char*) RL_MALLOC(
                  attribute->count * 4 * sizeof(unsigned char)
                );
                LOAD_ATTRIBUTE_SCENE(
                  attribute, 4, unsigned char, scene.meshes[meshIndex].colors
                )
              } else if (attribute->component_type ==
                         cgltf_component_type_r_16u) {
                scene.meshes[meshIndex].colors = (unsigned char*) RL_MALLOC(
                ...
                for (unsigned int c = 0; c < attribute->count * 4; c++)
                  scene.meshes[meshIndex].colors[c] =
                    (unsigned char) (((float) temp[c] / 65535.0f) * 255.0f);
                RL_FREE(temp);
              } else if (attribute->component_type ==
                         cgltf_component_type_r_32f) {
              ...
              if (attribute->component_type == cgltf_component_type_r_32f) {
                ...
                for (unsigned int c = 0, k = 0; c < (attribute->count * 4 - 3);
                     c += 4, k += 3) {
                  scene.meshes[meshIndex].colors[c] =
                    (unsigned char) (temp[k] * 255.0f);
                  scene.meshes[meshIndex].colors[c + 1] =
                    (unsigned char) (temp[k + 1] * 255.0f);
                  scene.meshes[meshIndex].colors[c + 2] =
                    (unsigned char) (temp[k + 2] * 255.0f);
        ...
        if ((mesh->primitives[p].indices != NULL) &&
          ...
          if (attribute->component_type == cgltf_component_type_r_16u) {
            scene.meshes[meshIndex].indices = (unsigned short*) RL_MALLOC(
              attribute->count * sizeof(unsigned short)
            );
            LOAD_ATTRIBUTE_SCENE(
              attribute, 1, unsigned short, scene.meshes[meshIndex].indices
            )
          } else if (attribute->component_type == cgltf_component_type_r_8u) {
            scene.meshes[meshIndex].indices = (unsigned short*) RL_MALLOC(
              attribute->count * sizeof(unsigned short)
            );
            LOAD_ATTRIBUTE_CAST_SCENE(
              attribute,
              1,
              unsigned char,
              scene.meshes[meshIndex].indices,
              unsigned short
            )
          ...
          scene.meshes[meshIndex].triangleCount =
            scene.meshes[meshIndex].vertexCount / 3;
...
#undef LOAD_ATTRIBUTE_CAST_SCENE
  ...
  TRACELOG(
    LOG_WARNING,
    "MODEL: [%s] GLTF format not supported, recompile with "
...
void UnloadGltfScene(GltfScene scene) {
  ...
  for (int i = 0; i < scene.materialCount; i++)
    RL_FREE(scene.materials[i].maps);
