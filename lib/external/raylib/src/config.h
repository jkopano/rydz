 ...
 *
 *   This file defines the configuration flags for different raylib features
 * per-module
 *
 *   NOTE: Additional values are configured per-module and can be set on compile
 * time
 *
 *   LICENSE: zlib/libpng
 *
...
// KeyCallback() WARNING: It requires SUPPORT_FILEFORMAT_PNG flag
...
// NOTE: Below values are alread defined inside [rcore.c] so there is no need to
// be redefined here, in case it must be done, uncomment the required line and
// update the value; it can also be done on compilation with
// -DVALUE_TO_REDEFINE=128
//------------------------------------------------------------------------------------
// #define MAX_TRACELOG_MSG_LENGTH       256       // Max length of one
// trace-log message #define MAX_FILEPATH_CAPACITY        8192       // Maximum
// file paths capacity #define MAX_FILEPATH_LENGTH          4096       //
// Maximum length for filepaths (Linux PATH_MAX default value) #define
// MAX_KEYBOARD_KEYS             512       // Maximum number of keyboard keys
// supported #define MAX_MOUSE_BUTTONS               8       // Maximum number
// of mouse buttons supported #define MAX_GAMEPADS                    4       //
// Maximum number of gamepads supported #define MAX_GAMEPAD_AXES 8       //
// Maximum number of axes supported (per gamepad) #define MAX_GAMEPAD_BUTTONS 32
// // Maximum number of buttons supported (per gamepad) #define
// MAX_GAMEPAD_VIBRATION_TIME      2.0f    // Maximum vibration time in seconds
// #define MAX_TOUCH_POINTS               10       // Maximum number of touch
// points supported #define MAX_KEY_PRESSED_QUEUE          16       // Maximum
// number of keys in the key input queue #define MAX_CHAR_PRESSED_QUEUE 16 //
// Maximum number of characters in the char input queue #define
// MAX_DECOMPRESSION_SIZE         64       // Max size allocated for
// decompression in MB #define MAX_AUTOMATION_EVENTS       16384       //
...
// NOTE: Below values are alread defined inside [rlgl.h] so there is no need to
// be redefined here, in case it must be done, uncomment the required line and
// update the value; it can also be done on compilation with
// -DVALUE_TO_REDEFINE=128
//------------------------------------------------------------------------------------
...
// RL_CULL_DISTANCE_FAR                4000.0    // Default projection matrix
// far cull distance
...
// NOTE: When a new shader is loaded, locations are tried to be set for
// convenience, looking for the names defined here In case custom shader names
// are used, it's up to the user to set locations with GetShaderLocation*()
// functions
...
// // Bound by default to shader location:
// RL_DEFAULT_SHADER_ATTRIB_LOCATION_INSTANCETRANSFORM #define
...
// projection matrix #define RL_DEFAULT_SHADER_UNIFORM_NAME_MODEL "matModel" //
...
// diffuse (tint color, multiplied by texture color) #define
...
// perlin-noise, cellular... NOTE: Perlin noise requires stb_perlin.h library
...
// -DVALUE_TO_REDEFINE=128
//------------------------------------------------------------------------------------
// #define AUDIO_DEVICE_FORMAT     ma_format_f32    // Device output format
// (miniaudio: float-32bit) #define AUDIO_DEVICE_CHANNELS               2    //
// Device output channels: stereo #define AUDIO_DEVICE_SAMPLE_RATE            0
// // Device sample rate (device default) #define
// AUDIO_DEVICE_PERIOD_SIZE_IN_FRAMES  0    // Device period size (controls
// latency, 0 defaults to 10ms) #define MAX_AUDIO_BUFFER_POOL_CHANNELS     16 //
