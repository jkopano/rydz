...
inline auto load_image(char const* path) -> Image {
...
inline auto gen_image_with_color(i32 width, i32 height, ecs::Color color)
  -> Image {
  return rl::GenImageColor(width, height, color);
}

inline auto load_texture(char const* path) -> Texture {
  return rl::LoadTexture(path);
}

inline auto load_texture(std::string const& path) -> Texture {
...
inline auto load_sound(char const* path) -> Sound {
...
inline auto gen_plane(
  float width = 10.0F, float length = 10.0F, i32 res_x = 1, i32 res_z = 1
) -> Mesh {
  return rl::GenMeshPlane(width, length, res_x, res_z);
}

inline auto gen_cylinder(
  float radius = 0.5F, float height = 1.0F, i32 slices = 16
) -> Mesh {
  return rl::GenMeshCylinder(radius, height, slices);
}

inline auto gen_torus(
  float radius = 0.5F,
  float size = 0.2F,
  i32 radial_segments = 16,
  i32 sides = 16
) -> Mesh {
...
inline auto set_vertex_attribute(
  ...
  rl::rlSetVertexAttribute(
    index, component_count, type, normalized, stride, offset
  );
}
...
inline auto load_texture_cubemap(
  void const* data, i32 size, i32 format, i32 mipmaps
) -> u32 {
