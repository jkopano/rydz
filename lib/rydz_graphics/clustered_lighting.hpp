...
inline float cluster_slice_distance(const ClusterConfig &config,
  ...
  return gl::cluster_slice_distance(config, near_plane, far_plane, orthographic,
                                    slice_index);
}
...
inline ClusterGpuRecord build_cluster_record(const ClusterConfig &config,
                                             ...
                                             i32 tile_z) {
  return gl::build_cluster_record(config, inverse_projection, orthographic,
                                  near_plane, far_plane, tile_x, tile_y,
                                  tile_z);
}
